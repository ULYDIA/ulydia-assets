...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-prem-row{display:grid;grid-template-columns:96px 1fr;gap:12px;align-items:center;}
      /* FAQ */
...
function renderSkillTags(title, items){
  ...
  function renderPage(ctx){
    ...
    // Sponsor banner (wide)
    var __tb = renderTopBanner(ctx); if (__tb) root.appendChild(__tb);
    ...
    var __pc = renderPremiumCard(ctx); if (__pc) right.appendChild(__pc);
    ...
    var __fq = renderFAQ(faqs); if (__fq) root.appendChild(__fq);
  }

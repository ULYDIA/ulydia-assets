...
/* ===== ULYDIA_LANG_FINAL_FETCH_PATCH (FIX8) =====
   Goal: metier content is by final language, not by country.
   - Intercepts Worker calls to /v1/metier-page?slug=...&iso=XX
   - Adds &lang=FINAL_LANG automatically when missing.
   - FINAL_LANG resolved from /v1/countries?lang=<uiLang> (cached in sessionStorage 6h)
*/
(function(){
  try{
    if (window.__ULYDIA_LANG_FINAL_FETCH_PATCH__) return;
    window.__ULYDIA_LANG_FINAL_FETCH_PATCH__ = true;

    var WORKER = (window.ULYDIA_WORKER_URL || "").replace(/\/+$/,"");
    if(!WORKER) return;
    ...
    function getUiLang(){
      var l = (document.documentElement && document.documentElement.lang) ? String(document.documentElement.lang) : "fr";
      l = l.toLowerCase();
      return (l==="fr"||l==="en"||l==="de"||l==="es"||l==="it") ? l : "fr";
    }
    function cacheKey(uiLang){ return "ul_country_final_lang_map_v1_" + uiLang; }

    function loadMap(uiLang){
      try{
        var raw = sessionStorage.getItem(cacheKey(uiLang));
        if(!raw) return null;
        var obj = safeJsonParse(raw);
        if(!obj || !obj.ts || !obj.map) return null;
        if(now() - obj.ts > 6*60*60*1000) return null;
        return obj.map;
      }catch(e){ return null; }
    }
    function saveMap(uiLang, mapObj){
      try{
        sessionStorage.setItem(cacheKey(uiLang), JSON.stringify({ ts: now(), map: mapObj }));
      }catch(e){}
    }

    var inflight = null;

    function ensureMap(uiLang){
      var cached = loadMap(uiLang);
      if(cached) return Promise.resolve(cached);
      if(inflight) return inflight;

      inflight = origFetch(WORKER + "/v1/countries?lang=" + encodeURIComponent(uiLang), { credentials:"omit" })
        .then(function(r){ return r.text().then(function(t){ return { ok:r.ok, status:r.status, text:t }; }); })
        .then(function(resp){
          inflight = null;
          if(!resp.ok) return null;
          var data = safeJsonParse(resp.text);
          if(!data || data.ok !== true || !Array.isArray(data.countries)) return null;

          var mapObj = {};
          for(var i=0;i<data.countries.length;i++){
            var c = data.countries[i] || {};
            var iso = String(c.iso || c.code || "").toUpperCase();
            if(!/^[A-Z]{2}$/.test(iso)) continue;
            var fl = String(c.final_lang || c.lang_final || c.finalLang || uiLang).toLowerCase();
            if(!(fl==="fr"||fl==="en"||fl==="de"||fl==="es"||fl==="it")) fl = uiLang;
            mapObj[iso] = fl;
          }
          saveMap(uiLang, mapObj);
          return mapObj;
        })
        .catch(function(){ inflight = null; return null; });

      return inflight.then(function(m){ return m || {}; });
    }

    function addLangToMetierPageUrl(urlStr){
      try{
        var u = new URL(urlStr, location.origin);
        if(!u.pathname.includes("/v1/metier-page")) return null;
        var sp = u.searchParams;
        if(sp.get("lang")) return null;
        var iso = String(sp.get("iso") || "").toUpperCase();
        if(!iso) return null;
        var uiLang = getUiLang();
        return ensureMap(uiLang).then(function(mapObj){
          var fl = (mapObj && mapObj[iso]) ? mapObj[iso] : uiLang;
          sp.set("lang", fl);
          return u.toString();
        });
      }catch(e){
    ...
    window.fetch = function(input, init){
      try{
        ...
        if(urlStr && urlStr.indexOf("/v1/metier-page?") !== -1 && urlStr.indexOf("lang=") === -1){
          var p = addLangToMetierPageUrl(urlStr);
          if(p && typeof p.then === "function"){
            return p.then(function(newUrl){
              if(!newUrl) return origFetch(input, init);
              if(typeof input === "string") return origFetch(newUrl, init);
              // Request object: rebuild
              try{

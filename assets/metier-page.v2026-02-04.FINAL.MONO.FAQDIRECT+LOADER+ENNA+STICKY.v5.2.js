...
// ULYDIA BUILD MARKER
window.__ULYDIA_BUILD__ = 'v4.12.6 LOADER-UNIFY+FAST+SECTOR+STICKY';
try{console.log('[ULYDIA] Loaded', window.__ULYDIA_BUILD__);}catch(e){}
...
(function(){
  ...
  function ensureNAStyle(){
    ...
    s.textContent =
      '.ul-loader-wrap{max-width:1100px;margin:34px auto;padding:18px 22px;border-radius:16px;background:#fff;box-shadow:0 10px 30px rgba(15,23,42,.06);border:1px solid rgba(99,102,241,.18);display:flex;align-items:center;justify-content:center;gap:12px;font-family:Montserrat,system-ui;color:#475569}'
      +'.ul-loader-dot{width:18px;height:18px;border:3px solid #e5e7eb;border-top-color:#6366f1;border-radius:50%;animation:ulspin 1s linear infinite}'
      +'.ul-na-card{max-width:1100px;margin:24px auto;padding:22px;border-radius:16px;background:#fff;box-shadow:0 10px 30px rgba(15,23,42,.06);border:1px solid rgba(99,102,241,.18)}'
      +'.ul-na-badge{display:inline-flex;align-items:center;gap:8px;padding:6px 12px;border-radius:999px;background:#eef2ff;color:#3730a3;font-weight:700;font-family:Montserrat,system-ui;font-size:13px}'
      +'.ul-na-title{margin:12px 0 6px;font-size:22px;font-weight:800;color:#0f172a;font-family:Montserrat,system-ui}'
      +'.ul-na-text{color:#475569;line-height:1.6;font-family:Montserrat,system-ui}'
      +'@keyframes ulspin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}';
  ...
  window.showMetierLoader = function(){
    try{
      var now = Date.now();
      if (window.__ULYDIA_LAST_LOADER_SHOWN__ && (now - window.__ULYDIA_LAST_LOADER_SHOWN__) < 150) return;
      window.__ULYDIA_LAST_LOADER_SHOWN__ = now;
    }catch(e){}
    var root = document.getElementById('ulydia-metier-root');
    if (!root) return;
    try{
      // Use the same card renderer as the fiche (consistent look)
      var spinnerHtml = '<div style="display:flex;align-items:center;gap:12px;">'
        + '<span class="ul-loader-dot" aria-hidden="true"></span>'
        + '<span style="color:var(--text-light,#6b7280);font-weight:600;">Loading…</span>'
        + '</div>';
      root.innerHTML = '';
      root.appendChild(card('Loading', '', spinnerHtml));
    }catch(e){
      // fallback minimal
      root.innerHTML = '<div style="max-width:1100px;margin:24px auto;padding:18px;border:1px solid #e5e7eb;border-radius:16px;background:#fff;display:flex;align-items:center;gap:12px;"><span class="ul-loader-dot"></span><span>Loading…</span></div>';
    }
  };
  ...
  window.renderNotAvailableCard = function(country){
    ...
    var body = '<div style="color:var(--text-light,#6b7280);line-height:1.6;">'
      + 'This job description is not yet available in <b>'+ (c||'this country') +'</b>. '
      + 'Please change the country or pick another job using the filters above.'
      + '</div>';
    try{
      root.innerHTML = '';
      root.appendChild(card('Not available', '', body));
    }catch(e){
      root.innerHTML = '<div style="max-width:1100px;margin:24px auto;padding:18px;border:1px solid #e5e7eb;border-radius:16px;background:#fff;"><b>Not available</b><div style="margin-top:8px;">'+body+'</div></div>';
    }
  };
...
function ficheHasContent(f){
  if (!f) return false;
  // Require at least one meaningful field so we don't render slugs-only objects.
  var fields = [
    f.job_title, f.title, f.name, f.Nom,
    f.accroche, f.accroche_texte, f.accrocheTexte,
    f.overview, f.description, f.overview_html, f.description_html,
    f.missions, f.missions_html,
    f.environnements, f.environnements_html, f.work_environment,
    f.skills, f.competences, f.competences_html,
    f.profil_recherche, f.profile, f.profile_html,
    f.evolutions, f.evolutions_possibles, f.career_path
  ];
  for (var i=0;i<fields.length;i++){
    var t = stripHtmlToText(fields[i]);
    if (String(t||"").replace(/\s+/g," ").trim()) return true;
  }
  return false;
}
...
function fetchMetierPageData(metierSlug, iso){
  // Full Airtable payload via Worker (/v1/metier-page). Safe: never throw.
  return new Promise(function(resolve){
    try{
      var base = getWorkerUrl();
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-sponsor-square-link img{width:100%;display:block;aspect-ratio:1/1;object-fit:cover;border-radius:14px;} 
...
function renderPage(ctx){
    ...
    if (ctx && ctx.notAvailable){
      ...
      mb.innerHTML = '<div class="u-muted" style="padding:6px 0;">This job description is not yet available in the country</div>';
    ...
    if (__workerPayload){
      try{
        window.__ULYDIA_PAGE_DATA__ = {
          ...
          mpb: (__workerPayload.mpb || __workerPayload.metier_pays_blocs || __workerPayload.metierPaysBlocs || __workerPayload.mpb_blocks || null),
...
// Keep loader homogeneous: ensure it is visible at least MIN_LOADER_MS to avoid flicker
    ...
    if (!ctx.fiche) {
      try{
        if (window.renderNotAvailableCard) window.renderNotAvailableCard((ctx.country && (ctx.country.label||ctx.country.name||ctx.country.iso)) || ctx.iso || "");
        else {

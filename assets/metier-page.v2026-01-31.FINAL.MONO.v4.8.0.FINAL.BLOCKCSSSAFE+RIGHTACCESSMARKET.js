...
if (Array.isArray(ctx.mpb.blocks) && ctx.mpb.blocks.length){
  ctx.mpb.blocks.forEach(function(b){
    ...
    // Move MPB Access + Market to the RIGHT column (per request)
    if (mpb){
      if (mpb.acces_bloc){
        right.appendChild(card(ui(lang,'access'), 'pastel-green', cleanRichHTML(String(mpb.acces_bloc))));
      }
      if (mpb.marche_bloc){
        right.appendChild(card(ui(lang,'market'), 'pastel-cyan', cleanRichHTML(String(mpb.marche_bloc))));
      }
    }

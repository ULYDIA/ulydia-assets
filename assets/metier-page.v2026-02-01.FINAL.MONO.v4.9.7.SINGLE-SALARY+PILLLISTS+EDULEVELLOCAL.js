...
function renderPage(ctx){
    ...
    if (ctx.mpb){
      if (ctx.mpb.formation) {
        var eduLoc = (ctx.mpb.education_level_local || ctx.mpb.education_level || ctx.mpb.educationLevelLocal || ctx.mpb.educationLevel || '').toString().trim();
...
if (indItems.length){
  ...
  right.appendChild(card(ui(lang2,'key_indicators'), 'bg-purple', htmlInd));
      ...
      // (v4.9.7) Salary card removed here to avoid duplicates.
      ...
      if (ctx.mpb.certifications){
        var certArr = splitTagsLoose(ctx.mpb.certifications);

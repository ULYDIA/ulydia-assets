...
// ---------------------------------------------------------
  ...
  async function detectVisitorISO(){
    // 1) IPinfo if available (visitor country)
...
<div class="ul-faq-item" data-open="0">
  ...
  async function main(){
    ...
    const startISO = storedISO || visitorISO || url0.country;
    ...
    async function renderMetier(slug){
      ...
      const fallbackWide = pickNonSponsorBanner(countryObj, "wide", lang);
      const fallbackSquare = pickNonSponsorBanner(countryObj, "square", lang);

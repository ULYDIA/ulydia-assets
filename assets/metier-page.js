...
(() => {
  ...
  function relocateRootIfNeeded(root){
    try{
      if (root && document.head && document.head.contains(root)) {
        warn("#ulydia-metier-root was inside <head>. Moving it to <body>.");
        (document.body || document.documentElement).prepend(root);
      }
      const header = document.querySelector(".Header.Wrapper, header, [data-wf-element='header']");
      if (header && root) {
        const pos = header.compareDocumentPosition(root);
        const rootBeforeHeader = !!(pos & Node.DOCUMENT_POSITION_PRECEDING);
        if (rootBeforeHeader) {
          warn("#ulydia-metier-root was before header. Moving it after header.");
          header.insertAdjacentElement("afterend", root);
        }
      }
    } catch(e){
      log("relocateRootIfNeeded error", e);
    }
  }

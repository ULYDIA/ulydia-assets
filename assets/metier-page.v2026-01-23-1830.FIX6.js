...
*/
window.__METIER_PAGE_BUILD__ = "metier-page v2026-01-23 FIX6 (unified showLoaderOverlay + no white screen + FAQ DOM fallback + countries from catalog)";
...
(() => {
  ...
  (function injectBootStyle(){
    try{
      ...
      st.textContent =
        'html.ul-metier-loading body{opacity:1 !important;}' +
        'html.ul-metier-loading #ulydia-metier-root{opacity:0 !important;}' +
        'html.ul-metier-loading .ul-hide-while-loading{opacity:0 !important;}';
  ...
  function showLoader(){
    try{
      if (typeof showLoaderOverlay === "function") showLoaderOverlay();
    }catch(_){}
  }
  function hideLoader(){
    try{
      if (typeof hideLoaderOverlay === "function") hideLoader();
    }catch(_){}
  }

  // show immediately (prevents perceived "blank" time)
  showLoader();
...
setRich("description-title", f.description || "");
    ...
    function faqMatches(item){
      ...
      const ok = s2Parts.some(p => p === want || slugify(p) === wantS);
      if (!ok) return false;

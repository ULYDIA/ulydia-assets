/* ===== ULYDIA MONO FIX11 SHIM =====
- Fallback when /v1/metier-page?iso!=FR returns "empty" (country used for sponsor, but content is language-based)
- Dispatches ulysia:metier:rendered when #ulydia-metier-root gets content
==================================== */
(function(){
  try{
    var _fetch = window.fetch;
    if(typeof _fetch === "function" && !window.__ULYDIA_FETCH_SHIM_FIX11__){
      window.__ULYDIA_FETCH_SHIM_FIX11__ = true;

      function looksEmpty(obj){
        if(!obj || typeof obj !== "object") return true;
        if(obj.ok === false) return true;
        // heuristics: if any of these exist, it's likely a real page
        var keys = ["title","name","metier","job","hero","blocks","content","sections","faq","mpb","data","payload"];
        for(var i=0;i<keys.length;i++){
          if(obj[keys[i]] != null) return false;
        }
        // nested common shapes
        if(obj.page && (obj.page.title || obj.page.blocks || obj.page.sections)) return false;
        if(obj.metier && (obj.metier.title || obj.metier.blocks)) return false;
        if(obj.data && (obj.data.title || obj.data.blocks || obj.data.sections)) return false;
        return true;
      }
      ...
      window.fetch = async function(input, init){
        ...
        try{
          if(url && url.indexOf("/v1/metier-page") !== -1 && url.indexOf("iso=") !== -1){
            // first fetch
            var res1 = await _fetch(input, init);
            // try read json safely
            var clone = res1.clone();
            var txt = await clone.text();
            var j = null;
            try{ j = JSON.parse(txt); }catch(e){}
            var isoMatch = url.match(/[?&]iso=([A-Za-z]{2})/);
            var iso = isoMatch ? isoMatch[1].toUpperCase() : "";
            if(iso && iso !== "FR" && (j == null || looksEmpty(j))){
              // fallback to FR for content
              var url2 = url.replace(/([?&]iso=)[A-Za-z]{2}/, "$1FR");
              return _fetch(url2, init);
            }
            // return original response (recreate if we consumed body)
            return new Response(txt, { status: res1.status, statusText: res1.statusText, headers: res1.headers });
          }
        }catch(e){}
  ...
  // rendered event when root receives content
  try{
    var root = document.getElementById("ulydia-metier-root");
    if(root && !window.__ULYDIA_RENDER_OBS_FIX11__){
      window.__ULYDIA_RENDER_OBS_FIX11__ = true;
      var fired = false;
      function fire(){
        if(fired) return;
        fired = true;
      ...
      if(root.children && root.children.length > 0) fire();
      var obs = new MutationObserver(function(){
        if(root.children && root.children.length > 0) fire();
      });
      obs.observe(root, { childList:true, subtree:true });
      // hard fallback (do not block nav loader forever)
...
var q = getQuery();
    ...
    if (!q.metier){
      // ✅ STRICT: no job selected => keep page blank (waiting for selection)
      // Do not show "not available" messaging here.
      try{
        var root0 = ensureRoot();
        root0.innerHTML = "";
      }catch(e){}
      try{ window.hideMetierLoader && window.hideMetierLoader(); }catch(e){}
      // Also close navigation loader if any
      try{ window.dispatchEvent(new CustomEvent("ulydia:metier:rendered")); }catch(e){}
      return;
    }
    ...
    async function go(){
    var __navLoading = false;
    try{ __navLoading = (sessionStorage.getItem("ulydia_nav_loading") === "1"); }catch(e){}
    // If navigation loader is already displayed by the search bar, don't show MONO loader (avoid double loaders)
    if(!__navLoading){
...
/* ===== ULYDIA MONO FIX12 FOOTER ===== */
(function(){
  try{ sessionStorage.removeItem("ulydia_nav_loading"); }catch(e){}
  // If MONO doesn't render for any reason, still free the nav loader.
  setTimeout(function(){
    try{ window.dispatchEvent(new CustomEvent("ulydia:metier:rendered")); }catch(e){}
  }, 6000);
})();

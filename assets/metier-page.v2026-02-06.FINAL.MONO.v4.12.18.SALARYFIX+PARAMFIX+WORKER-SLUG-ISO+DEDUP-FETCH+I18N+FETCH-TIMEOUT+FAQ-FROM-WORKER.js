...
// ULYDIA BUILD MARKER
window.__ULYDIA_BUILD__ = 'v4.12.18 SALARYFIX+PARAMFIX+WORKER-SLUG-ISO';
...
var maxAll = 0;
        ...
        // Prefer rich salary bloc if provided (country-specific)
        if (!hasNums && ctx.mpb && ctx.mpb.salaire_bloc){
          try{ html += mpbToVisual(ctx.mpb.salaire_bloc, ui(lang2,'salary')); }
          catch(e){ html += '<div class="u-muted" style="padding:6px 0;">' + esc(htmlToText(String(ctx.mpb.salaire_bloc))) + '</div>'; }
        }
        if (ctx.mpb && ctx.mpb.salary_notes){
          html += '<div class="u-salary-sub">' + esc(htmlToText(String(ctx.mpb.salary_notes))) + '</div>';
        }
        if (!html){
          // Last resort: show a generic message (keeps UX consistent)
          html = '<div class="u-muted" style="padding:6px 0;">Rémunération variable selon l\'expérience, la localisation et le type de structure.</div>';
        }

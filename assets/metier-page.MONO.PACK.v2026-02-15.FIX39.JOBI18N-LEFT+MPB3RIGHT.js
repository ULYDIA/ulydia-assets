...
var row = document.createElement('div');
    ...
    if (ctx.fiche){
      var f = ctx.fiche;
      // Prefer Supabase job_i18n fields
      var overviewHtml = f.description || f.overview || f.accroche || ctx.accroche || '';
      if (overviewHtml) left.appendChild(card(ui(langL,'overview'), '✨', cleanBlock(overviewHtml)));
      if (f.missions) left.appendChild(card(ui(langL,'missions'), '✅', cleanBlock(f.missions)));
      if (f.environnements) left.appendChild(card(ui(langL,'work_environment'), '🏢', cleanBlock(f.environnements)));
      if (f.profil_recherche) left.appendChild(card(ui(langL,'profile'), '🧩', cleanBlock(f.profil_recherche)));
      if (f.evolutions_possibles) left.appendChild(card(ui(langL,'career_path'), '📈', cleanBlock(f.evolutions_possibles)));
    }
    }

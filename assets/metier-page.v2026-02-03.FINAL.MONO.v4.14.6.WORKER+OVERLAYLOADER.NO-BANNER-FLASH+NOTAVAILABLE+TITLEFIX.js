...
(function(){
  // =========================================================
  // ULYDIA — OVERLAY LOADER (integrated) — 2026-02-03
  // - Shows full-screen overlay while page is hydrating (Worker + Sponsor)
  // - Hidden when ULYDIA:PAGE_READY is dispatched (or after fail-safe)
  // =========================================================
  (function(){
    if (window.__ULYDIA_OVERLAY_LOADER__) return;
    window.__ULYDIA_OVERLAY_LOADER__ = true;
    ...
    function boot(){
      ...
      window.addEventListener("ULYDIA:PAGE_READY", hide, { once:false });
      window.addEventListener("ULYDIA:SHOW_LOADER", add, { once:false });
      window.addEventListener("ULYDIA:HIDE_LOADER", hide, { once:false });
    }
...
function __maybeFinalize(ctx, hadFicheFromDom){
  ...
  // If fiche missing after worker+dom, show message and stop (no banners).
  if (ctx.__workerDone && !ctx.fiche && !hadFicheFromDom){
    ctx.__dead = true;
    try{
      var r = ensureRoot();
      r.innerHTML = '';
      r.appendChild(renderNotAvailable(ctx));
...
// Append columns (FIX: left was missing in v4.8)
  ...
  async function main(){
    ...
    async function go(){
      try{ window.dispatchEvent(new Event('ULYDIA:SHOW_LOADER')); }catch(e){}

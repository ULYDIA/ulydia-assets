...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-ind-item{display:grid;grid-template-columns:44px 1fr auto;align-items:center;gap:12px;padding:12px 12px;border:1px solid var(--border);border-radius:16px;background:#fff;box-shadow:0 6px 18px rgba(16,24,40,.05);}
      .u-ind-ico{width:44px;height:44px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:rgba(99,102,241,.10);border:1px solid rgba(99,102,241,.18);font-size:20px;flex:0 0 auto;}
      .u-ind-txt{min-width:0;display:flex;flex-direction:column;gap:4px;line-height:1.2;}
      .u-ind-lbl{font-size:12px;color:var(--muted);font-weight:800;}
      .u-ind-val{font-size:14px;font-weight:900;color:#0f172a;justify-self:end;display:inline-flex;align-items:center;padding:6px 10px;border-radius:999px;border:1px solid rgba(17,24,39,.10);background:rgba(17,24,39,.03);white-space:nowrap;}
      .u-ind-item[data-kind="stack"], .u-ind-item[data-kind="text"]{grid-template-columns:44px 1fr;align-items:start;}
      .u-ind-item[data-kind="stack"] .u-ind-val{grid-column:2;justify-self:start;display:block;padding:0;border:none;background:transparent;white-space:normal;line-height:1.45;color:rgba(17,24,39,.85);font-weight:800;}
      .u-ind-item[data-kind="text"] .u-ind-val{grid-column:2;justify-self:start;display:block;margin-top:6px;padding:10px 12px;border-radius:14px;border:1px solid rgba(99,102,241,.18);background:rgba(99,102,241,.08);white-space:normal;line-height:1.5;color:rgba(17,24,39,.85);font-weight:800;}
      @media (hover:hover){.u-ind-item:hover{transform:translateY(-1px);box-shadow:0 10px 24px rgba(16,24,40,.08);}}
...
if (Array.isArray(ctx.mpb.blocks) && ctx.mpb.blocks.length){
  ctx.mpb.blocks.forEach(function(b){
    ...
    if (ctx.mpb){
      ...
      function pickIndKind(label, value){
        var v = String(value||"").trim();
        var l = String(label||"").toLowerCase();
        // Naturellement "longs" → encart
        if (l.indexOf("demande")>-1 || l.indexOf("délai")>-1 || l.indexOf("delai")>-1 || l.indexOf("croissance")>-1) return "text";
        // Court → badge
        if (v.length <= 14) return "badge";
        // Moyen → stack
        if (v.length <= 80) return "stack";
        // Long → encart
        return "text";
      }
      ...
      if (indItems.length){
        var htmlInd = '<div class="u-ind-list">' + indItems.map(function(it){
          var kind = pickIndKind(it.lbl, it.val);
          return '<div class="u-ind-item" data-kind="'+esc(kind)+'"><div class="u-ind-ico">'+esc(it.ico)+'</div><div class="u-ind-txt"><div class="u-ind-lbl">'+esc(it.lbl)+'</div><div class="u-ind-val">'+esc(it.val)+'</div></div></div>';
        }).join('') + '</div>';

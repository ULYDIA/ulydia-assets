...
// DOM fallback (Webflow embed):
    ...
    function faqMatches(item, ctx){
      ...
      const metierSlug = String(payload?.metier?.slug || params.metier || params.slug || "").trim();
      const metierName = String(payload?.metier?.name || "").trim();
      const isoNow = String(params.country || params.iso || payload?.iso || iso || "").trim().toUpperCase();
      const langNow = String(payload?.lang || payload?.metier?.lang || params.lang || lang || "").trim().toLowerCase();

      // 1) Match the FAQ to the current metier
      if (!faqMatchesMetier(item, metierSlug, metierName)) return false;

      // 2) Optional constraints if present on the FAQ item
      const itemIso = String(item?.iso || item?.country || item?.country_code || item?.code_iso || item?.pays || "").trim().toUpperCase();
      if (itemIso && isoNow && itemIso !== isoNow) return false;

      const itemLang = String(item?.lang || item?.language || item?.langue || "").trim().toLowerCase();
      if (itemLang && langNow && itemLang !== langNow) return false;
    ...
    faqFiltered = baseList.filter(function(it){ return faqMatches(it, { params: params, payload: payload, iso: iso, lang: lang }); });

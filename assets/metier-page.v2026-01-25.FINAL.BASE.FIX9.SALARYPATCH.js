...
function showPaysCardsByHeader() {
  ...
  function applySalaryGrid(sal, currency, iso){
    ...
    // ---------------------------------------------------------
    // Minimal direct bindings (do not depend on DOM structure)
    // Supports both:
    //  - ids: js-sal-*-range (shell)
    //  - ids: js-sal-*-min / js-sal-*-max (Webflow custom)
    // ---------------------------------------------------------
    function symFor(cur){
    ...
    // range fields (if present)
    if (junior) setTextById("js-sal-junior-range", junior);
    if (mid)    setTextById("js-sal-mid-range", mid);
    if (senior) setTextById("js-sal-senior-range", senior);

    // min/max fields (if present)
    setTextById("js-sal-junior-min", fmtK(sal?.junior?.min));
    setTextById("js-sal-junior-max", fmtK(sal?.junior?.max));
    setTextById("js-sal-mid-min",    fmtK(sal?.mid?.min));
    setTextById("js-sal-mid-max",    fmtK(sal?.mid?.max));
    setTextById("js-sal-senior-min", fmtK(sal?.senior?.min));
    setTextById("js-sal-senior-max", fmtK(sal?.senior?.max));

    // variable share / generation (if present in data + DOM)
    const varShare = sal?.variable_share || sal?.variableShare || sal?.variable || sal?.part_variable || "";
    if (varShare) setTextById("js-sal-variable-share", varShare);

    const gen = sal?.generation || sal?.statut_generation || sal?.statutGeneration || "";
    if (gen) setTextById("js-statut-generation", gen);

    // ---------------------------------------------------------
    // Existing behavior (kept): update rows inside the card
    // ---------------------------------------------------------
    ...
    const any = [ok1, ok2, ok3].some(Boolean) || !!(junior || mid || senior);

...
(() => {
  ...
  function readSectorMapFromDOM() {
    ...
    const map = new Map();
    const slugNodes = Array.from(document.querySelectorAll(".js-sector-slug"));
...
// ---------------------------
  ...
  function readJobsFromDOM() {
    ...
    itemsSet.forEach(item => {
      ...
      const lang = normalizeLang(qText(item, ".js-fiche-lang")) || "";
      const country = upper(qText(item, ".js-fiche-country-code") || qText(item, ".js-fiche-code-iso") || "");

      // title: take the displayed name (already in the item's language)
      const title = qText(item, ".js-metier-name") || qText(item, ".js-metier-title") || qText(item, ".js-fiche-title") || qText(item, ".js-fiche-name") || "";

      const titles = {};
      if (lang && title) titles[lang] = title;

      // sector slug(s): primary field .js-metier-sector (reference slug or text)
      const sectorRaw = qText(item, ".js-metier-sector") || qText(item, ".js-fiche-sector") || "";

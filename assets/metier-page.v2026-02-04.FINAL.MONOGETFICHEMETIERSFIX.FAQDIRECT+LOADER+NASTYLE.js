...
// ================= ULYDIA LOADER (content-only, no banner flash) =================
(function(){
  if (window.__ULYDIA_LOADER_READY__) return;
  window.__ULYDIA_LOADER_READY__ = true;

  function ensureStyle(){
    if (document.getElementById('ulydia-loader-style')) return;
    var s = document.createElement('style');
    s.id = 'ulydia-loader-style';
    s.textContent = `
      @keyframes uly-spin { from { transform: rotate(0deg);} to { transform: rotate(360deg);} }
      .ul-loader-wrap{max-width:1100px;margin:36px auto;padding:28px;border-radius:16px;
        background:#fff;box-shadow:0 10px 30px rgba(17,24,39,.08);
        display:flex;align-items:center;justify-content:center;gap:12px;
        font-family:Montserrat,system-ui;color:#6b7280}
      .ul-loader-dot{width:18px;height:18px;border:3px solid #e5e7eb;border-top-color:#6366f1;border-radius:50%;
        animation:uly-spin 1s linear infinite}
      .ul-na-card{max-width:1100px;margin:24px auto;padding:28px;border-radius:16px;
        background:linear-gradient(180deg,#ffffff, #f9fafb);
        box-shadow:0 10px 30px rgba(17,24,39,.08);font-family:Montserrat,system-ui}
      .ul-na-badge{display:inline-flex;align-items:center;gap:8px;
        padding:6px 10px;border-radius:999px;background:#eef2ff;color:#3730a3;font-weight:600}
      .ul-na-title{margin:14px 0 6px;font-size:22px;font-weight:800;color:#0f172a}
      .ul-na-text{color:#475569}
    `;
  ...
  window.showMetierLoader = function(){
    ...
    if (!root) return;
    root.innerHTML = '<div class="ul-loader-wrap"><span class="ul-loader-dot"></span><span>Loading job description…</span></div>';
  };
  ...
  window.renderNotAvailableCard = function(country){
    ...
    root.innerHTML =
      '<div class="ul-na-card">' +
        ...
        '<div class="ul-na-title">This job description is not available in this country</div>' +
        '<div class="ul-na-text">We are still working on localized content for <b>'+ (c||'this country') +'</b>. ' +
        'You can change the country or explore other jobs using the filters above.</div>' +
      '</div>';
  };
})();
...
function var __MIN_LOADER_MS = 400;
    var __elapsed = Date.now() - __loaderStart;
    if (__elapsed < __MIN_LOADER_MS) { await new Promise(r=>setTimeout(r, __MIN_LOADER_MS-__elapsed)); }
    renderPage(ctx){
    injectCSS();
    var root = ensureRoot();
...
// Keep loader homogeneous: ensure it is visible at least MIN_LOADER_MS to avoid flicker
    ...
    if (!ctx.fiche) {
      // English, styled, consistent with the job page UI
      try{

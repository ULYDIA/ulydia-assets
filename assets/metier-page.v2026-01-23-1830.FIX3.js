...
const cta = String(b?.cta || o?.cta || "").trim();
  ...
  function renderUnavailableState(message, sub){
    ...
    // Prefer inside the main content container
    const host = root.querySelector(".main-content") || root.querySelector(".left-panel") || root;
    // IMPORTANT: ensure we don't show any previously-rendered job content when unavailable
    try { if (host && host !== root) host.innerHTML = ""; } catch(_) {}
    ...
    card.innerHTML = `
      ...
      <div style="color:var(--muted);font-size:14px;line-height:1.45">
        ${escapeHtml(sub || "You can search for another job or select a different country.")}
      </div>
    `;

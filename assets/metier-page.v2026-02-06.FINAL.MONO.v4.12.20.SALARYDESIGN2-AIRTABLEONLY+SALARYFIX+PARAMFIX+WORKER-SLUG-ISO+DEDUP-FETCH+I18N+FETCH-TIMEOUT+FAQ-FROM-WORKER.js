...
// ULYDIA BUILD MARKER
window.__ULYDIA_BUILD__ = 'v4.12.20 SALARYDESIGN2-AIRTABLEONLY+SALARYFIX+PARAMFIX+WORKER-SLUG-ISO';
  ...
  window.renderNotAvailableCard = function(country){
    ...
    root.innerHTML =
      '<div class="ul-na-card">' +
        ...
        '<div class="ul-na-title">This job description is not yet available in this country</div>' +
        '<div class="ul-na-text">We are still working on localized content for <b>'+ (c||'this country') +'</b>. You can change the country or explore other jobs using the filters above.</div>' +
      '</div>';
  };
...
(function ensureSalaryStyle(){
          try{
            ...
            st.textContent =
              '.u-salary-wrap{padding:2px 0 0;font-family:Montserrat,system-ui}' +
              '.u-salary-head{display:flex;align-items:center;justify-content:space-between;padding:12px 14px 10px;margin:-12px -12px 12px;border-radius:14px 14px 10px 10px;' +
                'background:linear-gradient(135deg, rgba(34,197,94,1), rgba(16,185,129,1));color:#fff}' +
              '.u-salary-pill .t{font-weight:900;font-size:16px;display:flex;align-items:center;gap:10px}' +
              '.u-salary-pill .t .ic{filter:drop-shadow(0 1px 0 rgba(0,0,0,.12))}' +
              '.u-salary-item{padding:10px 0;border-bottom:1px solid rgba(148,163,184,.35)}' +
              '.u-salary-item:last-child{border-bottom:none}' +
              '.u-salary-top{display:flex;align-items:center;justify-content:space-between;gap:10px}' +
              '.u-salary-left{display:flex;align-items:center;gap:10px;font-weight:900;color:#0f172a}' +
              '.u-salary-left .emoji{width:20px;display:inline-flex;justify-content:center}' +
              '.u-salary-range{font-weight:900;color:#0f172a;opacity:.9}' +
              '.u-salary-bar{height:14px;border-radius:999px;background:rgba(148,163,184,.30);overflow:hidden;margin:10px 0 8px}' +
              ...
              '.u-salary-subtxt{color:#64748b;font-weight:700;font-size:12px}' +
              '.u-salary-note{margin-top:10px;color:#64748b;font-size:12px;font-weight:700;line-height:1.35}' +
              '.u-salary-muted{color:#64748b;font-weight:700;font-size:12px}' +
              '.u-salary-sub{margin-top:8px;color:#64748b;font-weight:700;font-size:12px;line-height:1.35}';
            document.head.appendChild(st);
          }catch(e){}
        })();
        ...
        function fmtRange(min,max,cur){
          ...
          var unit = (c === 'EUR' || c === '€') ? 'K€' : (c === 'USD' || c === '$' ? 'K$' : ('K ' + c));
        ...
        function parseVariable(v){
          ...
          if (typeof v === 'number') return {min:v, max:v, raw:String(v)};
          var s = String(v).replace(/\s+/g,' ').trim();
          if (!s) return null;
          var m = s.match(/(\d+(?:[.,]\d+)?)\s*(?:[-–]\s*(\d+(?:[.,]\d+)?))?/);
          if (!m) return {raw:s};
          var a = parseFloat(m[1].replace(',','.'));
          var b = m[2] ? parseFloat(m[2].replace(',','.')) : a;
          if (!isFinite(a)) return {raw:s};
          return {min:a, max:(isFinite(b)?b:a), raw:s};
        }
        ...
        var body = '<div class="u-salary-wrap">' +
          '<div class="u-salary-pill"><div class="t"><span class="ic">💰</span><span>'+esc(salaryTitle)+'</span></div></div>' +
          '<div class="u-salary-note" style="margin-top:0;margin-bottom:10px;">' +
            esc((String(lang2)==='fr') ? 'Les agents perçoivent souvent une part variable liée à la performance commerciale.' : 'Many roles include a variable component linked to sales performance.') +
          '</div>';

        var anyRow = false;
        if (hasNums){
          var r1 = itemRow(ui(lang2,'junior'), '🧳', jmin, jmax, expJ); if (r1){ body += r1; anyRow=true; }
          var r2 = itemRow(ui(lang2,'mid'),    '🚀', mmin, mmax, expM); if (r2){ body += r2; anyRow=true; }
          var r3 = itemRow(ui(lang2,'senior'), '⭐', smin, smax, expS); if (r3){ body += r3; anyRow=true; }
        }
        ...
        if (!anyRow && ctx.mpb && ctx.mpb.salaire_bloc){
          body += '<div class="u-salary-item"><div class="u-salary-muted">' + esc(htmlToText(String(ctx.mpb.salaire_bloc))) + '</div></div>';
          anyRow = true;
        }
        if (ctx.mpb && ctx.mpb.salary_notes){
          body += '<div class="u-salary-sub">' + esc(htmlToText(String(ctx.mpb.salary_notes))) + '</div>';
        }
        if (!anyRow){
          body += '<div class="u-salary-item"><div class="u-salary-muted" style="padding:2px 0;">' +

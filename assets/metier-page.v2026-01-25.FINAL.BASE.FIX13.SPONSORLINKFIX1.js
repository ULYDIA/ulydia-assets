...
// If the template doesn't contain placeholders, inject them (so banners always work)
    ...
    function looksBadHref(x){
      try{
        if (typeof x === "function") return true;
        const s = String(x||"").trim();
        if (!s) return true;
        if (s.indexOf("function") === 0) return true;
        if (s.indexOf("[native code]") >= 0) return true;
        if (/^javascript:/i.test(s)) return true;
        return false;
      }catch(e){ return true; }
    }
    function normalizeHref(x){
      if (looksBadHref(x)) return "";
      let s = String(x||"").trim();
      // allow mailto/tel
      ...
      // relative path => keep as-is (internal)
      if (s[0] === "/") return s;
      // bare domain
...
function setLinks(linkUrl){
      ...
      // if invalid, do nothing (caller can pass fallbackLink)
      if (!href) return;
      const isExternal = /^https?:\/\//i.test(href) || /^\/\//.test(href);
      if (wideA) { wideA.href = href; wideA.target = isExternal ? "_blank" : "_self"; wideA.rel = isExternal ? "noopener" : (wideA.rel||""); }
      if (logoA) { logoA.href = href; logoA.target = isExternal ? "_blank" : "_self"; logoA.rel = isExternal ? "noopener" : (logoA.rel||""); }
      if (ctaBtn) { ctaBtn.href = href; ctaBtn.target = isExternal ? "_blank" : "_self"; ctaBtn.rel = isExternal ? "noopener" : (ctaBtn.rel||""); }
    }
...
const fb = await resolveCountryBanners(iso, payload);
    ...
    const wfLink = normalizeHref(wfLinkRaw?.url || wfLinkRaw?.href || wfLinkRaw?.link || wfLinkRaw || "");
    ...
    const link2 = wfLink || normalizeHref(sObj?.link || sObj?.url || sObj?.href || "");

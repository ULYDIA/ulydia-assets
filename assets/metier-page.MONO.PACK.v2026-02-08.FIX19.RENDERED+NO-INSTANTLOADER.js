...
try{
  ...
  window.renderNotAvailableCard = function(country){
    ...
    root.innerHTML = ''
      + '<div class="ul-na">'
      +   '<div class="ul-na-tag">✨ Not available</div>'
    ...
    // signal "rendered" so the nav-loader can close even when we show NA card
    try{ ulydiaNotifyRendered(); }catch(e){}
  };
...
try{ window.__ULYDIA_normLang__ = normLang; }catch(e){}
  ...
  function ulydiaNotifyRendered(){
    // Notify external scripts (searchbar) that render finished, and kill any nav loader overlay
    try{ try{ sessionStorage.removeItem("ulydia_nav_loading"); }catch(e){} }catch(e){}
    try{
      var nl = document.getElementById("ulydia-nav-loader");

...
function renderPage(ctx){
    ...
    // FICHE + MPB — PRO layout (clear 2-column logic)
// LEFT  = understand the job (overview, missions, context, access, market)
// RIGHT = decide & position yourself (profile, path, indicators, skills/tools, salary)
...
function cleanBlock(html){
  ...
  // If it's a long comma-separated list, turn into bullets for readability
  html = listifyIfNeeded(html);
  return html;
}
...
if (ctx.fiche){
  ...
  if (f.description) left.appendChild(card(ui(langL,'overview'), '', cleanBlock(f.description)));

  if (f.missions) left.appendChild(card(ui(langL,'missions'), '', cleanBlock(f.missions)));

  // Work environment: prefer MPB variant if available (country-specific)
  ...
  // Skills (fiche) only if MPB skills cards are not present (avoid duplication)
  var mpbHasSkills = !!(ctx.mpb && hasAny(ctx.mpb.key_fields, ctx.mpb.skills_must_have, ctx.mpb.soft_skills, ctx.mpb.tools_stack));
  if (!mpbHasSkills && f.competences){
    left.appendChild(card(ui(langL,'skills'), '', cleanBlock(f.competences)));
  }
}

// MPB blocks (country-specific): keep them in LEFT for “understand”
if (ctx.mpb){
  ...
  if (ctx.mpb.marche) left.appendChild(card(ui(langL,'market'), null, mpbToVisual(ctx.mpb.marche, ui(langL,'market'))));

  // Generic MPB blocks (one item per block) -> LEFT
...
// --- RIGHT column (decision / positioning) ---
if (ctx.fiche){
  var f2 = ctx.fiche;

  if (f2.profil_recherche) right.appendChild(card(ui(langL,'profile'), '', cleanBlock(f2.profil_recherche)));

  if (f2.evolutions_possibles) right.appendChild(card(ui(langL,'career_path'), '', cleanBlock(f2.evolutions_possibles)));

  // If MPB skills exist, keep fiche competences out of the way; otherwise allow it on RIGHT (shorter decision view)
  var mpbHasSkills2 = !!(ctx.mpb && hasAny(ctx.mpb.key_fields, ctx.mpb.skills_must_have, ctx.mpb.soft_skills, ctx.mpb.tools_stack));
  if (mpbHasSkills2 && f2.competences){
    // show as compact bullets (not full duplicate)
    right.appendChild(card(ui(langL,'skills'), 'pastel-purple', listifyIfNeeded(cleanRichHTML(String(f2.competences)))));
  }
}

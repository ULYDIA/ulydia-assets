...
function renderPage(ctx){
    ...
    // MPB blocks
    if (ctx.mpb){
      if (ctx.mpb.formation) left.appendChild(card(ui(langFromCtx(ctx),'training'), null, mpbToVisual(ctx.mpb.formation, ui(langFromCtx(ctx),'training'))));
      if (ctx.mpb.acces) left.appendChild(card(ui(langFromCtx(ctx),'access'), null, mpbToVisual(ctx.mpb.acces, ui(langFromCtx(ctx),'access'))));
      if (ctx.mpb.marche) left.appendChild(card(ui(langFromCtx(ctx),'market'), null, mpbToVisual(ctx.mpb.marche, ui(langFromCtx(ctx),'market'))));
  ...
  ctx.mpb.blocks.forEach(function(b){
    ...
    if (ctx.mpb){
      ...
      function block(label, arr){
        ...
        // If tokens were collapsed into a single long sentence, render as paragraph (more readable than chips)
        ...
        if (singleLong || fragmented){
          inner = '<div class="kpi-text">'+esc(arr.join(' ').replace(/\s+/g,' ').trim())+'</div>';
        } else {
          inner = '<div class="kpi-chips">'+arr.map(function(t){
            return '<span class="chip badge-primary kpi-chip">'+esc(t)+'</span>';
          }).join('')+'</div>';
        }

        return '<div class="kpi-box" style="margin-bottom:12px;">'
          + '<div class="kpi-icon" style="background:rgba(99,102,241,0.10);color:#6366f1;font-weight:900;">✓</div>'
          + '<div style="flex:1;">'

...
;(function(){
  ...
  function findMetierDom(slug){
    ...
    for (var i=0;i<nodes.length;i++){
      ...
      if (!item) continue;
      var val = text(item.querySelector(".js-metier-slug"));
      if (slugify(val) !== s) continue;

      // Prefer the item that actually contains Fiche Metier fields (some pages may have multiple metier lists)
      var hasFiche = !!(item.querySelector(".js-fiche-description") || item.querySelector(".js-fiche-missions") || item.querySelector(".js-fiche-accroche"));
      if (hasFiche) return item;

      // else keep as fallback and continue searching
  ...
  function findCountryDom(iso){
    ...
    for (var i=0;i<nodes.length;i++){
      var item = nodes[i].closest(".w-dyn-item") || nodes[i].closest("[data-wf-collection-item]") || nodes[i].parentElement;
      if (!item) continue;
      var t = text(item.querySelector(".js-country-iso")).toUpperCase();
      if (t !== iso) continue;

      // Prefer item that has banner images
      var hasBanners = !!(item.querySelector(".js-country-banner-wide") || item.querySelector(".js-country-banner-square"));
      if (hasBanners) return item;

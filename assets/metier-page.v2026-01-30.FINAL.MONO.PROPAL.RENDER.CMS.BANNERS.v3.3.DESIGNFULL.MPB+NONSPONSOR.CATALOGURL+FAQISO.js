...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-sq{width:100%;aspect-ratio:1/1;min-height:240px;border-radius:12px;border:1px solid #eef0f4;background:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;}
      .u-sq img{width:100%;height:100%;object-fit:cover;display:block;}
      .u-prem-row{display:flex;flex-direction:column;align-items:stretch;gap:12px;}
      .u-prem-name{font-weight:900;color:#111827;font-size:13px;}
      .u-prem-sub{color:#6b7280;font-weight:700;font-size:12px;}
      .u-btn{display:flex;align-items:center;justify-content:center;padding:10px 12px;border-radius:10px;border:1px solid #dbe2ff;background:#f3f5ff;color:#4f46e5;font-weight:900;font-size:12px;text-decoration:none;width:100%;}
...
`;
  ...
  function renderBeforeFaqBanner(ctx){
    // Same asset logic as top banner, but intended for the slot before FAQ
    var sponsor = ctx.sponsor;
    var country = ctx.country;
    ...
    var wide = pickUrl(
      ...
      country && (country.banniere_sponsorisation_image_1)
    );
    if (!wide) return null;
    ...
    var a = document.createElement('a');
    a.className = 'u-banner u-banner-beforefaq';
    a.href = href;
    a.target = '_blank';
    a.rel = 'noopener';
    ...
    // Force stable sizing across browsers
    a.style.background = 'url('+JSON.stringify(wide)+') center / cover no-repeat';
    a.style.borderRadius = '14px';
    a.style.height = '120px';
    a.style.maxWidth = '100%';
    a.style.overflow = 'hidden';

    // Transparent overlay to keep clickability consistent
    var overlay = document.createElement('span');
    overlay.style.display = 'block';
    overlay.style.width = '100%';
    overlay.style.height = '100%';
    overlay.style.background = 'linear-gradient(90deg,rgba(79,70,229,.35),rgba(139,92,246,.25))';

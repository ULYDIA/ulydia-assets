...
if (Array.isArray(ctx.mpb.blocks) && ctx.mpb.blocks.length){
  ...
  async function main(){
    ...
    if (q.country && !country && !catCountry){
      ...
      // Build examples from the 249 Countries list (dynamic)
      var ex = [];
      try{
        var cc = getCountries();
        if (Array.isArray(cc)){
          for (var i=0;i<cc.length && ex.length<8;i++){
            var iso2 = String(cc[i] && (cc[i].iso || cc[i].code_iso || cc[i].codeIso || cc[i].country_code || cc[i].countryCode || cc[i].code || "") || "").toUpperCase();
            if (/^[A-Z]{2}$/.test(iso2) && ex.indexOf(iso2)===-1) ex.push(iso2);
          }
        }
      }catch(e){}
      if (!ex.length) ex = ["FR","US","DE","ES","IT","GB","JP","BR"];

      var hint = "";
      if (String(q.country||"").toUpperCase() === "EN"){
        hint = '<div style="margin-top:10px" class="u-muted">Note: <b>EN</b> is a language code. For United Kingdom, use <b>GB</b>.</div>';
      }

      var wrapBad = document.createElement('div');
      wrapBad.className = 'u-wrap';
      var bodyBad = '<div class="u-muted" style="line-height:1.6">' +
        'Invalid country code "<b>' + esc(String(q.country||'')) + '</b>". Use an <b>ISO2 country</b> from your Countries list (249). ' +
        'Examples: <b>' + esc(ex.join(", ")) + '</b>.' +
        hint +
        '</div>';

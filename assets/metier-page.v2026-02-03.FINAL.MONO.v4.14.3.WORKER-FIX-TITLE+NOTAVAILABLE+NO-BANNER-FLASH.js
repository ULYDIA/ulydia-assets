...
function getMetierTitle(ctx){
  ...
  var t = (f && (f.title || f.job_title || f.jobTitle || f.nom || f.name)) ? (f.title || f.job_title || f.jobTitle || f.nom || f.name) : "";
  t = String(t||"").trim();
  if (t) return t;
  // Fallback: prettify slug
  var slug = String((ctx && (ctx.slug || ctx.metier || ctx.metier_slug)) || "").trim();
  if (!slug) return "";

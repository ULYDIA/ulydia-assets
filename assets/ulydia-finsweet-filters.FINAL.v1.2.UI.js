...
// v1.2 — UI polish + metier visibility rebuild — fixes:
...
(() => {
  ...
  async function rebuildMetierSelect() {
    ...
    var c = document.getElementById("uf-count");
    if (c) c.textContent = String(metiers.length);
  }
  ...
  async function boot() {
    ...
    var btnReset = document.getElementById("uf-reset");
    if (btnReset){
      btnReset.addEventListener("click", async function(){
        // keep current country, reset sector + metier
        if (selSector) selSector.value = "";
        if (selMetier) selMetier.value = "";
        clickTrigger("sector", "");
        await rebuildMetierSelect();
      });
    }

...
(function(){
  // --- Robust worker base (prevents "/undefined/v1/..." on current origin)
  var WORKER = (window.ULYDIA_WORKER_URL || window.WORKER_URL || window.WORKER || window.ULYDIA_WORKER || window.__ULYDIA_WORKER_URL__ || "").trim();

if(!WORKER){
  WORKER = "https://ulydia-data.contact-871.workers.dev"; // last-resort default
}
WORKER = WORKER.replace(/\/+$/,"");
// Re-assert globally (some embeds/scripts rely on these)
try{
  window.ULYDIA_WORKER_URL = WORKER;
  window.WORKER_URL = WORKER;
  window.WORKER = WORKER;
  window.ULYDIA_WORKER = WORKER;
  window.__ULYDIA_WORKER_URL__ = WORKER;
}catch(e){}

  if (WORKER) WORKER = WORKER.replace(/\/+$/,"");
  if (!WORKER){
    console.error("[ULYDIA] Worker base URL is missing. Define window.ULYDIA_WORKER_URL (and/or WORKER_URL) before loading this script.");
  }
...
function themeForLeftTitle(title){
  ...
  function pickSectorFromObject(obj){
    ...
    var keys = [
      "secteur_name","secteurName","sector_name","sectorName","secteur_label","secteurLabel","sector_label","sectorLabel",
...
function findMPB(slug, iso){
  ...
  function getIsoKey(b){
    // Supabase MPB uses iso2
    return (b && (b.iso2 || b.iso_2 || b.iso || b.code_iso || b.codeIso || b.country || b.country_code || b.countryCode || b.code)) || "";
  }
  function getMetierKey(b){
    // Supabase MPB uses job_slug
    return String((b && (b.job_slug || b.jobSlug || b.job || b.slug || b.metier_slug || b.metierSlug || b.metier || b.metier_name || b.metierName || b.name)) || "").trim();
  }
  ...
  function normalizeMPB(b){
    ...
    out.metier = pick(b.job_slug, b.jobSlug, b.slug, b.metier_slug, b.metierSlug, b.metier, b.metier_name, b.name, out.metier);
    out.iso    = String(pick(b.iso2, b.iso_2, b.iso, b.code_iso, b.country, b.country_code, out.iso) || "").toUpperCase();
...
// ---------- Worker (sponsor) ----------
  function getWorkerUrl(){
    ...
    try{ window.WORKER = u; window.ULYDIA_WORKER = u; }catch(e){}
    return u;
  }
...
`;
  ...
  // FIX45: backward-compatible helper used by older renderers
  function cardSection(title, headerTheme, html){
    return card(title, headerTheme, html);
  }
...
var row = document.createElement('div');
    ...
    // -------------------------
    // LEFT column — Job content (job_i18n)
    // -------------------------
    var ji = ctx.job_i18n || ctx.fiche || null;
    var langL = langFromCtx(ctx);
    function t(key, fallback){ return ui(langL, key) || fallback || key; }

    if (ji){
      // Overview: prefer rich description, then accroche
      var overviewHtml = ji.description || ji.overview || ji.accroche || "";
      if (overviewHtml) leftCol.appendChild(cardSection(t('job_overview','Aperçu'), "✨", cleanPara(overviewHtml), "overview"));

      if (ji.missions)          leftCol.appendChild(cardSection(t('job_missions','Missions'), "✅", cleanBlock(ji.missions), "missions"));
      if (ji.environnements)    leftCol.appendChild(cardSection(t('job_environments','Environnements de travail'), "🏢", cleanBlock(ji.environnements), "env"));
      if (ji.profil_recherche)  leftCol.appendChild(cardSection(t('job_profile','Profil recherché'), "🎯", cleanPara(ji.profil_recherche), "profile"));
      if (ji.evolutions_possibles) leftCol.appendChild(cardSection(t('job_evolution','Évolutions possibles'), "📈", cleanPara(ji.evolutions_possibles), "evolution"));
    }

    // -------------------------
    // RIGHT column — Sponsor + MPB (Formation / Accès / Marché)
    // -------------------------
    ...
    // 0b) MPB 3 cards on the RIGHT
    var langR = langFromCtx(ctx);
    function tr(key, fallback){ return ui(langR, key) || fallback || key; }
    if (ctx.mpb){
      var mpb3 = ctx.mpb;
      if (mpb3.formation) right.appendChild(cardSection(tr('mpb_training','Formation'), "🎓", cleanPara(mpb3.formation), "mpb-training"));
      if (mpb3.acces)     right.appendChild(cardSection(tr('mpb_access','Accès'),     "📘", cleanPara(mpb3.acces),     "mpb-access"));
      if (mpb3.marche)    right.appendChild(cardSection(tr('mpb_market','Marché'),    "📊", cleanPara(mpb3.marche),    "mpb-market"));
    }
    ...
    if (__workerPayload){
      try{
        window.__ULYDIA_PAGE_DATA__ = {
          metier: (__workerPayload.metier || __workerPayload.fiche || __workerPayload.fiche_metier || (__workerPayload.fiche_metiers && __workerPayload.fiche_metiers[0]) || __workerPayload.job || __workerPayload.metier_data || __workerPayload.data || null),
          mpb: (
          __workerPayload.mpb ||
...
try{
  var __wp = (window.__ULYDIA_PAGE_DATA__ && (window.__ULYDIA_PAGE_DATA__.metier || window.__ULYDIA_PAGE_DATA__.fiche || window.__ULYDIA_PAGE_DATA__.fiche_metier || (window.__ULYDIA_PAGE_DATA__.fiche_metiers && window.__ULYDIA_PAGE_DATA__.fiche_metiers[0]) || window.__ULYDIA_PAGE_DATA__.job)) ? (window.__ULYDIA_PAGE_DATA__.metier || window.__ULYDIA_PAGE_DATA__.fiche || window.__ULYDIA_PAGE_DATA__.fiche_metier || (window.__ULYDIA_PAGE_DATA__.fiche_metiers && window.__ULYDIA_PAGE_DATA__.fiche_metiers[0]) || window.__ULYDIA_PAGE_DATA__.job) : null;

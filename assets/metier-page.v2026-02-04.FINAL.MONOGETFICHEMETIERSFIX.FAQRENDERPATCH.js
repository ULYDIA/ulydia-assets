...
function findFAQsForMetier(metierSlug, ficheName, iso, lang){
    var list = getFAQs();
    ...
    var langL = String(lang||"").toLowerCase().trim();
    ...
    for (var i=0;i<list.length;i++){
      ...
      var recSlug = slugify(f.slug || f.record_slug || f.item_slug || f.slug_faq || "");
      ...
      var itIso = normalizeIso(f.iso || f.country || f.code_iso || f.codeISO || f.pays_code || "");
      if (itIso && isoU && itIso !== isoU) continue;

      // Prefer language-specific columns when present (Airtable schema: Question_FR/Response_FR, Question_EN/Response_EN)
      var qTxt = "";
      var aHtml = "";

      if (langL === "fr"){
        qTxt = pick(f.question, f.Question_FR, f["Question_FR"], f.Question, f.QUESTION, f.q, f.Q, f.intitule, f.Intitule, f.titre, f.title);
        aHtml = pick(f.reponse, f.Response_FR, f["Response_FR"], f.Reponse, f.REPONSE, f.answer, f.ANSWER, f.a, f.html);
      } else if (langL === "en"){
        qTxt = pick(f.question, f.Question_EN, f["Question_EN"], f.Question, f.QUESTION, f.q, f.Q, f.intitule, f.Intitule, f.titre, f.title);
        aHtml = pick(f.reponse, f.Response_EN, f["Response_EN"], f.Reponse, f.REPONSE, f.answer, f.ANSWER, f.a, f.html);
      } else {
        qTxt = pick(f.question, f.Question_FR, f["Question_FR"], f.Question_EN, f["Question_EN"], f.Question, f.QUESTION, f.q, f.Q, f.intitule, f.Intitule, f.titre, f.title);
        aHtml = pick(f.reponse, f.Response_FR, f["Response_FR"], f.Response_EN, f["Response_EN"], f.Reponse, f.REPONSE, f.answer, f.ANSWER, f.a, f.html);
      }

      qTxt = norm(qTxt);
      aHtml = String(aHtml||"").trim();

      if (!qTxt) continue;

      out.push({ question: qTxt, answer: aHtml });
    }
...
// Append columns (FIX: left was missing in v4.8)
    ...
    // FAQ bottom
    var faqs = findFAQsForMetier(ctx.slug, (ctx.fiche && (ctx.fiche.name||ctx.fiche.Nom)) || ctx.name || ctx.slug, ctx.iso, langFromCtx(ctx));

...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
/* =========================================================
   ULYDIA — DESIGN SYSTEM (INTEGRATED IN MONO)
   - keeps existing DOM classes (.u-*)
   - adds Ulydia-ready tokens + pastel headers + soft hover
========================================================= */
...
:root{
  ...
  --shadow-hover:0 18px 45px rgba(16,24,40,.10);
...
.u-accroche{color:var(--muted);font-size:16px;line-height:1.5;margin:0 0 24px 0;max-width:780px;}

/* =========================================================
   CARDS (applies to existing .u-card and optional .bloc)
========================================================= */
.u-card,.bloc{
  background:var(--card);
...
.u-card-b,.bloc-body{padding:18px;}

/* Pastel themes (keep your existing class names) */
.u-card-h.bg-purple,.bloc-header.primary{background:rgba(99,102,241,.22);color:var(--text);}
.u-card-h.bg-green,.bloc-header.success{background:rgba(34,197,94,.18);color:var(--text);}
.u-card-h.bg-cyan{background:rgba(56,189,248,.20);color:var(--text);}
.u-card-h.bg-yellow,.bloc-header.warning{background:rgba(245,158,11,.22);color:var(--text);}
.u-card-h.bg-red,.bloc-header.danger{background:rgba(239,68,68,.16);color:var(--text);}

/* If some headers were getting overridden, force it */
.u-card-h.bg-purple,.u-card-h.bg-green,.u-card-h.bg-cyan,.u-card-h.bg-yellow,.u-card-h.bg-red{
...
/* =========================================================
   TEXT + LISTS
========================================================= */
.u-card-b p,.bloc-body p{color:var(--text);line-height:1.65;margin:0 0 10px 0;}
.u-card-b p:last-child,.bloc-body p:last-child{margin-bottom:0;}
.u-card-b ul{margin:0;padding-left:1.1rem;}
.u-card-b li{margin:.35rem 0;line-height:1.55;color:var(--text);}

/* =========================================================
   TAGS (skills / domaines / tools)
========================================================= */
.u-tags{
  display:flex;
  flex-wrap:wrap;
...
/* =========================================================
   ROW ITEMS (soft skills / indicators / simple lists)
========================================================= */
.u-rowitem{
  display:flex;
  gap:12px;
...
/* =========================================================
   PREMIUM sponsor square (kept)
========================================================= */
.ul-premium-square{padding:12px;background:linear-gradient(180deg,#ffffff,#fbfbff);}
.u-sponsor-square-link img{width:100%;display:block;border-radius:14px;border:1px solid var(--border);}

/* =========================================================
   FAQ (keep neutral text like 'Marché')
========================================================= */
.u-faq-q{color:var(--text);font-weight:800;}
.u-faq-a{color:var(--text);opacity:.92;line-height:1.6;}
`;
  ...
  function themeForTitle(title){
    ...
    switch(key){
      ...
      case "salary": return "gradient-success";
...
function renderNotAvailable(ctx){
  ...
  function renderPremiumCard(ctx){
    // Square sponsor banner (image only) — minimal, no extra text/button.
    var sponsor = ctx.sponsor;
    if (sponsor === '__PENDING__') return null;
    var country = ctx.country;
    ...
    var c = document.createElement('div');
    c.className = 'u-card ul-premium-square';

    var a = document.createElement('a');
    a.href = href;
    a.className = 'u-sponsor-square-link';
    a.rel = 'noopener';
    a.style.display = 'block';
...
function renderPage(ctx){
    ...
    // MPB blocks
    if (ctx.mpb){
      // Move these 3 blocks to RIGHT column (per design spec)
      if (ctx.mpb.formation) right.appendChild(card(ui(langFromCtx(ctx),'training'), null, mpbToVisual(ctx.mpb.formation, ui(langFromCtx(ctx),'training'))));
      if (ctx.mpb.acces) right.appendChild(card(ui(langFromCtx(ctx),'access'), null, mpbToVisual(ctx.mpb.acces, ui(langFromCtx(ctx),'access'))));
      if (ctx.mpb.marche) right.appendChild(card(ui(langFromCtx(ctx),'market'), null, mpbToVisual(ctx.mpb.marche, ui(langFromCtx(ctx),'market'))));
      if (ctx.mpb.environnements) left.appendChild(card(ui(langFromCtx(ctx),'work_environment'), null, mpbToVisual(ctx.mpb.environnements, ui(langFromCtx(ctx),'work_environment'))));
...
if (Array.isArray(ctx.mpb.blocks) && ctx.mpb.blocks.length){
  ctx.mpb.blocks.forEach(function(b){
    ...
    if (ctx.mpb && (ctx.mpb.junior_min || ctx.mpb.junior_max || ctx.mpb.mid_min || ctx.mpb.mid_max || ctx.mpb.senior_min || ctx.mpb.senior_max || ctx.mpb.salary_notes || ctx.mpb.devise || ctx.mpb.variable_share)){
      ...
      // Salary grid values are monthly -> display annual (*12)
      var ANNUAL_FACTOR = 12;
      function annual(n){ n = Number(n); return (isFinite(n) ? n * ANNUAL_FACTOR : n); }
      ...
      var maxAll = Math.max(
        annual(ctx.mpb.junior_max||0),
        annual(ctx.mpb.mid_max||0),
        annual(ctx.mpb.senior_max||0)
      );
      ...
      var j = range(annual(ctx.mpb.junior_min), annual(ctx.mpb.junior_max));
      var m = range(annual(ctx.mpb.mid_min), annual(ctx.mpb.mid_max));
      var se = range(annual(ctx.mpb.senior_min), annual(ctx.mpb.senior_max));
      body.innerHTML = `
        <div style="display:flex;flex-direction:column;gap:14px;">
          ...
          <div>
            ...
            <div class="salary-bar-container"><div class="salary-bar-fill ul-gradient-success" style="width:${pct(annual(ctx.mpb.junior_max),40)}%;"></div></div>
          ...
          <div>
            ...
            <div class="salary-bar-container"><div class="salary-bar-fill ul-gradient-success" style="width:${pct(annual(ctx.mpb.mid_max),60)}%;"></div></div>
          ...
          <div>
            ...
            <div class="salary-bar-container"><div class="salary-bar-fill ul-gradient-success" style="width:${pct(annual(ctx.mpb.senior_max),80)}%;"></div></div>
    ...
    if (ctx.mpb){
      ...
      function splitTagsLoose(s){
        // presentation helper: accept arrays or strings, keep readable tokens
        if (Array.isArray(s)) {
          var arr = s.map(function(x){ return norm(x); }).filter(function(t){ return t && t !== '✓' && t !== '✔' && t !== '&nbsp'; });
          // If data looks "fragmented" (too many tiny tokens), keep as one sentence
          var tiny = arr.filter(function(t){ return t.length <= 3; }).length;
          if (arr.length && (tiny / arr.length) >= 0.55){
            return [norm(arr.join(" "))];
          }
          return arr;
        }
        s = String(s||'');
        ...
        // If we still look fragmented, collapse to one readable line
        var tiny2 = out.filter(function(t){ return t.length <= 3; }).length;
        if (out.length && (tiny2 / out.length) >= 0.55){
...
function skillCard(title, theme, icon, arr, mode){
        if (!arr || !arr.length) return null;
        // DESIGN: match "image 1" (simple content inside card, no inner kpi-box)
        var content = (mode === 'soft_lines') ? blockContentSoft(arr) : blockContent(arr);
        return card(title, theme, content);
      }

      // Split "Compétences & parcours" into 4 independent blocks (more presentable)
// Domaines clés must be FIRST in right column
var _keyFieldsCard = skillCard(ui(lang2,'key_fields'), 'pastel-purple', '✓', topFields);
if (_keyFieldsCard) right.insertBefore(_keyFieldsCard, right.firstChild);

var _mustHaveCard = skillCard(ui(lang2,'must_have'), 'bg-purple', '⚡', mustHave);
if (_mustHaveCard) right.appendChild(_mustHaveCard);

var _softCard = skillCard(ui(lang2,'soft_skills'), 'pastel-pink', '🧩', soft, 'soft_lines');
if (_softCard) right.appendChild(_softCard);

var _toolsCard = skillCard(ui(lang2,'tools_stack'), 'pastel-cyan', '🛠️', tools);

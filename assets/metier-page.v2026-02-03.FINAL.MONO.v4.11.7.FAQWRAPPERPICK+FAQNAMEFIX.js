...
function findFAQsForMetier(metierSlug, ficheName, iso){
    ...
    for (var i=0;i<list.length;i++){
      ...
      out.push({
        question: norm(f.question || f.QUESTION || f.q || f.Q || f.intitule || f.Intitule),
        answer: String(f.answer || f.ANSWER || f.reponse || f.Reponse || f.reponse_html || f.answer_html || f.a || f.html || f.HTML || "").trim()
      });
    }

...
(function(){
  ...
  function norm(s){ return String(s||"").replace(/&nbsp;/gi," ").replace(/\u00a0/g," ").replace(/\s+/g," ").trim(); }
  ...
  // STRICT mapping:
  // Sponsor:
  //  - wide  = sponsor_logo_2 (or banner_1 / image_1)
  //  - square= sponsor_logo_1 (or banner_2 / image_2)
  // Fallback country:
  //  - wide  = banner_1 (680x120)
  //  - square= banner_2 (300x300)
  var s=getSponsorInfo();
  if (s){
    var sb = (s.banners || s.banner || s) || {};
    var wide  = pickUrl(sb.sponsor_logo_2 || sb.logo_2 || sb.banner_1 || sb.image_1 || sb.image1 || sb.wide || sb.landscape);
    var square= pickUrl(sb.sponsor_logo_1 || sb.logo_1 || sb.banner_2 || sb.image_2 || sb.image2 || sb.square);
    var href  = pickUrl(sb.lien_sponsor || sb.url || sb.href || sb.link || sb.cta_url || sb.ctaUrl);
    var name  = norm(sb.name || sb.title || "");
    if (wide || square) return { kind:"sponsor", wide:wide, square:square, href:href, name:name };
  }
  var c=getCountryByIso(iso)||{};
  var b=(c.banners||c.banner||c.fallback_banners||c.fallback||c||{});
...
function renderNotFound(root, ctx, extra){
    root.innerHTML = "";
    ...
    var details = "<div style='color:var(--text);font-weight:900'>Impossible de charger la fiche métier (MPB introuvable).</div>" +
      "<div style='color:var(--muted);margin-top:6px;font-weight:800'>URL : metier="+escapeHtml(ctx.metierSlug)+" • country="+escapeHtml(ctx.iso)+"</div>";

    if (extra){
      if (typeof extra.mpbCount === "number"){
        details += "<div style='margin-top:10px;color:var(--text);font-weight:800'>MPB chargés : "+escapeHtml(extra.mpbCount)+"</div>";
      }
      if (extra.sample){
        details += "<div style='margin-top:8px;color:var(--muted);font-weight:800;font-size:13px'>Exemples de clés détectées :</div>";
        details += "<div style='margin-top:6px;padding:10px;border:1px solid rgba(226,232,240,1);border-radius:12px;background:#fff;color:var(--text);font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace;font-size:12px;line-height:1.5;white-space:pre-wrap'>"+escapeHtml(extra.sample)+"</div>";
      }
    }

    details += "<div style='margin-top:12px;color:var(--muted);font-size:12px;line-height:1.5'>" +
      "Si tu vois ce message : (1) vérifie que les embeds CMS remplissent window.__ULYDIA_MPB__ avec des clés metier/iso (ou job_slug/country_code), " +
      "(2) vérifie que la valeur metier dans MPB correspond bien au paramètre ?metier= (slug), " +
      "(3) vérifie que l’ISO est bien sur 2 lettres (FR, DE...).</div>";

    box.appendChild(el("div",{class:"ul-card-b"}, details));
...
// find MPB (robust: accepts multiple key names)
var bloc=null;
var mpb=getMPB();
...
function extractMetierKey(b){
  // prefer explicit slug-like fields
  var v = pickFirst(b, ["metier","metier_slug","metierSlug","job_slug","jobSlug","job","slug","job_id","jobId","metier_lie","metierLie","metier_lie_slug","metierLieSlug"]);
  if (v && typeof v === "object") v = pickFirst(v, ["slug","value","name","id"]);
  return slug(v);
}
function extractMetierName(b){
  var v = pickFirst(b, ["metier_name","metierName","job_name","jobName","name","title"]);
  if (v && typeof v === "object") v = pickFirst(v, ["name","title","value"]);
  return norm(v);
}
function extractIsoKey(b){
  var v = pickFirst(b, ["iso","country_code","countryCode","country","pays","pays_iso","paysIso","country_iso","countryIso"]);
  if (v && typeof v === "object") v = pickFirst(v, ["iso","code","value","name"]);
  return iso2(v);
}

for (var k=0;k<mpb.length;k++){
  var b=mpb[k];
  var mKey = extractMetierKey(b);
  var iKey = extractIsoKey(b);
  if (mKey===metierSlug && iKey===iso){ bloc=b; break; }
  // fallback match by name if slug missing
  if (!bloc && !mKey && iKey===iso){
    var mName = extractMetierName(b);
    if (mName && slug(mName)===slug(metierName)) { bloc=b; break; }
  }
}
    ...
    if (!bloc){
      ...
      try{
        var mpb=getMPB();
        var s=mpb.slice(0,3).map(function(b){
          return JSON.stringify({metier:(b&& (b.metier||b.metier_slug||b.job_slug||b.slug||b.name||"")), iso:(b&&(b.iso||b.country_code||b.countryCode||b.country||""))});
        }).join('\n');
        sample=s;
      }catch(e){}
      renderNotFound(root, ctx, { mpbCount: getMPB().length, sample: sample });

...
// ✅ Ensure Worker base URL is always defined (prevents /undefined/v1/* calls)
(function(){
  var BASE = "https://ulydia-data.contact-871.workers.dev";
  try{
    if (!window.ULYDIA_WORKER_URL || typeof window.ULYDIA_WORKER_URL !== "string" || !window.ULYDIA_WORKER_URL.trim()){
      window.ULYDIA_WORKER_URL = BASE;
    }
  }catch(e){
    try{ window.ULYDIA_WORKER_URL = BASE; }catch(e2){}
  }
})();
(function(){
...
try{ window.__ULYDIA_normLang__ = normLang; }catch(e){}
  ...
  // --- FIX Option A: legacy endpoint /v1/jobs -> /v1/filters (Worker list API)
  // Some older builds (or external snippets) request "/v1/jobs?iso=XX".
  // The data worker exposes "/v1/filters?iso=XX" instead. We transparently rewrite GET requests.
  (function(){
    try{
      if (window.__ULYDIA_FETCH_REWRITE_JOBS_TO_FILTERS__) return;
      window.__ULYDIA_FETCH_REWRITE_JOBS_TO_FILTERS__ = true;
      ...
      window.fetch = function(input, init){
        try{
          ...
          // --- FIX: some legacy snippets build "undefined/v1/..." (or "/undefined/v1/...") against the current origin.
          // We rewrite those calls to the data worker base URL.
          try{
            var BASE = (window && window.ULYDIA_WORKER_URL) ? String(window.ULYDIA_WORKER_URL) : "https://ulydia-data.contact-871.workers.dev";
            if (BASE && BASE.indexOf("/v1") !== -1) BASE = BASE.replace(/\/v1\/?$/,"");
            function rewriteUndefined(u){
              if(!u) return u;
              u = String(u);
              var p = null;

              // Absolute URL containing /undefined/v1/...
              var k = u.indexOf("/undefined/v1/");
              if (k !== -1) {
                p = u.substring(u.indexOf("/v1/", k));
              } else if (u.indexOf("undefined/v1/") === 0) {
                p = u.substring("undefined".length); // -> /v1/...
              } else if (u.indexOf("/undefined/v1/") === 0) {
                p = u.substring("/undefined".length); // -> /v1/...
              }
            ...
            if (url) {
              var newU = rewriteUndefined(url);
              if (newU && newU !== url) {
                if (typeof input === "string") {
                  input = newU;
                } else if (isReq && req) {
                  input = new Request(newU, req);
                }
                url = newU;
              }
            }
          ...
          if (url && url.indexOf("/v1/jobs") !== -1) {
            // Only rewrite safe GET-like requests
            var method = (init && init.method) || (isReq && req && req.method) || "GET";
            method = String(method || "GET").toUpperCase();
            if (method === "GET" || method === "HEAD") {
              var newUrl = url.replace("/v1/jobs", "/v1/filters");
              if (typeof input === "string") {
                input = newUrl;
              } else if (isReq && req) {
...
function themeForLeftTitle(title){
  ...
  function tr(key, params){
    // Prefer an external i18n engine if present, but ALWAYS fall back to the inline I18N map
    // so we never render raw keys like "missing_job_title".
    var lang = (typeof finalLang !== "undefined" && finalLang) ? String(finalLang).toLowerCase()
             : (window.__ULYDIA_LANG_FINAL__ ? String(window.__ULYDIA_LANG_FINAL__).toLowerCase()
             : (document.documentElement && document.documentElement.lang ? String(document.documentElement.lang).toLowerCase() : "en"));
    lang = normLang(lang);
    ...
    // Inline fallback (defined earlier in this file)
    if (out == null){
      try{
        if (I18N && I18N[lang] && I18N[lang][key] != null) out = I18N[lang][key];
        else if (I18N && I18N.en && I18N.en[key] != null) out = I18N.en[key];
      }catch(e){}
    }
...
function __ulFormatCurrency(code){
  // Accept either a simple code ("EUR", "€") OR a JSON/object like:
  // {"icon":"💰","label":"Devise","value_short":"€","note":"..."}
  try{
    if (code && typeof code === "object"){
      var v = code.value_short || code.valueShort || code.code || code.iso || code.currency || code.currency_code || code.currencyCode || "";
      if (v) code = v;
  ...
  // JSON string -> extract value_short / code
  if (s[0] === "{" && s.indexOf("value_short") !== -1){
    try{
      var o = JSON.parse(s);
      var vs = (o && (o.value_short || o.valueShort)) ? String(o.value_short || o.valueShort).trim() : "";
      if (vs) s = vs;
      else if (o && (o.code || o.iso || o.currency || o.currency_code || o.currencyCode)) s = String(o.code || o.iso || o.currency || o.currency_code || o.currencyCode).trim();
    }catch(e){}
  }
  ...
  // If we only have a symbol, keep it (UI shows it best)
  if (s === "€" || s === "$" || s === "£" || s === "¥") return s;
...
// ---------- Worker (sponsor) ----------
  function getWorkerUrl(){
    ...
    u = u.trim();
    // If config wasn't set (or is read as undefined), fall back to the known data worker.
    // This prevents fetch('undefined/v1/...') -> https://www.ulydia.com/undefined/v1/...
...
// ---------- DOM render ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      #ulydia-metier-root .u-row{display:grid;grid-template-columns:1.65fr 1fr !important;gap:24px;align-items:start;}#ulydia-metier-root .u-right{max-width:520px;width:100%;}
...
function renderNotAvailable(ctx){
  ...
  // Update only banner slots (avoid full re-render that reloads everything)
  function updateBanners(ctx){
    try{
      var aTop = document.getElementById("ulydia-top-banner-anchor");
      if (aTop){
        aTop.innerHTML = "";
        var nTop = renderTopBanner(ctx);
        if (nTop) aTop.appendChild(nTop);
      }
      var aFaq = document.getElementById("ulydia-before-faq-anchor");
      if (aFaq){
        aFaq.innerHTML = "";
        var nFaq = renderBeforeFaqBanner(ctx);
        if (nFaq) aFaq.appendChild(nFaq);
      }
    }catch(e){}
  }
...
function renderPage(ctx){
    ...
    // Anchor: top banner (so we can update it without re-rendering everything)
    var __topAnchor = document.createElement('div');
    __topAnchor.id = 'ulydia-top-banner-anchor';
    root.appendChild(__topAnchor);

    // Sponsor banner (wide)
    var topB = renderTopBanner(ctx);
    if (topB){
      __topAnchor.appendChild(topB);
      var sp=document.createElement('div'); sp.style.height='10px'; __topAnchor.appendChild(sp);
    }
...
(function ensureSalaryStyle(){
          try{
            ...
            st.textContent =
              ...
              '.u-salary-left{display:flex;align-items:center;gap:10px;font-weight:700;color:#334155;font-size:15px;letter-spacing:-.01em}' +
              '.u-salary-left .emoji{width:22px;display:inline-flex;justify-content:center}' +
              '.u-salary-range{font-weight:700;color:#334155;opacity:.95;font-size:15px;letter-spacing:-.01em}' +
...
// Append columns (FIX: left was missing in v4.8)
    ...
    var __faqAnchor = document.createElement('div');
    __faqAnchor.id = 'ulydia-before-faq-anchor';
    root.appendChild(__faqAnchor);

    var beforeFaq = renderBeforeFaqBanner(ctx);

    // FAQ bottom
    var faqs = getFAQs();
    if (beforeFaq) __faqAnchor.appendChild(beforeFaq);
...
var q = getQuery();
    ...
    // ✅ Worker-first: always enrich/override the legacy Webflow fiche with Worker payload
    // This guarantees we display description/missions/etc even if Webflow exports are partial.
    try{
      var wFiche = (window.__ULYDIA_PAGE_DATA__ && window.__ULYDIA_PAGE_DATA__.metier) ? window.__ULYDIA_PAGE_DATA__.metier : null;
      if (wFiche){
        try{
          // Keep legacy keys if needed, but Worker wins for content fields
          fiche = Object.assign({}, (fiche||{}), wFiche);
        }catch(e){
          fiche = wFiche;
        }
      }
...
// ✅ Worker payload fallback (do NOT override with DOM-only pickFicheForLang)
// If DOM fiche is missing or lang-mismatch, but Worker returned a fiche, use it.
try{
  var __wp = (window.__ULYDIA_PAGE_DATA__ && window.__ULYDIA_PAGE_DATA__.metier) ? window.__ULYDIA_PAGE_DATA__.metier : null;
  if (!fiche && __wp && ficheHasContent(__wp) && (__wp.not_available !== true)) {
    fiche = __wp;
    if (!ficheAny) ficheAny = __wp;
    // mark mode for debugging
    pick.mode = (pick.mode ? (pick.mode + "+worker") : "worker");
  }
}catch(e){}
...
// Keep loader homogeneous: ensure it is visible at least MIN_LOADER_MS to avoid flicker
    ...
    // Start sponsor fetch early, but avoid full re-render (which reloads images & feels like double loading)
    window.__ULYDIA_SPONSOR_PENDING__ = true;
    var __sponsorPromise = fetchSponsor(ctx.slug, ctx.iso).then(function(s){ return s || null; });

    // Wait a short grace period for sponsor (fast path). If it doesn't arrive quickly, render once with non-sponsor.
    var __sponsorGraceMs = 450;
    var __sponsorFast = null;
    try{
      __sponsorFast = await Promise.race([
        __sponsorPromise,
        new Promise(function(resolve){ setTimeout(function(){ resolve("__TIMEOUT__"); }, __sponsorGraceMs); })
      ]);
    }catch(e){ __sponsorFast = "__TIMEOUT__"; }

    if (__sponsorFast !== "__TIMEOUT__") {
      ctx.sponsor = __sponsorFast;
    } else {
      ctx.sponsor = null;
    }
    window.__ULYDIA_SPONSOR_PENDING__ = false;

    try{ window.hideMetierLoader && window.hideMetierLoader(); }catch(e){}
    renderPage(ctx);

    // If sponsor arrives later AND exists, update only banners (no full re-render)
    if (__sponsorFast === "__TIMEOUT__") {
      __sponsorPromise.then(function(s){
        if (!s) return;
        ctx.sponsor = s;
        updateBanners(ctx);
      });
...
/* ULYDIA — Fiche blocks fallback (AFTER TITLE) — integrated */
(function(){
  if (window.__ULYDIA_FICHE_FALLBACK_AFTER_TITLE__) return;
  window.__ULYDIA_FICHE_FALLBACK_AFTER_TITLE__ = true;

  const esc = (s)=>String(s||"").replace(/[&<>"']/g, m=>({ "&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;" }[m]));
  const isFilled = (v)=> !!String(v||"").trim();
  function pickHtml(m){
    if(!m || typeof m!=="object") return null;
    const cands = [m.overview_html, m.overview, m.description, m.description_html, m.missions_html, m.missions, m.environment_html, m.environment, m.profile_html, m.profile, m.competences_html, m.competences, m.skills_html, m.skills];
    if(!cands.some(isFilled)) return null;
    return {
      overview: m.overview_html || m.overview || m.description_html || m.description || "",
      missions: m.missions_html || m.missions || "",
      environment: m.environment_html || m.environment || "",
      profile: m.profile_html || m.profile || "",
      competences: m.competences_html || m.competences || "",
      skills: m.skills_html || m.skills || ""
    };
  }

  function hasNativeBlocks(){
    try{
      const root = document.querySelector("#ulydia-metier-root") || document.body;
      const heads = Array.from(root.querySelectorAll("h2,h3,h4,.u-card__title,[data-ulydia-block-title]")).map(n=>String(n.textContent||"").trim().toLowerCase());
      const needles = ["aperçu","apercu","missions","environnement","profil","compétences","competences","skills","overview","tasks","environment","profile","competencies"];
      return heads.some(t => needles.some(n=>t===n || t.startsWith(n+" ")));
    }catch(e){ return false; }
  }

  function findTitleEl(){
    return document.querySelector('[data-ulydia-title="1"]')
      || document.querySelector("#ulydia-metier-root h1")
  ...
  function currentSlug(){
    try{
      ...
      return String(qs.get("metier") || qs.get("slug") || qs.get("job") || "").trim();
    }catch(e){ return ""; }
  }

  function removeFallback(){
    try{
      const w = document.getElementById("ulydia-fiche-fallback");
      if (w) w.remove();
    }catch(e){}
  }

  function killIfNative(){
    try{
      if (hasNativeBlocks()) removeFallback();
    }catch(e){}
  }

function ensureContainer(){
    // Never render if native blocks already exist
    if (hasNativeBlocks()) return null;

    let wrap = document.getElementById("ulydia-fiche-fallback");
    if (wrap) return wrap;
    ...
    wrap.id = "ulydia-fiche-fallback";
    wrap.setAttribute("data-ulydia-fiche-fallback","1");
    wrap.style.maxWidth = "1100px";
    wrap.style.display = "none";
    wrap.style.opacity = "0";
    wrap.style.transition = "opacity 120ms ease";
    wrap.style.margin = "16px auto 0 auto";
    wrap.style.padding = "0 16px";

    // Insert strictly AFTER title (not before)
    title.insertAdjacentElement("afterend", wrap);
    return wrap;
  }

  function card(title, html){
    if(!isFilled(html)) return "";
    return (
      '<div class="u-card u-card--soft" style="margin:12px 0;">' +
        '<div class="u-card__head" style="padding:14px 16px 0 16px;"><h3 class="u-card__title" style="margin:0;font-size:14px;font-weight:700;">'+esc(title)+'</h3></div>' +
        '<div class="u-card__body" style="padding:10px 16px 16px 16px;line-height:1.5;">' + html + '</div>' +
      '</div>'
    );
  }

  function renderIfNeeded(metier){
    const blocks = pickHtml(metier);
    if(!blocks) return;

    const wrap = ensureContainer();
    if(!wrap) return;

    // If already rendered (or native blocks appeared), stop.
    if (wrap.getAttribute("data-rendered")==="1" || hasNativeBlocks()) return;

    const html =
      card("Aperçu", blocks.overview) +
      card("Missions", blocks.missions) +
      card("Environnement", blocks.environment) +
      card("Profil recherché", blocks.profile) +
      card("Compétences", blocks.competences) +
      card("Compétences indispensables", blocks.skills);

    if(!isFilled(html)) return;

    wrap.innerHTML = html;
    wrap.setAttribute("data-rendered","1");
    wrap.setAttribute("data-slug", currentSlug() || "");
    // Avoid "flash": only show if native blocks are still absent after a short grace period
    wrap.style.display = "none";
    wrap.style.opacity = "0";
    setTimeout(function(){
      try{
        if (hasNativeBlocks()) { removeFallback(); return; }
        // show fallback
  ...
  function tryRender(){
    try{
      killIfNative();
      const m = (window.__ULYDIA_PAGE_DATA__ && (window.__ULYDIA_PAGE_DATA__.fiche || window.__ULYDIA_PAGE_DATA__.metier)) || null;
      if(!m) return;

      const slugNow = currentSlug();
      const wrap = document.getElementById("ulydia-fiche-fallback");
      if (wrap){
        const prev = wrap.getAttribute("data-slug") || "";
        if (slugNow && prev && slugNow !== prev){
          // navigating to another job: hide old fallback immediately
          wrap.style.display = "none";
          wrap.style.opacity = "0";
          wrap.innerHTML = "";
          wrap.removeAttribute("data-rendered");
          wrap.setAttribute("data-slug", slugNow);
        }
      }
  ...
  window.addEventListener("popstate", ()=>{ setTimeout(tryRender, 50); setTimeout(tryRender, 400); });


  // Watch DOM: if native blocks appear later, remove fallback
  try{
    new MutationObserver(function(){ killIfNative(); }).observe(document.documentElement, {childList:true, subtree:true});
  }catch(e){}

  // Initial
  window.addEventListener("load", ()=>{ setTimeout(tryRender, 50); setTimeout(tryRender, 400); setTimeout(tryRender, 1200); });

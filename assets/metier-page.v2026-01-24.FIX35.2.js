/* metier-page.js — Ulydia — FIX35.2 (Propal layout without Tailwind + slug matching + hide empty boxes)
*/
(() => {
  if (window.__ULYDIA_METIER_PAGE_FIX352__) return;
  window.__ULYDIA_METIER_PAGE_FIX352__ = true;

  const DEBUG = !!window.__METIER_PAGE_DEBUG__;
  const log = (...a) => DEBUG && console.log("[metier-page FIX35.2]", ...a);
...
`;
  ...
  async function tryFetchBanner({ slug, iso }){
    ...
    try {
      ...
      const link = pickUrl(sponsor?.link || sponsor?.website || "");

      if (!wide && !square) return null;
      return { wide, square, link };
    } catch(e) {
      log("banner fetch failed", e);
      return null;
    }
  }
          ...
          <div class="ul-container">
            <div class="ul-header-row">
              <div class="ul-title-wrap">
                ...
                <div class="ul-sub">Pays : <b>${escapeHtml(iso)}</b> • script=FIX35.2</div>
      ...
      <div class="ul-metier">
        <header class="ul-header">
          <div class="ul-container">
            <div class="badge badge-primary">🎯 Fiche métier</div>
            <div class="ul-h1">${escapeHtml(title)}</div>
            <div class="ul-sub">Chargement… • Pays : <b>${escapeHtml(iso)}</b> • script=FIX35.2</div>
          </div>
        </header>
      </div>
    `;
  ...
  try { main(); } catch(e) { console.error("[metier-page FIX35.2] fatal", e); }
})();

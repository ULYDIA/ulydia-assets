...
function pickNumber(){
  ...
  function getFAQs(){
    // Prefer worker-provided FAQ (Airtable via Worker)
    try{
      var pd = getPageData && getPageData();
      if (pd){
        var fw = pd.faq || pd.faqs || pd.FAQ || pd.FAQs || null;
        if (Array.isArray(fw) && fw.length) return fw;
      }
    }catch(e){}
  ...
  function getFicheMetiers(){
    // Prefer worker-provided data (Airtable via Worker)
    try{
      var pd = getPageData && getPageData();
      if (pd){
        var mw = pd.metier || pd.fiche || pd.job || null;
        var mwArr = pd.metiers || pd.fiches || pd.jobs || null;
        if (Array.isArray(mwArr) && mwArr.length) return mwArr;
        if (mw && typeof mw === "object") return [mw];
      }
    }catch(e){}
  ...
  function findFiche(slug){
    ...
    return (list||[]).find(function(x){
      if (!x) return false;
      var s = slugify(x.slug || x.Slug || x.metier_slug || x.metier || x.name);

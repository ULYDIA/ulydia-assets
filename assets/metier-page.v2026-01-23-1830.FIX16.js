...
// ---------- FAQ ----------
  ...
  // normalize (payload: question/answer; cms: q/a)
  const items = raw.map((f) => {
    const q = String(f?.q || f?.question || f?.Question || f?.title || f?.name || "").trim();
    const a = String(f?.a || f?.answer || f?.Answer || f?.reponse || f?.response || f?.html || "").trim();
    const ordre = Number(f?.ordre || f?.order || f?.pos || 0) || 0;
    return { q, a, ordre };
  }).filter(x => x.q && x.a);

  let box = cardByTitleId("faq-title");
  if (!box) {
    box = createSection({
      id: "ul-faq",
      titleId: "faq-title",
      title: T("Questions fréquentes", "FAQ"),
      icon: "❓",
      color: "ul-yellow"
    });
  }
  ...
  const body = box.querySelector(".ul-card-body");
  body.innerHTML = "";

  const acc = document.createElement("div");
  acc.className = "ul-accordion";

  items.forEach((it) => {
    const det = document.createElement("details");
    det.className = "ul-acc-item";

    const sum = document.createElement("summary");
    sum.className = "ul-acc-q";
    sum.textContent = it.q;

    const ans = document.createElement("div");
    ans.className = "ul-acc-a";
    ans.innerHTML = it.a;
...
setRich("description-title", f.description || "");
    ...
    if (!okBloc) {
      ...
      hidePaysCardsByHeader();
        ...
        // FAQ
    // Prefer payload.faq (already scoped to this metier by the Worker)
    let faqToRender = [];
    if (payload && Array.isArray(payload.faq) && payload.faq.length) {
      faqToRender = payload.faq;
    } else {
      // Fallback: read from hidden Webflow CMS list and filter by metier slug/name if provided
      let faqList = [];
      try { faqList = readFAQSource(); } catch (e) { faqList = []; }
      const metSlug = String(metierSlug || (metier && metier.slug) || "").trim().toLowerCase();
      const metName = String((metier && metier.name) || "").trim().toLowerCase();
      faqToRender = (faqList || []).filter(f => {
        const m = String(f?.metier || f?.job || f?.metier_slug || "").trim().toLowerCase();
        if (!m) return true; // if CMS didn't provide a metier key, keep it
        return m === metSlug || m === metName;
      });
    }
    renderFAQ(faqToRender);  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", () => boot().catch(e => overlayError("Boot failed", e)));

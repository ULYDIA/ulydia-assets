...
function getPageData(){
  return window.__ULYDIA_METIER_PAGE_V29__ || window.__ULYDIA_METIER_PAGE_V28__ || window.__ULYDIA_METIER_PAGE__ || window.__ULYDIA_PAGE_DATA__ || null;
}
...
function pickArr(){
  ...
  var c = pickArr(
    ...
    pd.countries, pd.pays, pd.country_list,
...
function readFAQsFromDOM(){
  ...
  var root = document.querySelector(".ul-cms-faqs-source")
          || document.querySelector(".ul-cms-faq-source")
          || document;

  var items = Array.prototype.slice.call(root.querySelectorAll(".w-dyn-item")).filter(function(it){
  ...
  items.forEach(function(it){
    ...
    out.push({ metier: metier, iso: iso, question: question, reponse: answer });
  });
  ...
  var fm = pickArr(
    ...
    pd.fiche_metiers, pd.ficheMetiers, pd.fiches, pd.fiche, pd.metiers, pd.jobs,
  ...
  if (Array.isArray(m) && m.length){
    var hasAny = m.some(function(x){
      return x && (x.description || x.missions || x.environnements || x.profil_recherche || x.evolutions_possibles || x.competences || x.accroche);
    });
    if (hasAny) return m;
  }

  // last resort: dom scan of metier list elements (only those that have a js-metier-slug)
  try{
    var items = Array.prototype.slice.call(document.querySelectorAll('.w-dyn-item'));
    var out = [];
    items.forEach(function(item){
      if (!item.querySelector('.js-metier-slug') && !item.querySelector('.js-fiche-slug')) return;
      var slug = norm((item.querySelector('.js-metier-slug')||item.querySelector('.js-fiche-slug')).textContent);
      if (!slug) return;
      var name = norm((item.querySelector('.js-metier-name')||item.querySelector('.js-fiche-name')) && (item.querySelector('.js-metier-name')||item.querySelector('.js-fiche-name')).textContent);
      var lang = norm((item.querySelector('.js-metier-lang')||item.querySelector('.js-fiche-lang')) && (item.querySelector('.js-metier-lang')||item.querySelector('.js-fiche-lang')).textContent).toLowerCase();
      out.push({ slug: slug, name: name, lang: lang });
    });
...
// ---------- Worker (sponsor) ----------
  ...
  var body = '<div class="u-muted" style="line-height:1.6">' +
    'This job description is not yet available in the country "<b>' + esc(countryName) + '</b>".' +
    '</div>';
  wrap.appendChild(card('Not available', '', body));
  return wrap;
}
...
if (fiche){
  var ficheLang = normLang(fiche.lang || fiche.language || fiche.locale || fiche.langue || '');
  if (ficheLang && ficheLang !== finalLang) fiche = null;
  if (!ficheLang && finalLang !== 'fr') fiche = null; // legacy single-language dataset assumed FR
}

...
function pickNumber(){
  ...
  function getFicheMetiers(){
    ...
    // New: Supabase worker schema
    if (pd.fiche && typeof pd.fiche === "object") return [pd.fiche];
    if (Array.isArray(pd.fiche_metiers) && pd.fiche_metiers.length) return pd.fiche_metiers;
    if (pd.metier && typeof pd.metier === "object") return [pd.metier];

    // 1) Arrays (legacy Webflow exports / JSON scripts)
    var arr = pickArr(
      window.__ULYDIA_FICHE_METIERS__,
      pd.fiche_metiers, pd.ficheMetiers
    );
    if (arr && arr.length) return arr;

    // 2) Single object fallback (legacy)
    var o = pd.fiche_metier || pd.ficheMetier || pd.job || null;
    if (o) return [o];

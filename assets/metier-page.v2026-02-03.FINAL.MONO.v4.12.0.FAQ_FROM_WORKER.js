...
/* =========================================================
   ULYDIA — FAQ ADAPTER (Worker-first, DOM fallback)
   - Consumes ctx.faqs from /v1/metier-page?slug=&iso=
   - Falls back to window.__ULYDIA_FAQS__ if present
   - Renders into #ulydia-faq-root (or creates it)
========================================================= */
(function(){
  try{
    var root = document.getElementById('ulydia-faq-root');
    if(!root){
      root = document.createElement('section');
      root.id = 'ulydia-faq-root';
      var mount = document.getElementById('ulydia-metier-root') || document.body;
    ...
    function pickFaqs(){
      // 1) worker ctx
      try{
        if (window.__ULYDIA_PAGE_CTX__ && Array.isArray(window.__ULYDIA_PAGE_CTX__.faqs)){
          return window.__ULYDIA_PAGE_CTX__.faqs;
        }
      }catch(e){}
      // 2) legacy/global
      if (Array.isArray(window.__ULYDIA_FAQS__)) return window.__ULYDIA_FAQS__;
      return [];
    }

    var faqs = pickFaqs().filter(function(f){
      return (f && (f.question || f.Question) && (f.answer || f.Reponse || f.Reponse_FR));
    });

    if(!faqs.length){
      root.innerHTML = '<div class="ul-empty-faq">Aucune question disponible pour ce métier.</div>';
      return;
    }

    var html = '<h2 class="ul-faq-title">FAQs</h2><div class="ul-faq-list">';
    for(var i=0;i<faqs.length;i++){
      var q = faqs[i].question || faqs[i].Question || '';
      var a = faqs[i].answer || faqs[i].Reponse || faqs[i].Reponse_FR || '';
      html += '<details class="ul-faq-item"><summary>'+esc(q)+'</summary><div class="ul-faq-a">'+a+'</div></details>';
    }
    html += '</div>';
    root.innerHTML = html;
  }catch(e){}

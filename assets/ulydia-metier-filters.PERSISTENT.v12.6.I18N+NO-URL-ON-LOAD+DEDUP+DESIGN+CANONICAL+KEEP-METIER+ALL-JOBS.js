/* ULYDIA – METIER FILTER BAR (PERSISTENT) — DOWNLOADABLE
   v2026-02-05.PATCH.FILTERS.v12.6 (CANONICAL-URL-ORDER + KEEP-METIER-ON-LOAD + JOBS-ALL-WHEN-NO-SECTOR) (COUNTRY+SECTOR+METIER, COUNTRY-AVAILABILITY, PERSISTENT BAR)
...
(function(){
  ...
  "use strict";
  if (window.__ULYDIA_METIER_FILTERS_V12_6__) return;
  window.__ULYDIA_METIER_FILTERS_V12_6__ = true;
  ...
  function langNow(){
    try{
      var l = window.__ULYDIA_FINAL_LANG__ || window.__ULYDIA_LANG_FINAL__ || window.__ULYDIA_LANG__ || "";
  ...
  function trLabel(key, fallback){
    try{
      var lang = langNow();
      var dict = (window.__ULYDIA_I18N_LABELS__ && (window.__ULYDIA_I18N_LABELS__[lang] || window.__ULYDIA_I18N_LABELS__.en)) || {};
      return dict[key] || fallback || key;
    }catch(e){ return fallback || key; }
  }
var __userInteracted = false;
  function markUser(e){ try{ if(e && e.isTrusted) __userInteracted = true; }catch(_){ __userInteracted = true; } }
  ...
  function setQ(next){
    ...
    if (url !== (location.pathname + location.search)){
      // 🚫 On initial load, do NOT rewrite URL if no metier is selected
      if (!metier && !__userInteracted){
        // keep the original URL as-is
      } else {
        try{
          if (__userInteracted) history.pushState({}, "", url);
  ...
  function buildOnce(){
    ...
    function rebuild(){
      ...
      var sectorPlaceholder = trLabel("sector","Sector");
      ...
      var metierPlaceholder = trLabel("missing_job_title", "Job");

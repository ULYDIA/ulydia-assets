/*!
 * ULYDIA — Filter Bar (PATCH13 FULL REBUILD)
 * File: ulydia-metier-filters.v2026-02-02.FINAL.PATCH13.EVENTMODEL.FOCUSLOCK.js
 *
 ...
 */
(() => {
  if (window.__ULYDIA_METIER_FILTERS_PATCH13__) return;
  window.__ULYDIA_METIER_FILTERS_PATCH13__ = true;
  ...
  function buildOptions(state) {
    ...
    if (sectorGlobals.length) {
      ...
      // fallback: derive from job items
      jobs.forEach(j => {
        const label = pickByLang(j.sectors || {}, finalLang);
        if (!label) return;
        const key = slugify(label);
        if (key && !sectorMap.has(key)) sectorMap.set(key, label);
      });
    ...
    return { ui, finalLang, countryOptions, sectorOptions, jobOptions, jobsCount: jobs.length, sectorGlobalsCount: sectorGlobals.length };
  }

...
setRich("description-title", f.description || "");
    ...
    let faqFiltered = [];
    if (faqFromPayload) {
      // Payload FAQs are already metier-specific; only enforce iso/lang if present on items
      faqFiltered = (payload.faq || []).filter(it => {
        if (!it) return false;
        const iso2 = String(it.iso || it.country_code || it.code_iso || it.country || "").trim().toUpperCase();
        const lang2 = String(it.lang || it.langue || it.language || "").trim().toLowerCase();
        if (iso2 && iso2 !== iso) return false;
        if (lang2 && lang2 !== lang) return false;
        return true;
      });
    } else {
      faqFiltered = Array.isArray(faqList) ? faqList.filter(faqMatches) : [];
    }

...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-ind-val{font-size:16px;font-weight:900;color:var(--primary);}
  ...
  ctx.mpb.blocks.forEach(function(b){
    ...
    if (ctx.mpb){
      ...
      function splitTagsLoose(s){
        ...
        if (Array.isArray(s)) {
          var arr = s.map(function(x){ return norm(x); }).filter(Boolean);
...
function skillCard(title, theme, icon, arr, mode){
        ...
        var html = '<div class="kpi-box" style="margin-bottom:0;">'
          + '<div class="kpi-icon" style="background:rgba(99,102,241,0.10);color:#6366f1;font-weight:900;">'+esc(icon||'✓')+'</div>'
          + '<div style="flex:1;">'
          + content
          + '</div></div>';
        right.appendChild(card(title, theme, html));
      }

      // Split "Compétences & parcours" into 4 independent blocks (more presentable)
      skillCard(ui(lang2,'key_fields'), 'pastel-purple', '✓', topFields);
      skillCard(ui(lang2,'must_have'), 'bg-purple', '⚡', mustHave);
      skillCard(ui(lang2,'soft_skills'), 'pastel-pink', '🧩', soft, 'soft_lines');
      skillCard(ui(lang2,'tools_stack'), 'pastel-cyan', '🛠️', tools);

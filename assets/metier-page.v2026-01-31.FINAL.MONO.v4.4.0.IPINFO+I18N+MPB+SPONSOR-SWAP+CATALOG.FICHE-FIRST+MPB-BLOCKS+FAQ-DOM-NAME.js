...
(function(){
  ...
  function readFAQsFromDOM(){
    // Fallback: read FAQs from hidden Webflow Collection List (Option A)
    // Expected wrapper class: .ul-cms-faqs-source OR any .w-dyn-item with .js-faq-question + .js-faq-answer
    function _norm(s){ return String(s||"").replace(/\u00a0/g," ").replace(/\s+/g," ").trim(); }
    function _txt(root, sel){ var el=root.querySelector(sel); return el ? _norm(el.textContent||"") : ""; }
    function _html(root, sel){ var el=root.querySelector(sel); return el ? String(el.innerHTML||"").trim() : ""; }

    var root = document.querySelector(".ul-cms-faqs-source") || document.querySelector(".ul-cms-faq-source") || document;
    var items = Array.prototype.slice.call(root.querySelectorAll(".w-dyn-item"))
      .filter(function(it){
        return it && (it.querySelector(".js-faq-question") || it.querySelector(".js-faq-q") || it.querySelector(".js-faq-titre") || it.querySelector(".js-faq-title"));
      });
    ...
    items.forEach(function(it){
      // metier NAME can be exported as .js-faq-metier, or sometimes the item name is in .js-faq-name
      var metier = _txt(it, ".js-faq-metier") || _txt(it, ".js-faq-job") || _txt(it, ".js-faq-name") || _txt(it, ".js-faq-title-metier");
      // If not present, try Webflow name field export if you stored it
      if (!metier){
        metier = _txt(it, ".js-name") || _txt(it, ".w-dyn-item .js-name");
      }

      var iso = _txt(it, ".js-faq-iso") || _txt(it, ".js-faq-country") || _txt(it, ".js-faq-code-iso");

      var question = _txt(it, ".js-faq-question") || _txt(it, ".js-faq-q") || _txt(it, ".js-faq-titre") || _txt(it, ".js-faq-title");
      // answer often exported as rich text
      var answer = _html(it, ".js-faq-answer") || _html(it, ".js-faq-a") || _html(it, ".js-faq-reponse") || _html(it, ".js-faq-body");
  ...
  function getFAQs(){
    var list = window.__ULYDIA_FAQS__
    ...
    if (!Array.isArray(list) || list.length === 0){
      try{
        var dom = readFAQsFromDOM();
        if (dom && dom.length) list = dom;
      }catch(e){}
    }
    return list;
  }
  ...
  function findMPB(slug, iso){
    ...
    // collect all matches (because MPB can be "one item per block")
    var matches = [];
    for (var i=0;i<list.length;i++){
      var b = list[i];
      if (!b) continue;
    ...
    if (!matches.length){
      if (DEBUG){
        try{
          var sameIso = list.filter(function(x){
            return x && (!ciso || normalizeIso(getIsoKey(x)) === ciso);
          });
          console.warn('[ULYDIA][MPB] not found', {
            target:{ metier: sslug, iso: ciso },
            total: list.length,
            sameIso: sameIso.length,
            sample: sameIso.slice(0,8).map(function(x){
              return { iso: getIsoKey(x), metier: getMetierKey(x) };
            })
          });
        }catch(e){}
      }
    ...
    // If there is exactly one item and it already looks like the "flat fields" model, normalize it.
    ...
    // Otherwise: merge block-style items into one MPB object with `blocks[]`
    var merged = { iso: ciso, metier: slug };
    merged.blocks = matches.map(function(b){
      var title = pickText(b.bloc_title, b.title, b.titre, b.blocTitle, b.bloc_titre, b.name_title, b.heading, b.block_title) || pickText(b.key, b.section);
      var body  = pickText(b.bloc_body, b.body, b.contenu, b.html, b.rich, b.content, b.text) || pickHTML(b.body_html, b.html, b.rich_html, b.richText);
      var order = pickNumber(b.bloc_order, b.order, b.ordre, b.position, b.rank);
      // DOM reader may provide these under stable keys
      if (!title && b.bloc && typeof b.bloc === 'object'){
        title = title || pickText(b.bloc.title, b.bloc.titre);
        body  = body  || pickHTML(b.bloc.body, b.bloc.html);
        order = order || pickNumber(b.bloc.order);
      }
      // Fallback to DOM reader keys
      title = title || b.bloc_title || b.blocTitle || b.title || b.titre || "";
      body  = body  || b.bloc_body || b.blocBody || b.body || b.contenu || b.html || "";
      order = (order == null) ? (b.bloc_order != null ? b.bloc_order : null) : order;
      return { title: String(title||'').trim(), body: String(body||'').trim(), order: order };
    }).filter(function(x){ return x && x.body; });

    // sort blocks by order then title
    ...
    for (var i=0;i<list.length;i++){
      ...
      var bslug = slugify(metKey);
      // Sometimes "metier" is stored separately from slug fields
      var bname = slugify(b.metier || b.metier_name || b.metierName || b.name || "");

      // tolerant comparisons (handles "Orthophoniste (H/F)" etc.)
      var ok = (bslug === sslug) || (bname === sslug) ||
               (bslug && (bslug.indexOf(sslug) === 0 || sslug.indexOf(bslug) === 0));
...
function findFAQsForMetier(metierSlug, ficheName, iso){
  ...
  var targetName = slugify(ficheName || '');
  var ISO = normalizeIso(iso);
  ...
  for (var i=0;i<list.length;i++){
    ...
    // FAQ metier is a NAME field in your CMS (preferred)
    ...
    var okMetier = false;
    if (targetName){
      okMetier = (mName && slugify(mName) === targetName);
      // fallback: some datasets store the job name in another field
      if (!okMetier && mSlug && slugify(mSlug) === targetName) okMetier = true;
    }else{
      // fallback if no ficheName available
      if (mName && (slugify(mName) === targetSlug)) okMetier = true;
      else if (mSlug && slugify(mSlug) === targetSlug) okMetier = true;
    }
    if (!okMetier) continue;

    var fIso = normalizeIso(f.iso || f.code_iso || f.country_code || f.countryCode || f.codeIso || f.country || "");
    if (ISO && fIso && fIso !== ISO) continue;
...
// ---------- Worker (sponsor) ----------
  ...
  function renderNotAvailable(ctx){
    ...
    var body = '<div style="color:#334155;line-height:1.5">' +
      esc(ui(lang,'not_available_msg_prefix')) + '"' + esc(countryName) + '".</div>';
    wrap.appendChild(card(ui(lang,'not_available_title'), '', body));
    return wrap;
  }
...
function renderSkillTags(title, items){
  ...
  async function main(){
    ...
    if (q.country && !country && !catCountry){
      var rootBad = ensureRoot();
      rootBad.innerHTML = '';
      var ctxBad = { iso: q.country, country: { label: q.country, name: q.country, lang: 'en' } };
      // reuse not-available card style but with a clearer message
      var wrapBad = document.createElement('div');
      wrapBad.className = 'u-wrap';
      var bodyBad = '<div class="u-muted" style="line-height:1.6">Invalid country code "<b>' + esc(String(q.country||'')) + '</b>". Use ISO2 like <b>FR</b>, <b>DE</b>, <b>ES</b>, <b>IT</b>, <b>GB</b>.</div>';
      wrapBad.appendChild(card('Country not supported', '', bodyBad));
      rootBad.appendChild(wrapBad);
      return;
    }

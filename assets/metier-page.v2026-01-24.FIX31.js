window.__METIER_PAGE_BUILD__ = "metier-page v2026-01-24 FIX31 (strict FAQ + safe boot + build early)";
/* FIX31: versioned guard that never hides build info */
(function(){
  var VER = "FIX31";
  try{
    if (window.__ULYDIA_METIER_PAGE_LOADED_VER__ === VER) return;
    window.__ULYDIA_METIER_PAGE_LOADED_VER__ = VER;
  }catch(_){}
})();
...
(() => {
/* FIX31_UNHIDE_SAFETY: never leave page invisible on fatal */
function __ulydia_unhide_safe__(){
  try{ document.documentElement.classList.remove("ul-metier-loading"); }catch(_){}
  try{
    var r = document.getElementById("ulydia-metier-root");
    if (r) { r.style.opacity = "1"; r.style.display = ""; }
  }catch(_){}
}
...
function faqMatches(item, ctx){
  ...
  // --- FIX29: compute page identifiers robustly from ctx + H1 ---
  ...
  // Prefer human label/title; fallback to H1
  let pageName = String(ctx?.metier?.label || ctx?.metier?.title || ctx?.metier?.name || "").trim();
  if (!pageName) pageName = String(pageH1 || "").trim();
  ...
  // Candidate metier identifier stored in FAQ item (Make sets FAQ Name = metier name)
  const candidates = [];
  [
    it.metier,
    it.metier_name,
  ...
  for (const raw of candidates){
    ...
    if (pageSlugKey && cSlug && (cSlug === pageSlugKey)) return true;
if (pageNameSlug && cSlug && (cSlug === pageNameSlug)) return true;
}
  return false;
}
...
try {
  ...
  if ((!Array.isArray(_faqFiltered) || _faqFiltered.length === 0) && Array.isArray(faqAll) && faqAll.length) {
    ...
    if ((!Array.isArray(_faqFiltered) || _faqFiltered.length === 0) && Array.isArray(faqAll) && faqAll.length) {
  // FIX29: keep empty if no match; do NOT show unrelated FAQs
  try { console.warn("[metier-page][FAQ] No match after filters; keeping FAQ empty", { pageNameRaw, total: faqAll.length }); } catch(_){ }
}
} catch(e) {
  // ignore
}

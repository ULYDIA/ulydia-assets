...
(function(){
  ...
  function normalizeIso(value){
    // Accept ISO2 (FR) or country names (France) coming from CMS text fields
    var v = norm(value).toUpperCase();
    ...
    if (Array.isArray(list)){
      for (var i=0;i<list.length;i++){
        ...
        // sometimes CMS stores native name
  ...
  function findMPB(slug, iso){
    ...
    var ciso = normalizeIso(String(iso||""));
...
// ---------- Worker (sponsor) ----------
  ...
  function renderNotAvailable(ctx){
    ...
    var body = '<div class="u-muted" style="line-height:1.6">' +
      esc(ui(lang,'not_available_msg_prefix')) + '"' + esc(countryName) + '".' +
      '</div>';
...
function renderSkillTags(title, items){
  ...
  async function main(){
    ...
    // Availability is country-specific: if there is no MPB for this ISO, the job is not available for that country.
    if (!ctx.mpb) {

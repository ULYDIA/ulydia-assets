...
// -------------------------
    ...
    if (ji){
      // Prefer richer description; fallback to accroche/overview if needed
      var overviewHtml = ji.description || ji.overview || ji.accroche || "";
      if (overviewHtml) leftCol.appendChild(cardSection(ui.job_overview, "✨", cleanBlock(overviewHtml), "overview"));

      if (ji.missions)        leftCol.appendChild(cardSection(ui.job_missions, "✅", cleanBlock(ji.missions), "missions"));
      if (ji.environnements)  leftCol.appendChild(cardSection(ui.job_environments, "🏢", cleanBlock(ji.environnements), "env"));
      if (ji.competences)     leftCol.appendChild(cardSection(ui.job_skills, "🧠", cleanBlock(ji.competences), "skills"));
      if (ji.profil_recherche)leftCol.appendChild(cardSection(ui.job_profile, "🎯", cleanBlock(ji.profil_recherche), "profile"));
      if (ji.evolutions_possibles) leftCol.appendChild(cardSection(ui.job_evolution, "📈", cleanBlock(ji.evolutions_possibles), "evolution"));
    }

    // MPB blocks are rendered on the RIGHT column (3 cards: Formation / Accès / Marché)

    // RIGHT column — Decide (sponsor + country specifics)
    // -------------------------
    ...
    // 0b) MPB 3 cards on the RIGHT (Formation / Accès / Marché)
    if (ctx.mpb){
      var mpb3 = ctx.mpb;
      if (mpb3.formation) right.appendChild(cardSection(ui.mpb_training, "🎓", cleanPara(mpb3.formation), "mpb-training"));
      if (mpb3.acces)     right.appendChild(cardSection(ui.mpb_access,   "📘", cleanPara(mpb3.acces),     "mpb-access"));
      if (mpb3.marche)    right.appendChild(cardSection(ui.mpb_market,   "📊", cleanPara(mpb3.marche),    "mpb-market"));
    }

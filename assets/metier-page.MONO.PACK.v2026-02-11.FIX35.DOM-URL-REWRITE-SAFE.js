...
// FIX35 — Safe DOM-based rewrite for accidental /undefined/v1/... loads
// - No global Element.setAttribute override (can have side-effects)
// - Patch only common URL properties + scan added nodes/attributes
(function(){
  const BASE = (window.ULYDIA_WORKER_URL && String(window.ULYDIA_WORKER_URL).trim() && String(window.ULYDIA_WORKER_URL).trim() !== "undefined")
    ? String(window.ULYDIA_WORKER_URL).trim().replace(/\/+$/,"")
    : "https://ulydia-data.contact-871.workers.dev";
  ...
  function fixUrl(u){
    try{
      ...
      // absolute on ulydia origin
      if(ORIGIN && u.startsWith(ORIGIN + "/undefined/v1/")){
        return BASE + u.substring((ORIGIN + "/undefined").length);
      }
      // relative forms
      if(u.startsWith("undefined/v1/")) return BASE + u.substring("undefined".length);
      if(u.startsWith("/undefined/v1/")) return BASE + u.substring("/undefined".length);

      // any absolute containing /undefined/v1/
      if(u.includes("/undefined/v1/")){
        const i = u.indexOf("/v1/");
        if(i !== -1) return BASE + u.substring(i);
      }
      return u;
    }catch(e){
      return u;
  ...
  // Scan DOM for existing/added attributes that would trigger loads
  function rewriteAttrs(root){
    try{
      const nodes = root && root.querySelectorAll ? root.querySelectorAll("img,script,link,iframe,source,video,a") : [];
      const attrs = ["src","href","data-src","data-href"];
      for(const el of nodes){
        for(const a of attrs){
          const v = el.getAttribute && el.getAttribute(a);
          if(!v) continue;
          const nv = fixUrl(String(v));
          if(nv && nv !== v) el.setAttribute(a, nv);
        }
      }
    }catch(e){}
  }
  ...
  // Initial scan
  try{ rewriteAttrs(document); }catch(e){}

  // Observe future changes (lazyload libs often set data-src then src)
  try{
    new MutationObserver((muts)=>{
      for(const m of muts){
        if(m.type === "attributes" && m.target){
          const a = (m.attributeName||"").toLowerCase();
          if(a === "src" || a === "href" || a === "data-src" || a === "data-href"){
            try{
              const el = m.target;
        ...
        if(m.addedNodes){
          for(const n of m.addedNodes){
            if(n && n.nodeType === 1) rewriteAttrs(n);
          }
        }
      }
    }).observe(document.documentElement, {subtree:true, childList:true, attributes:true, attributeFilter:["src","href","data-src","data-href"]});
  }catch(e){}
})();
// ULYDIA BUILD MARKER
    ...
    try{
      ...
      window.fetch = function(input, init){
        try{
          ...
          // --- FIX: prevent fetch('undefined/v1/...') -> https://www.ulydia.com/undefined/v1/...
          try{
            var base = (window.ULYDIA_WORKER_URL && String(window.ULYDIA_WORKER_URL).trim() && String(window.ULYDIA_WORKER_URL).trim() !== "undefined")
              ? String(window.ULYDIA_WORKER_URL).trim().replace(/\/+$/,"")
              : "https://ulydia-data.contact-871.workers.dev";
            if (url) {
              if (url.indexOf("/undefined/v1/") !== -1) {
                url = base + url.substring(url.indexOf("/v1/"));
                if (typeof input === "string") input = url;
                else if (isReq && req) input = new Request(url, req);
              } else if (url.indexOf(location.origin + "/undefined/v1/") === 0) {
                url = base + url.substring((location.origin + "/undefined").length);
                if (typeof input === "string") input = url;
                else if (isReq && req) input = new Request(url, req);
              } else if (url.indexOf("undefined/v1/") === 0) {
                url = base + url.substring("undefined".length);
                if (typeof input === "string") input = url;
                else if (isReq && req) input = new Request(url, req);
              }
            }
...
var q = getQuery();
    ...
    // ✅ Worker-first fallback: if Webflow DOM fiche is missing, use Worker payload
    try{
      if (!fiche && window.__ULYDIA_PAGE_DATA__ && window.__ULYDIA_PAGE_DATA__.metier){
        fiche = window.__ULYDIA_PAGE_DATA__.metier;
      }
    }catch(e){}

/* metier-page.v12.9.js — Ulydia (FIX14)
   Fixes requested:
...
*/
window.__METIER_PAGE_BUILD__ = "metier-page v2026-01-24 FIX28 (FAQ fallback + debug)";
...
(() => {
  ...
  const BOOT_VER = "2026-01-24-1102";
...
if (hasSponsor) {
  ...
  function collectFaqsFromDOM(){
    const wrap = document.querySelector(".ul-cms-source");
    if (!wrap) return [];
    const items = Array.from(wrap.querySelectorAll(".w-dyn-item"));
    const out = items.map(it => {
      const metierEl = it.querySelector(".js-faq-metier");
      const qEl = it.querySelector(".js-faq-q");
      const aEl = it.querySelector(".js-faq-a");
      const metier = (metierEl?.textContent || metierEl?.innerText || "").trim();
      const qText = (qEl?.textContent || qEl?.innerText || "").trim();
      const qHtml = (qEl?.innerHTML || "").trim();
      const aText = (aEl?.textContent || aEl?.innerText || "").trim();
      const aHtml = (aEl?.innerHTML || "").trim();
      // prefer html when available (Webflow rich text)
      const question = qText || qHtml.replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
      const answerHtml = aHtml || "";
      const answerText = aText || answerHtml.replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
      return {
        metier,
        question,
        // prefer HTML when available (Webflow rich text)
        answer: answerHtml || answerText
      };
    }).filter(x => x.question && (x.answer || x.answer_html));
    return out;
  }
  ...
  function normalizeFaqArray(any){
    if (!any) return [];
    if (Array.isArray(any)) return any;
    if (typeof any === "object") {
      const cand = any.items || any.faqs || any.questions || any.entries || any.data;
      if (Array.isArray(cand)) return cand;
      if (any.question && any.answer) return [any];
    }
    return [];
  }
  function faqMatchesMetier(faq, metierSlug, metierName){
    const m = normStr(faq?.metier || faq?.metier_slug || faq?.metierSlug || faq?.job || "");
    if (!m) return false;

    const ms = normStr(metierSlug);
    const mn = normStr(metierName);
    if (m === ms || slugifyLite(m) === ms) return true;
    if (m === mn || slugifyLite(m) === slugifyLite(mn)) return true;

    // sometimes the CMS stores a URL like /fiche-metiers/xxx or /metier?metier=xxx
    const urlSlug = m.split("?metier=")[1] || m.split("/").filter(Boolean).pop() || "";
    if (normStr(urlSlug) === ms || slugifyLite(urlSlug) === ms) return true;

    return false;
  }
  async function waitFaqDOM(maxMs){
    const t0 = Date.now();
    while (Date.now() - t0 < (maxMs||2000)){
      const list = collectFaqsFromDOM();
...
// ---------- FAQ ----------
  ...
  function renderFAQ(list) {
    ...
    if (!faqCard) {
      const root = document.getElementById("ulydia-metier-root") || document.body;
      const host = root.querySelector(".ul-col-main") || root.querySelector(".ul-main") || root.querySelector(".left-panel") || root.querySelector(".main-content") || root;
...
function blocMatches(bloc, slug, iso) {
  ...
  async function boot() {
    ...
    ensureStyle("ulydia-metier-fouc", ` `);
...
setRich("description-title", f.description || "");
    ...
    let faqList = normalizeFaqArray(payload?.faq || payload?.faqs || payload?.FAQ || payload?.metier?.faq || payload?.metier?.faqs || null);
...
// DOM fallback (Webflow embed):
    // - Preferred: read the hidden Webflow Collection List in the DOM (ul-cms-source)
    // - Also supports: <script id="faqData" type="application/json">[...]</script>
    // - Legacy: window.__ULYDIA_FAQS__
    if (!Array.isArray(faqList) || faqList.length === 0) {
      try{
        const domFaqs = await waitFaqDOM(2500);
        if (Array.isArray(domFaqs) && domFaqs.length) faqList = domFaqs;
      }catch(_){}
    }
    if (!Array.isArray(faqList) || faqList.length === 0) {
      try{
    ...
    // Match FAQ -> current metier using NAME (robust) + supports legacy slug matching
function faqMatches(item, ctx){
  const it = item || {};

  // page identifiers
  const pageSlug = String(ctx?.slug || ctx?.metier?.slug || "").trim();

// Prefer payload human name; fallback to the H1 already rendered on the page (Webflow shell)
let pageName = String(ctx?.metier?.name || ctx?.metier?.title || ctx?.metier?.label || "").trim();
try {
  const h1 = document.getElementById("metier-title") || document.querySelector("h1");
  const h1t = (h1 && h1.textContent) ? String(h1.textContent).trim() : "";
  // If payload has no human title (or looks like a slug), use the H1
  if (!pageName || (pageName.includes("-") && !pageName.includes(" ")) ) {
    if (h1t) pageName = h1t;
  }
} catch(_) {}
// normalize to compare human names reliably (accents, punctuation, multiple spaces)
  function normName(s){
    return String(s || "")
      .trim()
      .toLowerCase()
  ...
  // slugify for tolerant comparisons
  function slugify(s){
    return String(s || "")
      .trim()
      .toLowerCase()
  ...
  const pageNameKey = normName(pageName);
  const pageSlugKey = String(pageSlug || "").trim().toLowerCase();

  // Candidate "metier name" stored in FAQ:
  // - Preferred: explicit metier fields
  // - Fallback: FAQ "name/title" (your Make scenario sets FAQ Name = metier name)
  const candidates = [];
  [
    it.metier,
    it.metier_name,
  ...
  if (candidates.length === 0) {
    // fallback to "name/title" ONLY if it looks like a metier label
    // (and not the question). We'll still accept it if it matches the current metier name.
    const nm = it.name || it["Name"] || it.title || it["Titre"] || "";
    if (nm) candidates.push(String(nm));
  }

  if (candidates.length === 0) return false;

  for (const raw of candidates){
    const cNameKey = normName(raw);
    if (pageNameKey && cNameKey && cNameKey === pageNameKey) return true;

    // tolerant: sometimes stored as "Controleur aerien (FR)" etc.
    if (pageNameKey && cNameKey && (cNameKey.includes(pageNameKey) || pageNameKey.includes(cNameKey))) return true;

    // legacy slug comparisons
    const cSlug = slugify(raw);
    const pageNameSlug = slugify(pageName);
    if (pageSlugKey && cSlug && (cSlug === pageSlugKey || cSlug.includes(pageSlugKey) || pageSlugKey.includes(cSlug))) return true;
    if (pageNameSlug && cSlug && (cSlug === pageNameSlug || cSlug.includes(pageNameSlug) || pageNameSlug.includes(cSlug))) return true;
  }
  ...
  return false;
    ...
    // Use the resolved faqList (payload -> endpoints -> DOM embed -> window var -> <script>),
// then apply matching to the current metier if needed.
const faqAll = (Array.isArray(faqList) && faqList.length)
  ? faqList
  : ((Array.isArray(ctx?.faq) && ctx.faq.length) ? ctx.faq : []);
...
// --- FIX28: If filtering yields 0 but we clearly have FAQs in the DOM, apply a tolerant fallback
// This prevents the whole FAQ card from being hidden due to naming/slug mismatches.
let _faqFiltered = faqFiltered;
try {
  const pageH1 = (document.getElementById("metier-title") || document.querySelector("h1"))?.textContent || "";
  const pageNameRaw = String((ctx?.metier?.name || ctx?.metier?.title || ctx?.metier?.label || "") || pageH1 || "").trim();
  const norm = (s) => String(s||"").trim().toLowerCase()
    ...
    .replace(/\s+/g," ")
    .trim();
  const pageKey = norm(pageNameRaw);

  if ((!Array.isArray(_faqFiltered) || _faqFiltered.length === 0) && Array.isArray(faqAll) && faqAll.length) {
    // Attempt: match by normalized metier/name fields
    const pass2 = (faqAll || []).filter(it => {
      const m = (it?.metier || it?.metier_name || it?.metierName || it?.name || it?.title || "") + "";
      const k = norm(m);
      if (!k || !pageKey) return false;
      return k === pageKey || k.includes(pageKey) || pageKey.includes(k);
    });
    if (pass2.length) _faqFiltered = pass2;

    // Last resort: show a small subset rather than hiding the whole section
    if ((!_faqFiltered || !_faqFiltered.length) && faqAll.length) {
      _faqFiltered = faqAll.slice(0, Math.min(10, faqAll.length));
      try { console.warn("[metier-page][FAQ] No match after filters; showing first items as fallback", { pageNameRaw, total: faqAll.length }); } catch(_){}
    } else {
      try { console.log("[metier-page][FAQ] matched", _faqFiltered.length, "of", faqAll.length, "for", pageNameRaw); } catch(_){}
    }
  }
} catch(e) {
  // ignore
    ...
    const faqReady = (_faqFiltered || faqFiltered).filter(x => {

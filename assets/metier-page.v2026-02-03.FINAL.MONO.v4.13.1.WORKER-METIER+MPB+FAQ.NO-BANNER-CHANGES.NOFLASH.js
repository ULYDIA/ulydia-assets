...
function applyWorkerPayloadToCtx(ctx, j){
  ...
  try{
    ...
    if (j.metier && typeof j.metier === "object"){
      ...
      ctx.name = String(m.title || m.Nom || m.name || ctx.name || ctx.slug || "").trim();
...
log('[ULYDIA] v4.13.1.1 ready', {metier: ctx.slug, iso: ctx.iso, lang: langFromCtx(ctx), hasFiche: !!ctx.fiche, hasMPB: !!ctx.mpb, hasCountry: !!ctx.country});
    }

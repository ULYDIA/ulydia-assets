...
(() => {
  ...
  function getRoot(){
    ...
    // If someone mistakenly placed the root in <head>, or before the template header,
    // relocate it right AFTER the header wrapper so it never appears above the header.
    try {
      const header =
        document.querySelector(".Header.Wrapper") ||              // Webflow: class "Header Wrapper"
        document.querySelector(".header-wrapper") ||
        document.querySelector("header");

      // If root is inside <head>, move it into <body> first.
      if (root.parentNode === document.head) {
        (document.body || document.documentElement).prepend(root);
      }

      if (header && header.parentNode) {
        const isBeforeHeader = !!(root.compareDocumentPosition(header) & Node.DOCUMENT_POSITION_FOLLOWING);
        const alreadyAfter = !!(header.compareDocumentPosition(root) & Node.DOCUMENT_POSITION_FOLLOWING);

        // We want: header ... then root
        // If root is before header (or not after), move it after header.
        if (!alreadyAfter || isBeforeHeader) {
          header.insertAdjacentElement("afterend", root);
        }
      }
    } catch(e){
      // don't break rendering if the DOM is exotic
      log("root placement warning", e?.message || e);
    }

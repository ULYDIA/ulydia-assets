    ...
    if (!q.metier){
      ...
      root0.innerHTML =
        '<div style="max-width:1100px;margin:24px auto;border-radius:18px;overflow:hidden;background:#fff;box-shadow:0 12px 30px rgba(15,23,42,.08);border:1px solid rgba(148,163,184,.35)">' +
          ...
          '</div>' +
        '</div>';
      try{ window.dispatchEvent(new CustomEvent(\"ulydia:metier:rendered\", { detail: { metier: \"\", country: (q.country||\"\"), notAvailable: false } })); }catch(e){}
      q.metier
    }
...
// Keep loader homogeneous: ensure it is visible at least MIN_LOADER_MS to avoid flicker
    ...
    try{ window.hideMetierLoader && window.hideMetierLoader(); }catch(e){}
    renderPage(ctx);
    // ✅ Notify external UI (search bar nav loader) that the page rendered
    try{ window.dispatchEvent(new CustomEvent("ulydia:metier:rendered", { detail: { metier: ctx.slug, country: ctx.iso, notAvailable: false } })); }catch(e){}

...
function applyWorkerPayloadToCtx(ctx, j){
  try{
    ...
    // country / lang from Worker (if provided)
    if (j.iso) ctx.iso = String(j.iso).toUpperCase();
    if (j.lang) ctx.lang = String(j.lang).toLowerCase();

    if (j.pays){
      ctx.country = j.pays;
      // Worker may provide banners at pays.banners
      if (j.pays.banners) ctx.banners = j.pays.banners;
      // also keep legacy paths
      if (!ctx.banners && j.pays.banniere_sponsorisation_image_2) ctx.banners = ctx.banners || {};
    }

    // sponsor
    var s = j.sponsor || j.sponsoring || j.sponsorship || null;
    if (!s && j.pays && j.pays.sponsor) s = j.pays.sponsor;
    ctx.sponsor = s || ctx.sponsor || null;
    ...
    if (j.metier && typeof j.metier === "object"){
      ...
      // Sector can come from Worker (string or object)
      if (j.secteur) ctx.sector = j.secteur;
      // keep a minimal list for any legacy readers
    ...
    if (Array.isArray(j.blocs_pays)){
      ...
      try{
        // prefer using existing matching logic
...
// Render ASAP if we already have Webflow fallback data
    ...
    // Then try Worker (Airtable-first) to hydrate: metier + mpb + faq + sponsor + banners
    fetchWorkerPage(ctx.slug, ctx.iso, langFromCtx(ctx)).then(function(payload){
      ...
      // Render with best available data (prevents "flash" non-sponsor / empty blocks)
      renderPage(ctx);
...
log('[ULYDIA] v4.13 ready', {metier: ctx.slug, iso: ctx.iso, lang: langFromCtx(ctx), hasFiche: !!ctx.fiche, hasMPB: !!ctx.mpb, hasCountry: !!ctx.country});
    }

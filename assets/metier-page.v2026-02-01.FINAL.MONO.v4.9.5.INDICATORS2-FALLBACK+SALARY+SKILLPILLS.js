...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-accroche{color:rgba(51,65,85,.86);font-size:16px;line-height:1.55;margin:0 0 18px 0;max-width:820px;font-weight:800;}
...
.u-sal-v{font-weight:900;color:rgba(17,24,39,92);}
      ...
      .u-ind-item[data-k="market_demand"] .u-ind-val{font-size:13px;font-weight:600;}
...
function renderPage(ctx){
    ...
    if (ctx.mpb){
      if (ctx.mpb.formation) left.appendChild(card(ui(langL,'training'), null, mpbToVisual(ctx.mpb.formation, ui(langL,'training'))));
      ...
      if (ctx.mpb.market_demand){
        var md = cleanRichHTML(String(ctx.mpb.market_demand||""));
        if (md) marketHtml += '<div style="height:10px"></div>' + '<div class="rich-content text-sm">' + esc(ui(langL,'market_demand')) + ' — ' + esc(htmlToText(md)) + '</div>';
      }
    ...
    if (ctx.mpb){
      ...
      function summarizeIndicator(key, val, lang){
        ...
        if (key === "currency") return formatCurrency(s);
        return s;
      }
...
if (remoteRaw) indItems.push({ key:'remote_level', ico:"🏠", lbl:ui(lang2,'remote_level'), raw:remoteRaw });
...
if (autoRaw) indItems.push({ key:'automation_risk', ico:"🤖", lbl:ui(lang2,'automation_risk'), raw:autoRaw });

var curRaw = indGet(ctx.mpb, (ctx.mpb.devise||ctx.mpb.currency||ctx.currency), ["Devise","Currency","Monnaie"]);
var curFmt = formatCurrency(curRaw || "");
...
if (empRaw) indItems.push({ key:'time_to_employability', ico:"⏱️", lbl:ui(lang2,'time_to_employability'), raw:empRaw });
...
if (growRaw) indItems.push({ key:'growth_outlook', ico:"📈", lbl:ui(lang2,'growth_outlook'), raw:growRaw });

var demRaw = indGet(ctx.mpb, ctx.mpb.market_demand, ["Demande du marché","Demande du marche","Market_demand","Demand","Market demand"]);
if (demRaw) indItems.push({ key:'market_demand', ico:"🔥", lbl:ui(lang2,'market_demand'), raw:demRaw });

if (indItems.length){
  var htmlInd = '<div class="u-ind-list">' + indItems.map(function(it){
    return '<div class="u-ind-item" data-k="'+esc(it.key||'')+'"><div class="u-ind-ico">'+esc(it.ico)+'</div><div class="u-ind-txt"><div class="u-ind-lbl">'+esc(it.lbl)+'</div><div class="u-ind-val '+esc(indicatorValueClass(it.key, it.raw, lang2))+'">'+esc(summarizeIndicator(it.key, it.raw, lang2))+'</div></div></div>';
  }).join('') + '</div>';
  right.appendChild(card(ui(lang2,'key_indicators'), 'bg-purple', htmlInd));
}

      // --------------------------------------------
// Salaire (RIGHT) — basé sur les min/max MPB si dispo
// --------------------------------------------
function fmtNum(n){
  if (n==null || n==="" || isNaN(Number(n))) return "";
  try{ return Number(n).toLocaleString(lang2==='fr'?'fr-FR':undefined); }catch(e){ return String(n); }
}
function fmtRange(minv, maxv, cur){
  minv = (minv!=null && minv!=="" ? Number(minv) : null);
  maxv = (maxv!=null && maxv!=="" ? Number(maxv) : null);
  var c = String(cur||"").trim();
  var sym = "";
  if (/EUR/i.test(c)) sym = "€";
  else if (/USD/i.test(c)) sym = "$";
  else if (/GBP/i.test(c)) sym = "£";
  if (!sym && /\(([^)]+)\)/.test(c)) sym = c.match(/\(([^)]+)\)/)[1];
  if (minv!=null && maxv!=null) return fmtNum(minv) + "–" + fmtNum(maxv) + (sym?sym:"");
  if (minv!=null) return fmtNum(minv) + (sym?sym:"");
  if (maxv!=null) return fmtNum(maxv) + (sym?sym:"");
  return "";
}

var curForSalary = formatCurrency(String(ctx.mpb.devise||ctx.mpb.currency||ctx.currency||""));
var sJ = fmtRange(ctx.mpb.junior_min, ctx.mpb.junior_max, curForSalary);
var sM = fmtRange(ctx.mpb.mid_min, ctx.mpb.mid_max, curForSalary);
var sS = fmtRange(ctx.mpb.senior_min, ctx.mpb.senior_max, curForSalary);
var vShare = String(ctx.mpb.variable_share||"").trim();
var hasSalary = !!(sJ || sM || sS || vShare);

if (hasSalary){
  var rows = [];
  if (sJ) rows.push('<div class="u-sal-row"><div class="u-sal-k">'+esc(ui(lang2,'junior'))+'</div><div class="u-sal-v">'+esc(sJ)+'</div></div>');
  if (sM) rows.push('<div class="u-sal-row"><div class="u-sal-k">'+esc(ui(lang2,'mid'))+'</div><div class="u-sal-v">'+esc(sM)+'</div></div>');
  if (sS) rows.push('<div class="u-sal-row"><div class="u-sal-k">'+esc(ui(lang2,'senior'))+'</div><div class="u-sal-v">'+esc(sS)+'</div></div>');
  if (vShare) rows.push('<div class="u-sal-row"><div class="u-sal-k">Variable</div><div class="u-sal-v">'+esc(vShare)+'</div></div>');
  var salHtml = '<div class="u-sal-wrap">'+rows.join('')+'</div>';
  right.appendChild(card(ui(lang2,'salary'), 'bg-mint', salHtml));
}
      ...
      function splitTagsLoose(s){
        if (Array.isArray(s)) {
          ...
          if (arr.length && (tiny / arr.length) >= 0.55) return [norm(arr.join(" "))];
          return arr;
        }
        s = String(s||'');
        s = s.replace(/<[^>]*>/g,' ');
      ...
      function blockContentPillCol(arr){
        ...
        return '<div class="u-pillcol">' + arr.map(function(t){
          t = norm(t);
          if (!t) return '';
          return '<div class="u-pillitem">'+esc(t)+'</div>';
        }).join('') + '</div>';
      }
      ...
      var _toolsCard = skillCard(ui(lang2,'tools_stack'), 'pastel-cyan', tools);
      ...
      if (ctx.mpb.certifications) right.appendChild(card(ui(lang2,'certifications'), 'bg-orange', cleanRichHTML(String(ctx.mpb.certifications))));
      if (ctx.mpb.schools_or_paths) right.appendChild(card(ui(lang2,'schools_paths'), 'bg-blue', listifyIfNeeded(String(ctx.mpb.schools_or_paths))));
      if (ctx.mpb.portfolio_projects) right.appendChild(card(ui(lang2,'portfolio_projects'), 'bg-mint', listifyIfNeeded(String(ctx.mpb.portfolio_projects))));
    }

    // 4) Profil + Évolutions possibles — LEFT (contexte)
    if (ctx.fiche){
      var f2 = ctx.fiche;
      if (f2.profil_recherche) left.appendChild(card(ui(langL,'profile'), '', cleanBlock(f2.profil_recherche)));
      if (f2.evolutions_possibles) left.appendChild(card(ui(langL,'career_path'), '', cleanBlock(f2.evolutions_possibles)));
    }

...
// ---------- Worker (sponsor) ----------
  ...
  function injectCSS(){
    ...
    var css = `
      ...
      .u-ind-item[data-k="market_demand"] .u-ind-val{font-size:14px;font-weight:650;}
...
function renderPage(ctx){
    ...
    if (ctx.mpb){
      ...
      var indItems = [];
      if (ctx.mpb.remote_level) indItems.push({ key:'remote_level', ico:"🏠", lbl:ui(lang2,'remote_level'), val:indVal(ctx.mpb.remote_level) });
      if (ctx.mpb.automation_risk) indItems.push({ key:'automation_risk', ico:"🤖", lbl:ui(lang2,'automation_risk'), val:translateAutomationRisk(indVal(ctx.mpb.automation_risk), lang2) });
      if (ctx.mpb.time_to_employability) indItems.push({ key:'time_to_employability', ico:"⏱️", lbl:ui(lang2,'time_to_employability'), val:indVal(ctx.mpb.time_to_employability) });
      if (ctx.mpb.growth_outlook) indItems.push({ key:'growth_outlook', ico:"📈", lbl:ui(lang2,'growth_outlook'), val:indVal(ctx.mpb.growth_outlook) });
      if (ctx.mpb.market_demand) indItems.push({ key:'market_demand', ico:"🔥", lbl:ui(lang2,'market_demand'), val:indVal(ctx.mpb.market_demand) });
      if (ctx.mpb.devise) indItems.push({ key:'currency', ico:"💰", lbl:ui(lang2,'currency'), val:formatCurrency(indVal(ctx.mpb.devise)) });

      if (indItems.length){
        var htmlInd = '<div class="u-ind-list">' + indItems.map(function(it){
          return '<div class="u-ind-item" data-k="'+esc(it.key||'')+'"><div class="u-ind-ico">'+esc(it.ico)+'</div><div class="u-ind-txt"><div class="u-ind-lbl">'+esc(it.lbl)+'</div><div class="u-ind-val">'+esc(it.val)+'</div></div></div>';
        }).join('') + '</div>';

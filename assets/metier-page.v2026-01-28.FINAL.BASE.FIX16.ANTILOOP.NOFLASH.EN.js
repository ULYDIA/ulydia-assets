(function(){
    ...
    try{
      ...
      ov.innerHTML =
        '<div class="box"><div class="row">' +
          '<div class="spin"></div>' +
          '<div><div class="t">Loading…</div><div class="s">Please wait.</div></div>' +
        '</div></div>';
      (document.documentElement || document.body).appendChild(ov);
    }catch(e){}

      try{
        var noflash = document.getElementById('ulydia_noflash_overlay');
        if (noflash){
          // remove other known overlays if stacked
          var other = ['ul-metier-boot','ul-metier-fallback-loader','ulydia_overlay_loader'];
          for (var i2=0;i2<other.length;i2++){
            var el2 = document.getElementById(other[i2]);
            if (el2) try{ el2.remove(); }catch(e){}
          }
        }
      }catch(e){}
  }
  ...
  function dedupeLoader(){
    try{
      // remove legacy/other overlay loaders that may stack
      var ids = ['ul-metier-boot','ul-metier-fallback-loader','ulydia_overlay_loader','ulydia_noflash_overlay'];
      for (var k=0;k<ids.length;k++){
        var id = ids[k];
        var nodes = document.querySelectorAll('#' + id);
        // keep NOFLASH overlay only if this file mounted it; otherwise remove duplicates
        for (var n=1;n<nodes.length;n++){ try{ nodes[n].remove(); }catch(e){} }
      }

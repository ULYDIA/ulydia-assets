...
/* =========================================================
 ULYDIA — SAFETY FIREWALL (ANTI INJECTOR / ANTI HOOK)
 - Blocks any script injection of other "metier-page*" bundles
 - Prevents legacy appendChild hooks from forcing hard refresh / 404
 - Does NOT block non metier-page scripts
========================================================= */
(function(){
  try{
    if (window.__ULYDIA_FIREWALL__) return;
    window.__ULYDIA_FIREWALL__ = true;

    function isScript(n){ return n && n.tagName === "SCRIPT"; }
    function getSrc(n){ return (n && (n.getAttribute && (n.getAttribute("src")||"")) ) || ""; }
    function norm(s){ return String(s||"").trim(); }

    // Determine "allowed" metier-page script (this bundle)
    var allowed = "";
    try{
      var cs = document.currentScript;
      if (cs && cs.src) allowed = cs.src;
    }catch(e){}
    if (!allowed){
      // fallback: last script in DOM containing "v2.DESIGNFULL"
      var ss = Array.prototype.slice.call(document.querySelectorAll('script[src*="metier-page"]'));
      for (var i=ss.length-1;i>=0;i--){
        var s = ss[i].src || "";
        if (s.indexOf("v2.DESIGNFULL") !== -1) { allowed = s; break; }
      }
    }

    function isBlockedMetierPageScript(node){
      if (!isScript(node)) return false;
      var src = norm(getSrc(node) || node.src || "");
      if (!src) return false;
      if (src.indexOf("metier-page") === -1) return false;
      if (allowed && src === allowed) return false;
      return true;
    }

    // Remove already-injected foreign metier-page scripts (best-effort)
    try{
      var existing = Array.prototype.slice.call(document.querySelectorAll('script[src*="metier-page"]'));
      existing.forEach(function(s){
        var src = norm(s.src);
        if (!src) return;
        if (allowed && src === allowed) return;
        // Keep the one explicitly in page body if it's the same as allowed; otherwise remove
        s.parentNode && s.parentNode.removeChild(s);
        console.warn("[ULYDIA] Firewall removed foreign metier-page script:", src);
      });
    }catch(e){}
    ...
    Node.prototype.appendChild = function(node){
      if (isBlockedMetierPageScript(node)){
        console.warn("[ULYDIA] Firewall blocked injected metier-page script:", norm(getSrc(node) || node.src));
    ...
    Node.prototype.insertBefore = function(node, ref){
      if (isBlockedMetierPageScript(node)){
        console.warn("[ULYDIA] Firewall blocked injected metier-page script (insertBefore):", norm(getSrc(node) || node.src));
        return node;
      }
      return _insertBefore.call(this, node, ref);
    };

    // Also block document.write injection patterns (rare but seen in legacy)
    try{
      var _dw = document.write;
      document.write = function(html){
        if (String(html||"").indexOf("metier-page") !== -1){
          console.warn("[ULYDIA] Firewall blocked document.write containing metier-page");
          return;
        }
...
(function(){
  ...
  function resolveMetierNameFromSlug(slug){
    var arr = window.__ULYDIA_METIERS__;
    if(!Array.isArray(arr)) return "";
    var key = slugify(slug);
    for(var i=0;i<arr.length;i++){
      var it = arr[i]||{};
      var s = it.slug || it.Slug || "";
      var n = it.name || it.nom || it.Name || "";
      if(slugify(s)===key || slugify(n)===key) return String(n||"").trim() || String(it.name||"").trim();
    }
    return "";
  }
var slug = (getParam("metier") || "").trim();
  ...
  var metierName = resolveMetierNameFromSlug(slug);
  var nameKey = slugify(metierName);
  ...
  function findMPB(mpbs, slug, iso){
    ...
    return mpbs.find(function(b){
      ...
      var bKey = slugify(bSlug);
      return bIso === iso && (bKey===slugKey || (nameKey && bKey===nameKey) || bSlug===slug);
    }) || null;
  }
  ...
  (async function main(){
    ...
    var faqs = getFaqArray().filter(function(f){
      ...
      // match on NAME preferred, but accept slug too
      var cand = String(f.metier || f.name || f.job_name || f.job_slug || f.slug || "");
      var fKey = slugify(cand);
      var okIso = !fIso || fIso===iso;
      var okMetier = (fKey===slugKey) || (nameKey && fKey===nameKey);
      return okIso && okMetier;
    });

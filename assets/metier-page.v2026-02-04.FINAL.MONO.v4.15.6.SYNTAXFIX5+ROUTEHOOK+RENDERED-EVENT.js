...
function renderNotAvailable(ctx){
  ...
  ctx = ctx || {}; ctx.lang = ctx.lang || lang;
  ctx.name = ctx.name || (ctx.slug || '');
  ctx.meta_title = ctx.meta_title || ui(lang,'not_available_title');
  ctx.meta_description = ctx.meta_description || (ui(lang,'not_available_msg_prefix') + '\"' + (ctx.iso||'') + '\".');
  applySEO(ctx);
...
function _ensureMetaTag(name, attrKey){
  try{
    var sel = attrKey === 'property' ? 'meta[property="'+name+'"]' : 'meta[name="'+name+'"]';
...
function applySEO(ctx){
  try{
    var lang = (ctx && ctx.lang) ? ctx.lang : "en";
    var name = (ctx && ctx.name) ? ctx.name : "";
    var countryIso = (ctx && ctx.iso) ? ctx.iso : "";
    var metaTitle = (ctx && ctx.meta_title) ? String(ctx.meta_title) : "";
    var metaDesc  = (ctx && ctx.meta_description) ? String(ctx.meta_description) : "";

    if(!metaTitle){
      metaTitle = name ? (name + (countryIso ? " ("+countryIso+")" : "") + " | ULYDIA") : "ULYDIA";
    }
    if(!metaDesc){
      var descSrc = (ctx && ctx.description) ? ctx.description : "";
      metaDesc = _stripHtmlToText(descSrc).slice(0, 200);
    }
    ...
    var md = _ensureMetaTag('description','name'); if(md) md.setAttribute('content', metaDesc || "");
    var ogt = _ensureMetaTag('og:title','property'); if(ogt) ogt.setAttribute('content', metaTitle || "");
    var ogd = _ensureMetaTag('og:description','property'); if(ogd) ogd.setAttribute('content', metaDesc || "");
    var ogu = _ensureMetaTag('og:url','property'); if(ogu) ogu.setAttribute('content', String(location.href||""));
    var twt = _ensureMetaTag('twitter:title','name'); if(twt) twt.setAttribute('content', metaTitle || "");
    var twd = _ensureMetaTag('twitter:description','name'); if(twd) twd.setAttribute('content', metaDesc || "");

    // JSON-LD
    var sector = (ctx && (ctx.sector_label || ctx.sector_name || ctx.sector_slug)) ? (ctx.sector_label || ctx.sector_name || ctx.sector_slug) : "";
    var ld = {
      "@context":"https://schema.org",
      "@type":"WebPage",
      "name": metaTitle || name || "ULYDIA",
      ...
      "about": {
        "@type":"Occupation",
        "name": name || "",
        "description": _stripHtmlToText((ctx && ctx.description) ? ctx.description : ""),
        "occupationalCategory": sector || undefined
      }
    };
    // remove undefined keys (simple)
    if(!ld.about.occupationalCategory) delete ld.about.occupationalCategory;

    var existing = document.getElementById("ulydia-jsonld");
    if(!existing){
      existing = document.createElement("script");
      existing.type = "application/ld+json";
      existing.id = "ulydia-jsonld";
...
function renderPage(ctx){
    ...
    // Build off-DOM first to avoid white-screen if an error happens mid-render
...
// Append columns (FIX: left was missing in v4.8)
    ...
    content.appendChild(renderFAQ(faqs, ctx));
      ...
      try{ console.error('[ULYDIA] renderPage error', e); }catch(_e){}
      try{
        // Show a safe message instead of crashing the whole page
        var c = ensureContentRoot();
        if (c && (!c.childNodes || c.childNodes.length===0)){
          var err = document.createElement('div');
          err.className = 'u-card';
          err.innerHTML = '<div class="u-card-h">⚠️ Error</div><div class="u-card-b">An error occurred while rendering this job page.</div>';
          c.appendChild(err);
        }
      }catch(__e){}
    }
...
// Resolve sponsor (banners) — keep existing working logic
  ...
  // =========================================================
  // ROUTE / FILTER INTEGRATION
  // - Expose a refresh hook for the filter bar (no full reload)
  // - Re-render on history navigation and custom 'ulydia:routechange'
  // =========================================================
  (function(){
    if (window.__ULYDIA_METIER_PAGE_REFRESH__ && typeof window.__ULYDIA_METIER_PAGE_REFRESH__ === "function") return;
    ...
    function keyFromLocation(){
      try{
        ...
        var m = (sp.get("metier") || sp.get("slug") || "").trim().toLowerCase();
        var c = (sp.get("country") || sp.get("iso") || "").trim().toUpperCase();
        return m + "|" + c;
      }catch(e){ return ""; }
    }
    ...
    function refresh(force){
      try{
        ...
        if (!force && k && k === lastKey) return;
        lastKey = k;
        inflight++;
        // main() handles its own loader (content-only)
        Promise.resolve().then(function(){ return main(); }).catch(function(e){ try{ console.error(e); }catch(_){} });
      }catch(e){}
    }

    window.__ULYDIA_METIER_PAGE_REFRESH__ = function(){ refresh(true); };

    // When filters patch pushes a new URL:
    window.addEventListener("ulydia:routechange", function(){ refresh(false); });
...
(function(){
  try{
    ...
    setTimeout(revealFallbackIfNeeded, graceMs);
      ...
      // Clear then append fragment
      while (content.firstChild) content.removeChild(content.firstChild);
      content.appendChild(frag);
      try{ window.dispatchEvent(new CustomEvent('ulydia:rendered', { detail: { slug: (ctx&&ctx.slug)||'', iso: (ctx&&ctx.iso)||'' } })); }catch(e){}
    }catch(e){
      try{ console.error(e); }catch(_){}
      // Keep previous content if possible; otherwise show a safe message
      try{
      ...
      }catch(e){ try{ console.error('[ULYDIA] boot error', e);}catch(_e){} }
    } finally {
      hideContentLoader();
    }
})();
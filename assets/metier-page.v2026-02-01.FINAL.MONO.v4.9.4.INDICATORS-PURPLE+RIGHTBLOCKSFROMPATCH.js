...
function renderPage(ctx){
    ...
    if (ctx.mpb){
      ...
      function indicatorValueClass(key, val, lang){
        ...
        // Always keep the "remote" / "time to employability" / "currency" in the premium purple tone (as in design)
        if (key === "remote_level") return "v-purple";
        if (key === "time_to_employability") return "v-purple";
      ...
      var indItems = [];
      if (ctx.mpb.remote_level) indItems.push({ key:'remote_level', ico:"🏠", lbl:ui(lang2,'remote_level'), raw:indVal(ctx.mpb.remote_level) });
      if (ctx.mpb.automation_risk) indItems.push({ key:'automation_risk', ico:"🤖", lbl:ui(lang2,'automation_risk'), raw:indVal(ctx.mpb.automation_risk) });
      if (ctx.mpb.time_to_employability) indItems.push({ key:'time_to_employability', ico:"⏱️", lbl:ui(lang2,'time_to_employability'), raw:indVal(ctx.mpb.time_to_employability) });
      if (ctx.mpb.growth_outlook) indItems.push({ key:'growth_outlook', ico:"📈", lbl:ui(lang2,'growth_outlook'), raw:indVal(ctx.mpb.growth_outlook) });
      if (ctx.mpb.market_demand) indItems.push({ key:'market_demand', ico:"🔥", lbl:ui(lang2,'market_demand'), raw:indVal(ctx.mpb.market_demand) });
      if (ctx.mpb.devise) indItems.push({ key:'currency', ico:"💰", lbl:ui(lang2,'currency'), raw:indVal(ctx.mpb.devise) });

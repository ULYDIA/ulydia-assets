...
(() => {
  ...
  function removeLegacyFaqQuestionsOnly(root){
    // Remove only true legacy "questions only" blocks.
    // ✅ Never remove the BASE FAQ section (it uses .faq-item/.faq-answer).
    const candidates = [...root.querySelectorAll("section,div")].filter(el => {
      // If it's clearly the BASE FAQ block or any real FAQ container, keep it.
      if (el.querySelector("#faq-title, .faq-item, .faq-question, .faq-answer, [data-ul-faqs]")) return false;
      if (el.querySelector(".ul-faq-card, .ul-faq-item")) return false;

      const t = (el.textContent || "").toLowerCase();
      // Must look like an FAQ title but WITHOUT any answer container
      const looksFaq = t.includes("questions fréquentes") || t.includes("faq");
      if (!looksFaq) return false;

      // If it already contains any known answer UI, keep it
      if (el.querySelector(".ul-faq-a, .ul-faq-answer, details, summary")) return false;

      // Heuristic: legacy blocks are small-ish and list-only
      return (t.length < 1200) && (el.querySelectorAll("li, a, p").length >= 3);
    });
  ...
  function pickFaqsFor(slug, iso){
    // Accept several possible globals (depending on which CMS reader populated them)
    const candidates = [
      window.__ULYDIA_FAQS__,
      window.__ULYDIA_FAQ__,
      window.__ULYDIA_FAQS
    ];
    const faqs = candidates.find(a => Array.isArray(a)) || [];

    const s = String(slug||"").trim().toLowerCase();
    const I = String(iso||"").trim().toUpperCase();

    if (!s) return []; // no slug => can't match reliably

    const out = faqs.filter(x => {
      const m = String(x?.job_slug || x?.metier_slug || x?.metier || x?.slug || x?.job || "").trim().toLowerCase();
      if (!m) return false;

      // tolerant match (exact or contains)
      if (m !== s && !m.includes(s) && !s.includes(m)) return false;

      const xi = String(x?.iso || x?.country_code || x?.country || "").trim().toUpperCase();
      // if ISO provided on item, enforce it

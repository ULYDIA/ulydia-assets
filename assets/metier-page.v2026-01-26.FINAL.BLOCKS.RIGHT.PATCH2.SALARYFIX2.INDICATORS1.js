...
.ul-ind-v.purple{ color:#7c3aed; }
  ...
  function getChip(b, key){
    ...
    // try variants
    return c[key] ?? c[key.toLowerCase()] ?? c[key.replace(/_/g,' ')] ?? null;
  }

  function buildIndicatorsHTML(b){
    // Pull from b.chips when possible (already present in your data model)
    const remote = getChip(b, "Remote_level") || getChip(b, "remote_level") || b.remote_level || b.remote || "";
    const automation = getChip(b, "Automation_risk") || getChip(b, "automation_risk") || b.automation_risk || "";
    const currency = b.currency || b.devise || getChip(b, "Currency") || getChip(b, "Devise") || "EUR (€)";
    const employability = b.employability_delay || b.delai_employabilite || getChip(b, "Employability_delay") || getChip(b, "Délai d'employabilité") || "";
    const growth = b.market_growth || b.croissance_marche || getChip(b, "Market_growth") || getChip(b, "Croissance du marché") || "";
    const demand = b.market_demand || b.demande_marche || getChip(b, "Market_demand") || getChip(b, "Demande du marché") || "";

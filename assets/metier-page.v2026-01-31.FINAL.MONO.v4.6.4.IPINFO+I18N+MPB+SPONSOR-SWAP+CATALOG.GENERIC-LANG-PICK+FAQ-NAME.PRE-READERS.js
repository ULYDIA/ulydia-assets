...
(function(){
  ...
  function readFaqsFromDOM(){
    var out = [];
    var qs = document.querySelectorAll(".js-faq-question");
    if (!qs || !qs.length) return out;
    ...
    for (var i=0;i<qs.length;i++){
      ...
      var aEl = item.querySelector(".js-faq-answer") || item.querySelector(".js-faq-reponse") || item.querySelector(".js-faq-response");
  ...
  function readMPBFromDOM(){
    var out = [];
    // best effort: select items by presence of js-bloc-title or js-bloc-body
    var nodes = document.querySelectorAll(".js-bloc-title, .js-bloc-body");
    if (!nodes || !nodes.length) return out;

    var items = [];
    for (var i=0;i<nodes.length;i++){
      var it = closestItem(nodes[i]);
      if (it && items.indexOf(it) === -1) items.push(it);
    }
    for (var k=0;k<items.length;k++){
      var item = items[k];
      var isoEl = item.querySelector(".js-bloc-iso") || item.querySelector(".js-mpb-iso") || item.querySelector(".js-country-iso");
      var metEl = item.querySelector(".js-bloc-metier") || item.querySelector(".js-mpb-metier") || item.querySelector(".js-job-name");
      var tEl = item.querySelector(".js-bloc-title") || item.querySelector(".js-mpb-title");
      var bEl = item.querySelector(".js-bloc-body") || item.querySelector(".js-mpb-body");
      var oEl = item.querySelector(".js-bloc-order") || item.querySelector(".js-mpb-order");

      var iso = up(isoEl ? isoEl.textContent : "");
      var metier = norm(metEl ? metEl.textContent : "");
      var title = norm(tEl ? tEl.textContent : "");
      var body = bEl ? String(bEl.innerHTML||"").trim() : "";
      var orderRaw = norm(oEl ? oEl.textContent : "");
      var order = orderRaw && !isNaN(parseFloat(orderRaw)) ? parseFloat(orderRaw) : null;

      if (!iso || !metier || (!title && !body)) continue;
      out.push({ iso: iso, metier: metier, title: title, body: body, order: order });
    }
  ...
  try{
    // FAQ
    var fa = window.__ULYDIA_FAQS__ || window.__ulydia_faqs__ || null;
    if (!Array.isArray(fa) || !fa.length){
      var domFaqs = readFaqsFromDOM();
      if (domFaqs.length){
        window.__ULYDIA_FAQS__ = domFaqs;
        window.__ulydia_faqs__ = domFaqs;
      }
    }

    // MPB
    var mpb = window.__ULYDIA_METIER_PAYS_BLOCS__ || window.__ulydia_metier_pays_blocs__ || window.__ULYDIA_MPB__ || window.__ulydia_mpb__ || null;
    if (!Array.isArray(mpb) || !mpb.length){
      var domMpb = readMPBFromDOM();
      if (domMpb.length){
        window.__ULYDIA_METIER_PAYS_BLOCS__ = domMpb;
        window.__ulydia_metier_pays_blocs__ = domMpb;
        window.__ULYDIA_MPB__ = domMpb;
        window.__ulydia_mpb__ = domMpb;
      }
    }
  }catch(e){}
  ...
  function getFAQs(){
    ...
    try{
      var wrap = document.querySelector(".ul-cms-faq-source") || document.querySelector(".ul-cms-faqs-source") || document.querySelector(".ul-cms-faq");
      if (!wrap) return [];
      ...
      for (var i=0;i<items.length;i++){
        ...
        var qEl    = it.querySelector(".js-faq-question");
        var aEl    = it.querySelector(".js-faq-answer");

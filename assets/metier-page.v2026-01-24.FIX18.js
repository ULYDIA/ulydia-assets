...
// DOM fallback (Webflow embed):
    ...
    function faqMatches(item){
      ...
      const s2 = String(
        item.job_slug || item.metier_slug || item.slug || item.metier || item.job || item.title || item.titre || item.name ||
      ...
      const s2Parts = s2Norm.split(/[,;]+/).map(x=>x.trim()).filter(Boolean); // do NOT split on whitespace (metier names contain spaces)
      const want = String(slug||"").trim();
      const wantS = slugify(want);
      const wantN = normSlug(want);
      const ok = s2Parts.some(p => p === want || slugify(p) === wantS || normSlug(p) === wantN || p.startsWith(want + "-") || slugify(p).startsWith(wantS + "-"));
      if (!ok) return false;

...
if (Array.isArray(ctx.mpb.blocks) && ctx.mpb.blocks.length){
  ctx.mpb.blocks.forEach(function(b){
    ...
    // Move MPB Access + Market to the RIGHT column (per latest design request)
    if (ctx.mpb){
      if (ctx.mpb.acces_bloc){
        right.appendChild(card(ui(langFromCtx(ctx),'access'), 'pastel-green', cleanRichHTML(String(ctx.mpb.acces_bloc))));
      }
      if (ctx.mpb.marche_bloc){
        right.appendChild(card(ui(langFromCtx(ctx),'market'), 'pastel-cyan', cleanRichHTML(String(ctx.mpb.marche_bloc))));
      }
    }

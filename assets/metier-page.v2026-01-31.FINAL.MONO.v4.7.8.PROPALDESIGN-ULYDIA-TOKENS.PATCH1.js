...
/* =========================================================
   ULYDIA — PROPAL DESIGN ADAPTER (structure propal, tokens Ulydia)
   - Keeps propal card/header/kpi/chips/salary/faq layouts
   - Uses Ulydia tokens if available (ulydia-ui.v2), fallback to defaults
========================================================= */
:root{
  --primary: var(--ul-primary, #c00102);
  --text: var(--ul-text, #1a1a1a);
  --muted: var(--ul-muted, #6b7280);
  --border: var(--ul-border, #e5e7eb);
  --bg: var(--ul-bg, #ffffff);
  --card: var(--ul-card, #fafafa);

  --accent: var(--ul-accent, #f59e0b);
  --success: var(--ul-success, #10b981);

  --radius-sm: var(--ul-radius-md, 8px);
  --radius-md: var(--ul-radius-lg, 12px);
  --radius-lg: var(--ul-radius-card, 16px);

  --shadow-card: var(--ul-shadow-card, 0 10px 30px rgba(0,0,0,.06));
  --font-family: var(--ul-font, 'Montserrat, system-ui, -apple-system, Segoe UI, Roboto, Arial');
  --font-base: var(--ul-base, 14px);
}

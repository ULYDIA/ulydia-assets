(function(){
  // =========================================================
  // ULYDIA — RIGHT COLUMN I18N PATCH1 (safe / non-breaking)
  // Translates ONLY fixed (non-data) labels created by:
  // metier-page...RIGHT...SALARY...INDICATORS2.js
  //
  // - Salary card: header + row labels + sublabels
  // - Indicators card: header + item labels
  //
  // Does NOT touch numeric values or country names in parentheses.
  // =========================================================

  if (window.__ULYDIA_RIGHT_I18N_PATCH1__) return;
  window.__ULYDIA_RIGHT_I18N_PATCH1__ = true;
  ...
  function patchSalary(lang){
    var t = D[lang] || D.fr;
    var card = document.querySelector("[data-ulydia-salary]");
    if (!card) return;
    ...
    // Row labels (left span in .ul-salary-top)
    ...
    subs.forEach(function(sub){
      ...
      else if (/bonus|int[eé]ressement|participation|profit|incentiv|pr[eé]mie|premi/i.test(s)) setText(sub, t.bonus);
    });
  }

  function patchIndicators(lang){
    var t = D[lang] || D.fr;
    var card = document.querySelector("[data-ulydia-indicators]");
    if (!card) return;
    ...
    items.forEach(function(it){
      ...
      if (!k) return;
      var s = norm(k.textContent);

      // Match by icon (stable)
      ...
      if (icon === "🏠") setText(k, t.remote);
        ...
        // Fallback by French/English keywords
        if (/remote|t[eé]l[eé]trav/i.test(s)) setText(k, t.remote);
        else if (/automat/i.test(s)) setText(k, t.automation);
        else if (/currency|devise|valuta|w[aä]hrung|divisa/i.test(s)) setText(k, t.currency);
        else if (/employability|d[eé]lai|tempo|zeit/i.test(s)) setText(k, t.employability);
        else if (/growth|croissance|crescita|wachstum|crecimiento/i.test(s)) setText(k, t.growth);
        else if (/demand|demande|domanda|nachfrage|demanda/i.test(s)) setText(k, t.demand);
      }
    });
  }
  ...
  function apply(){
    var lang = getLang();
    patchSalary(lang);
    patchIndicators(lang);
  }

  function boot(){
    apply();
    // Retry because right rail may render after base + patches
    var n = 0;
    (function loop(){
      n++;
      apply();
      if (n < 40) return setTimeout(loop, 150);
    })();
  }

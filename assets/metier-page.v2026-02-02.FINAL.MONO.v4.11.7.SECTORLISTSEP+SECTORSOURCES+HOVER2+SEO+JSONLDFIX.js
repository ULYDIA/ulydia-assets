...
/* =========================
   ULYDIA SEO PATCH v4.11.6
   - Inject <title>, meta description, canonical, JSON-LD (Airtable)
   - Non-breaking: runs in parallel; safe if worker unavailable
========================= */
;(function(){
  if (window.__ULYDIA_SEO_PATCH_V4116__) return;
  window.__ULYDIA_SEO_PATCH_V4116__ = true;

  function norm(s){ return String(s||"").replace(/\s+/g," ").trim(); }
  function upper2(s){ return norm(s).toUpperCase(); }
  ...
  function setJSONLD(jsonText){
    jsonText = String(jsonText||"").trim();
    if(!jsonText) return;

    // Accept only JSON-looking payloads
    if(!(jsonText.startsWith("{") || jsonText.startsWith("["))) return;

    // Decode common HTML entities (Airtable rich text / Webflow may escape)
    var decoded = jsonText
      .replace(/&quot;/g,'"')
      .replace(/&#34;/g,'"')
      .replace(/&#39;/g,"'")
      .replace(/&apos;/g,"'")
    ...
    // Validate JSON; if invalid, do not inject (prevents broken SEO + console errors)
    var parsed = null;
    try {
      parsed = JSON.parse(decoded);
    } catch(e){
      try {
        // try a light fix for trailing commas
        var fixed = decoded.replace(/,\s*([}\]])/g, "$1");
        parsed = JSON.parse(fixed);
        decoded = fixed;
      } catch(e2){
        console.warn("[ULYDIA] Invalid JSON-LD (skipped)", e2);
        return;
      }
    }

    // Canonicalize (stable output)
    var safeText = "";
    try { safeText = JSON.stringify(parsed); } catch(e3){ safeText = decoded; }

    // Remove any existing JSON-LD inserted by us
    var old = document.querySelector('script[type="application/ld+json"][data-ulydia="jsonld"]');
    if(old) old.remove();

    var s = document.createElement("script");
    s.type = "application/ld+json";
    s.setAttribute("data-ulydia","jsonld");
  ...
  function applyFromPayload(payload){
    ...
    var metier = payload.metier || payload.job || payload.data || {};
    // Some workers nest fields directly under metier.fields
    var f = metier.fields || metier;

    var metaTitle = pick(f, ["meta_title","Meta_title","metaTitle","title_meta","seo_title","MetaTitle"]);
    var metaDesc  = pick(f, ["meta_description","Meta_description","metaDescription","seo_description","MetaDescription","description_meta"]);
    var jsonld    = pick(f, ["jsonld","JSONLD","Jsonld","ld_json","schema_jsonld","SchemaJSONLD"]);
  ...
  function buildCanonical(slug, iso){
    // Canonical URL format requested: /metier?metier=...&country=FR
    var origin = location.origin;
    var path = location.pathname;
    var p = new URLSearchParams();
    if(slug) p.set("metier", slug);
    if(iso) p.set("country", iso);
    return origin + path + "?" + p.toString();
  }

  async function fetchJSON(url, headers){
    var res = await fetch(url, { method:"GET", headers: headers || {}, credentials:"omit" });
  ...
  function run(){
    try{
      ...
      var slug = norm(usp.get("metier") || usp.get("slug") || "");
      var iso  = upper2(usp.get("country") || usp.get("iso") || "FR");

      // Always ensure canonical exists (even if meta missing)
      setCanonical(buildCanonical(slug, iso));

      var base = norm(window.ULYDIA_WORKER_URL || "");
      if(!base) return;

      base = base.replace(/\/+$/,"");
      var secret = norm(window.ULYDIA_PROXY_SECRET || "");
      var headers = {};
      if(secret) headers["x-ulydia-proxy-secret"] = secret;

      // Worker typically supports /v1/metier-page?slug=...&iso=...
      var url1 = base + "/v1/metier-page?slug=" + encodeURIComponent(slug) + "&iso=" + encodeURIComponent(iso);
      var url2 = base + "/v1/metier-page?metier=" + encodeURIComponent(slug) + "&country=" + encodeURIComponent(iso);

      Promise.resolve()
        .then(function(){ return fetchJSON(url1, headers); })
        .then(function(payload){
          if(payload && payload.ok === true){ applyFromPayload(payload); return true; }
          return fetchJSON(url2, headers).then(function(p2){
            if(p2 && p2.ok === true) applyFromPayload(p2);
            return true;
          });
        })

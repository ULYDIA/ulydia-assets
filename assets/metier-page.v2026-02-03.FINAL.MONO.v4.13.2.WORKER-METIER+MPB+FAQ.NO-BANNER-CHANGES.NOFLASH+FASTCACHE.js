...
function themeForLeftTitle(title){
  ...
  function sectorToLabel(v){
    ...
    if (!v) return "";
    if (typeof v === "string") return stripHtmlToText(v);
...
// ---------- Worker (sponsor) ----------
  ...
  function fetchWorkerPage(metierSlug, iso, lang){
  // Worker payload (sponsor + faq + metier + mpb). Safe: never throw, never block render.
  return new Promise(function(resolve){
    try{
      ...
      var L = (lang ? String(lang) : "").toLowerCase();
      var url = base.replace(/\/+$/,"") + "/v1/metier-page?slug=" + encodeURIComponent(String(metierSlug||"")) +
        "&iso=" + encodeURIComponent(String(iso||"")) + (L ? "&lang=" + encodeURIComponent(L) : "");

      // session cache (speed up repeat visits)
      try{
        var k = "ulydia_wp_v1::" + String(metierSlug||"") + "::" + String(iso||"") + "::" + (L||"");
        var c = sessionStorage.getItem(k);
        if (c){
          var o = JSON.parse(c);
          if (o && o.t && o.p && (Date.now() - Number(o.t)) < 10*60*1000){
            return resolve(o.p);
          }
        }
      }catch(e){}

      // timeout guard (avoid blocking on slow networks)
      var done = false;
      var to = setTimeout(function(){ if (!done){ done = true; resolve(null); } }, 2200);
      ...
      fetch(url, {
        ...
        // store session cache (best-effort)
        try{
          var k2 = "ulydia_wp_v1::" + String(metierSlug||"") + "::" + String(iso||"") + "::" + (L||"");
          sessionStorage.setItem(k2, JSON.stringify({ t: Date.now(), p: j }));
        }catch(e){}
        return resolve(j);
      }).catch(function(){
        if (done) return;
...
function getSponsorFromPayload(j){
  try{
    if (!j) return undefined;
    var s = j.sponsor || j.sponsoring || j.sponsorship || null;
    if (!s && j.pays && j.pays.sponsor) s = j.pays.sponsor;
    // undefined means: no info. null means: explicitly no sponsor (show fallback).
    return (typeof s === "undefined") ? undefined : (s || null);
  }catch(e){
    return undefined;
  }
}

function applyWorkerPayloadToCtx(ctx, j){
  // IMPORTANT: do NOT touch banners/sponsor/country logic here.
  // Banner management is already stable and must remain unchanged.
  try{
    if (!ctx || !j) return;

    // metier / fiche
    if (j.metier && typeof j.metier === "object"){
      var raw = j.metier || {};
      // Normalize to MONO expected schema (prevents missing blocks like competences/profile)
      function pick(){
        for (var i=0;i<arguments.length;i++){
          var v = arguments[i];
          if (v === 0) return 0;
      ...
      var m = {};
      m.slug = pick(raw.slug, raw.metier_slug, raw.metierSlug, raw.job_slug, raw.JOB_SLUG, raw.Slug_clean, ctx.slug);
      m.lang = pick(raw.lang, raw.language, raw.locale, raw.langue, j.lang, ctx.finalLang, ctx.lang, "");
      m.title = pick(raw.title, raw.job_title, raw.jobTitle, raw.Nom, raw.name, raw.Name, ctx.name, ctx.slug);
      m.accroche = String(pick(raw.accroche, raw.accroche_texte, raw.accrocheTexte, "")).trim();
      m.description = pick(raw.description, raw.description_html, raw.descriptionHtml, raw.Description, "");
      m.missions = pick(raw.missions, raw.missions_html, raw.missionsHtml, "");
      m.competences = pick(raw.competences, raw.Competences, raw['Compétences'], raw.skills, raw.skills_html, raw.competences_html, "");
      m.environnements = pick(raw.environnements, raw.environnements_html, raw.work_environment, raw.workEnvironment, raw.work_environment_html, "");
      m.profil_recherche = pick(raw.profil_recherche, raw.profil_recherche_html, raw.profile, raw.profile_html, raw.profil, "");
      m.evolutions_possibles = pick(raw.evolutions_possibles, raw.evolutions_possibles_html, raw.career_path, raw.careerPath, raw.career_path_html, raw.evolutions, raw.evolutions_html, "");
      m.meta_title = pick(raw.meta_title, raw.metaTitle, "");
      m.meta_description = pick(raw.meta_description, raw.metaDescription, "");
      m.heroImageUrl = pick(raw.heroImageUrl, raw.hero_image, raw.heroImage, raw.image, raw.image_url, "");

      ctx.fiche = m; // renderPage consumes ctx.fiche fields directly
      ctx.name = String(m.title || ctx.name || ctx.slug || "").trim();
      ctx.accroche = String(m.accroche || ctx.accroche || "").trim();
      if (j.secteur) ctx.sector = j.secteur;

      // expose for existing pickers/fallback
      try{ window.__ULYDIA_FICHE_METIERS__ = [m]; }catch(e){}
    }

    // MPB / blocs_pays
    if (Array.isArray(j.blocs_pays)){
      try{ window.__ULYDIA_METIER_PAYS_BLOCS__ = j.blocs_pays; }catch(e){}
      try{
        var mpb = findMPB(ctx.slug, ctx.iso);
        if (mpb) ctx.mpb = mpb;
      }catch(e){}
    }

    // FAQ list (renderFAQ already reads from ctx.faq or window.__ULYDIA_FAQS__)
    if (Array.isArray(j.faq)) ctx.faq = j.faq;
  }catch(e){}
}
  // ---------- DOM render ----------
  function ensureRoot(){
...
// Render ASAP if we already have Webflow fallback data
    if (ctx.fiche){
      renderPage(ctx);
    }

    // Then try Worker (Airtable-first) to hydrate: metier + mpb + faq
    fetchWorkerPage(ctx.slug, ctx.iso, langFromCtx(ctx)).then(function(payload){
      var sp = getSponsorFromPayload(payload); // keep existing banner logic: sponsor is resolved AFTER first render
      if (payload) applyWorkerPayloadToCtx(ctx, payload);

      // If still missing fiche after Worker attempt => not available.
      if (!ctx.fiche){
      ...
      // Render with best available data (still sponsor pending -> prevents flash)
      renderPage(ctx);

      // Resolve sponsor (and therefore banners) without changing the banner subsystem.
      if (typeof sp !== "undefined"){
        ctx.sponsor = sp; // null => explicit no sponsor; object => sponsor
        renderPage(ctx);
      }

      log('[ULYDIA] Worker hydrated', {
        metier: ctx.slug,
        iso: ctx.iso,
        lang: langFromCtx(ctx),
        hasFiche: !!ctx.fiche,
        hasMPB: !!ctx.mpb,
        hasFAQ: !!(ctx.faq && ctx.faq.length),
        hasSponsor: !!ctx.sponsor
      });
    });
log('[ULYDIA] v4.13.2 ready', {metier: ctx.slug, iso: ctx.iso, lang: langFromCtx(ctx), hasFiche: !!ctx.fiche, hasMPB: !!ctx.mpb, hasCountry: !!ctx.country});
    }

 ...
 * Version: v2026-02-17.FIX02.DESIGN-MATCH-METIER.PROD
 *
 * Page: /formation?programme=<UUID>&country=<ISO2>
 * Worker: /v1/programme-page?programme=<UUID>&country=<ISO2>
 *
 * Goals:
 * - Reuse the SAME visual language as metier page (same root container if present)
 * - Silent-fail (never break pages)
 * - No external deps
 */
(() => {
  if (window.__ULYDIA_PROGRAMME_PAGE_V2__) return;
  window.__ULYDIA_PROGRAMME_PAGE_V2__ = true;
  ...
  function getRoot() {
    // Prefer the same root as metier page
    return (
  ...
  function workerBase() {
    // Try to reuse existing global base if your metier MONO already sets it
    // Fallback: current origin worker if you host worker under a fixed URL in window.__ULYDIA_WORKER_BASE__
  ...
  function showLoader() {
    ...
    // Fallback minimal overlay if metier loader isn't present
  ...
  function ensureCss() {
    if (document.getElementById("ulydia-programme-v2-css")) return;
    const css = `
      /* Keep it visually close to metier cards: rounded, soft shadow, clean typography */
    ...
    style.id = "ulydia-programme-v2-css";
  ...
  async function main() {
    ...
    // Only run on /formation (best effort)
    // If you prefer stricter, uncomment:
    // if (!location.pathname.includes("/formation")) return;

    if (!programme || !isUuid(programme) || !/^[A-Z]{2}$/.test(country)) return;

    ensureCss();
    showLoader();

    const base = workerBase();
    // If base is empty, try deriving from metier globals
    if (!base) {
      // last resort: if your worker is on same origin with /v1/*
      // you can set window.__ULYDIA_WORKER_BASE__ = "https://....workers.dev" globally.
    }
    ...
    // Strict final language behaviour (worker should already enforce it)

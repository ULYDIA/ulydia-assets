...
*/
window.__METIER_PAGE_BUILD__ = "metier-page v2026-01-25 BASE.FIX5 (stable reveal + sponsor click fix)";
...
(() => {
  ...
  if (window.__ULYDIA_METIER_BOOT_VER__ === "2026-01-25-BASE-FIX5") return;
  window.__ULYDIA_METIER_BOOT_VER__ = "2026-01-25-BASE-FIX5";
  // Keep legacy flag for debugging only (do not block)
  window.__ULYDIA_METIER_BOOT__ = true;

  // =========================================================
  // ULYDIA: Stable data exposure + event bus for patches
  // =========================================================
  (function(){
    if (window.__ULYDIA_METIER_BUS__) return;
    const listeners = [];
    function emit(name, detail){
      try { window.dispatchEvent(new CustomEvent(name, { detail })); } catch(e){}
    }
    window.__ULYDIA_METIER_BUS__ = {
      on(name, fn){ 
        const h = (ev)=>fn(ev.detail);
        window.addEventListener(name, h);
        listeners.push([name,h]);
        return ()=>{ try{ window.removeEventListener(name,h);}catch(e){} };
      },
      emit
    };
    // A single promise that resolves once when the page data is ready
    let _resolve;
    window.__ULYDIA_METIER_READY_PROMISE__ = new Promise(r => { _resolve = r; });
    window.__ULYDIA_METIER_READY_RESOLVE__ = (data) => { try{ _resolve && _resolve(data); }catch(e){} };
  })();
...
const ASSETS_BASE = "https://ulydia-assets.pages.dev/assets";
  ...
  function overlayError(title, err) {
    try {
      ...
      try{ document.documentElement.classList.remove("ul-metier-loading"); document.body.style.opacity="1"; }catch(_){ }
...
function showMessageOverlay(message, sub){
  ...
  // =========================================================
  // Helpers — metiers list validation + clickable banners
  // =========================================================
  function readMetiersData(){
  ...
  async function waitForMetiersData(timeoutMs){
    const t0 = Date.now();
    while (Date.now() - t0 < timeoutMs) {
      const list = readMetiersData();
      if (list && list.length) return list;
      await new Promise(r=>setTimeout(r,50));
    }
    return readMetiersData();
  }
  function renderNotFoundState(lang){
    const isFR = (String(lang||"").toLowerCase().startsWith("fr"));
    const title = isFR ? "Désolé, ce métier n'existe pas." : "Sorry, this job does not exist.";
    const sub   = isFR ? "Veuillez rechercher un autre métier." : "Please search for another job.";
    return renderUnavailableState(title, sub);
  }
  function bindOpenLink(el, href){
    href = String(href||"").trim();
    if (!el || !href) return;
    // Ensure absolute-ish url or mailto/tel is accepted
    if (!/^https?:\/\//i.test(href) && !/^mailto:/i.test(href) && !/^tel:/i.test(href)) {
      // allow relative (same site)
      if (!href.startsWith("/")) href = "https://" + href.replace(/^\/\/+/, "");
    }
    try {
    ...
    el.addEventListener("click", (e)=>{
      // Do not hijack real anchors with proper href
...
function getISO() {
  ...
  function makeClickable(el, url){
    ...
    if (!link) return;
    // If it's an anchor, use href normally
    ...
    // Otherwise make it act like a link
    el.style.cursor = "pointer";
    el.setAttribute("role","link");
    el.setAttribute("tabindex","0");
    el.addEventListener("click", () => window.open(link, "_blank", "noopener"));
    el.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); window.open(link, "_blank", "noopener"); }
    });
  }
     ...
     <div class="space-y-6"><!-- Sponsor Logo Square + CTA -->
       ...
       <div class="space-y-3">
        <div class="kpi-box">
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-remote_level" class="text-sm font-bold" style="color: var(--text);"></p>
         </div>
        </div>
        <div class="kpi-box">
         <div class="kpi-icon" style="background: rgba(16,185,129,0.1);"><span>🤖</span>
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-automation_risk" class="text-sm font-bold" style="color: #10b981;"></p>
         </div>
        </div>
        <div class="kpi-box">
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-currency" class="text-sm font-bold" style="color: var(--text);"></p>
         </div>
        </div>
        <div class="kpi-box">
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-time_to_employability" class="text-sm font-bold" style="color: var(--text);"></p>
         </div>
        </div>
        <div class="kpi-box">
         <div class="kpi-icon" style="background: rgba(99,102,241,0.1);"><span>📈</span>
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-growth_outlook" class="text-sm font-bold" style="color: var(--success);"></p>
         </div>
        </div>
        <div class="kpi-box">
         ...
         <div class="flex-1">
          ...
          <p id="js-chip-market_demand" class="text-sm font-bold" style="color: #ef4444;"></p>
         </div>
        </div>
       </div>
      ...
      <div class="card">
       <div class="card-header gradient-success" style="color: white;">
        <h3 class="section-title text-sm" style="color: white;">💵 Grille salariale <span id="js-salary-country"></span></h3>
       </div>
       <div class="space-y-4">
        <div>
         <div class="flex justify-between items-center mb-2"><span class="text-xs font-semibold" style="color: var(--muted);">💼 Junior</span> <span id="js-sal-junior-range" class="text-xs font-bold" style="color: var(--text);"></span>
         </div>
         <div class="salary-bar-container">
          <div id="js-sal-bar-junior" class="salary-bar-fill gradient-success" style="width: 0%;"></div>
         </div>
         <p class="text-xs mt-1" style="color: var(--muted);">0-2 ans d'expérience</p>
        </div>
        <div>
         <div class="flex justify-between items-center mb-2"><span class="text-xs font-semibold" style="color: var(--muted);">🚀 Confirmé</span> <span id="js-sal-mid-range" class="text-xs font-bold" style="color: var(--text);"></span>
         </div>
         <div class="salary-bar-container">
          <div id="js-sal-bar-mid" class="salary-bar-fill gradient-success" style="width: 0%;"></div>
         </div>
         <p class="text-xs mt-1" style="color: var(--muted);">3-5 ans d'expérience</p>
        </div>
        <div>
         <div class="flex justify-between items-center mb-2"><span class="text-xs font-semibold" style="color: var(--muted);">⭐ Senior</span> <span id="js-sal-senior-range" class="text-xs font-bold" style="color: var(--text);"></span>
         </div>
         <div class="salary-bar-container">
        ...
        <div class="pt-3 mt-3" style="border-top: 1px solid var(--border);">
         ...
         <div class="flex items-center gap-2">
          <div class="flex-1 h-2 rounded-full" style="background: var(--border);"></div><span id="js-sal-variable-share" class="text-xs font-bold" style="color: var(--accent);"></span>
         </div>
         <p id="js-bf-salary_notes" class="text-xs mt-2" style="color: var(--muted);"></p>
        </div>
       </div>
  ...
  <script>
    ...
    async function onConfigChange(config) {
      ...
      makeClickable(sponsorBannerLink, sponsorLink);
      makeClickable(sponsorLogoLink, sponsorLink);
      makeClickable(sponsorCta, sponsorLink);
...
// Metier_Pays_Bloc rich sections in template: hide + clear
  ...
  function forceAnchor(el, {id, dataBanner} = {}) {
    if (!el) return null;
    // If it's already an anchor, ensure basic attrs
    if (String(el.tagName || "").toUpperCase() === "A") {
      if (id && !el.id) el.id = id;
      if (dataBanner) el.setAttribute("data-ul-banner", dataBanner);
      if (!el.getAttribute("target")) el.setAttribute("target","_blank");
      if (!el.getAttribute("rel")) el.setAttribute("rel","noopener");
      // Ensure it's clickable even if inner img overlays
      try { el.style.cursor = "pointer"; } catch(_){}
      return el;
    }

    // Wrap non-anchor elements into an anchor overlay
    const a = document.createElement("a");
    if (id) a.id = id;
    if (dataBanner) a.setAttribute("data-ul-banner", dataBanner);
    a.href = "#";
    a.target = "_blank";
    a.rel = "noopener";
    // Keep layout of the wrapped element
    try {
      a.style.display = (getComputedStyle(el).display === "inline") ? "inline-block" : "block";
      a.style.width = el.style.width || "";
      a.style.maxWidth = el.style.maxWidth || "";
      a.style.height = el.style.height || "";
      a.style.cursor = "pointer";
    } catch(_){}

    // Copy classes so existing CSS keeps working
    try { a.className = (el.className || ""); } catch(_){}
...
// If the template doesn't contain placeholders, inject them (so banners always work)
    ...
    if (!logoA) {
      ...
      logoA = anchor;
        ...
        // Ensure placeholders are clickable anchors (some templates use DIVs)
    wideA = forceAnchor(wideA, { id: "sponsor-banner-link", dataBanner: "wide" });
    logoA = forceAnchor(logoA, { id: "sponsor-logo-link", dataBanner: "square" });
...
function sanitizeSponsorHref(v, fallback){
      // Accept string or {url|href|link} objects; reject accidental functions/native-code strings.
      var href = "";
      try{
        if (v && typeof v === "object") href = String(v.url || v.href || v.link || v.value || "").trim();
        else href = String(v || "").trim();
      }catch(e){ href = ""; }

      // Common broken values observed in the wild
      if (!href || href === "#" ) href = "";
      if (/^javascript:/i.test(href)) href = "";
      if (/^function\s*\w*\s*\(/i.test(href) || /\[native code\]/i.test(href)) href = "";

      // Normalize: if not absolute and not mailto/tel and not relative, prefix https://
      if (href && !/^https?:\/\//i.test(href) && !/^mailto:/i.test(href) && !/^tel:/i.test(href) && !href.startsWith("/")) {
        href = "https://" + href.replace(/^\/\/+/, "");
      }
      return href || String(fallback || "").trim() || "#";
    }
function setLinks(linkUrl){
      var safe = sanitizeSponsorHref(linkUrl, fallbackLink);
      if (!safe || safe === "#") return;
      if (wideA) { wideA.setAttribute("href", safe); wideA.target = "_blank"; wideA.rel="noopener"; }
      if (logoA) { logoA.setAttribute("href", safe); logoA.target = "_blank"; logoA.rel="noopener"; }
      if (ctaBtn) { ctaBtn.setAttribute("href", safe); }
    }
...
const fb = await resolveCountryBanners(iso, payload);
    ...
    const m = payload?.metier || payload?.job || payload?.item || null;
    const f = m ? (m.fieldData || m.fields || m) : {};

    // If job/metier is missing OR URL slug is invalid, show the SAME unavailable message as "not available for this country"
    const hasCore = !!(f && (f.Nom || f.nom || f.title || f.name || f.accroche || f.tagline || f.summary));
    if (!m) {
      const desiredLang = String(payload?.pays?.langue_finale || payload?.pays?.lang || payload?.lang || "fr").trim().toLowerCase();
      const msgByLang = {
        en: "Sorry, this job does not exist.",
        fr: "Désolé, ce métier n'existe pas.",
        es: "Lo sentimos, este oficio no existe.",
        de: "Leider existiert dieser Beruf nicht.",
        it: "Spiacenti, questo lavoro non esiste."
      };
      const hintByLang = {
        en: "Please search another job.",
        fr: "Veuillez rechercher un autre métier.",
        es: "Busca otro oficio.",
        de: "Bitte suche einen anderen Beruf.",
        it: "Cerca un altro lavoro."
      };
      showEmptyState(msgByLang[desiredLang] || msgByLang.fr, hintByLang[desiredLang] || hintByLang.fr);
      return;
    }
    if (!hasCore) {
      const desiredLang = String(payload?.pays?.langue_finale || payload?.pays?.lang || payload?.lang || "fr").trim().toLowerCase();
      ...
      const msg = msgByLang[(desiredLang||"fr").slice(0,2)] || msgByLang.fr;
      try { hideLoaderOverlay(); } catch(_) {}
      renderUnavailableState(msg, "Veuillez sélectionner un autre pays ou rechercher un autre métier.");
      return;
    }
...
function blocMatches(bloc, slug, iso) {
  ...
  const okSlug =
    ...
    (!!refJobName && norm(refJobName).includes(norm(slug))) || // legacy last-resort
  ...
  function getCmsPaysBlocs(){
    const a = window.__ULYDIA_METIER_PAYS_BLOCS__ || window.__ULYDIA_METIER_PAYS_BLOCS__ || null;
  ...
  // ---------------------------------------------------------
  // Capture Metier_Pays_Blocs rendered by Webflow BEFORE we wipe the DOM
  // Stores into window.__ULYDIA_METIER_PAYS_BLOCS__ so patches can work.
  // ---------------------------------------------------------
  function capturePaysBlocsFromDOMOnce(){
    try{
      if (Array.isArray(window.__ULYDIA_METIER_PAYS_BLOCS__) && window.__ULYDIA_METIER_PAYS_BLOCS__.length) return;
      const items = Array.from(document.querySelectorAll(".js-metier-pays-blocs .w-dyn-item, .js-metier-pays-blocs [data-w-id].w-dyn-item"));
      if (!items.length) return;

      function readRichOrText(el){
        if (!el) return "";
        const html = (el.innerHTML || "").trim();
        if (html && html !== "<br>" && html !== "<p><br></p>") return html;
      ...
      for (const it of items){
        const slug = (it.querySelector(".js-bloc-metier")?.textContent || "").trim();
        ...
        // rich-text fields
        const richClasses = [
          "formation","acces","salaire","marche",
          "education_level_local","top_fields","certifications","schools_or_paths",
          "equivalences_reconversion","entry_routes","first_job_titles","typical_employers",
          "portfolio_projects","skills_must_have","soft_skills","tools_stack",
          "time_to_employability","hiring_sectors","degrees_examples","growth_outlook","market_demand",
          "salary_notes","education_level"
        ];
        for (const k of richClasses){
          const el = it.querySelector(".js-bf-"+k);
          const v = readRichOrText(el);
          if (v) f[k] = v;
        }

        // chips / plain text
        const chipClasses = ["remote_level","automation_risk","currency"];
        for (const k of chipClasses){
          const v = (it.querySelector(".js-chip-"+k)?.textContent || "").trim();
          if (v) f[k] = v;
        }

        // salaries
        const salKeys = [
          "junior-min","junior-max","mid-min","mid-max","senior-min","senior-max","variable-share"
        ];
        for (const sk of salKeys){
          const cls = ".js-sal-"+sk;
          const v = (it.querySelector(cls)?.textContent || "").trim();
          if (v) f[sk.replace(/-/g,"_")] = v;
        }

        // meta
        const status = (it.querySelector(".js-statut-generation")?.textContent || "").trim();
        if (status) f.status_generation = status;

        out.push({ slug, iso, fieldData: f });
      }

      if (out.length) window.__ULYDIA_METIER_PAYS_BLOCS__ = out;
    } catch(e){
      // silent
    }
  }
  // ---------------------------------------------------------
  // DOM fallback: read Metier_Pays_Blocs collection items rendered in Webflow
  // Expects within each .w-dyn-item:
  //   .js-bloc-metier  (slug)
  //   .js-bloc-iso     (ISO2)
  //   plus fields with classes: js-bf-*, js-chip-*, js-sal-*
  // ---------------------------------------------------------
  function readRichOrText(el){
    if (!el) return "";
    // if this is a rich text element, keep HTML
    const isRich = el.classList && (el.classList.contains("w-richtext") || el.querySelector("p,ul,ol,blockquote,h1,h2,h3,h4,h5,h6"));
    return isRich ? (el.innerHTML || "") : (el.textContent || "");
  }

  function findBlocFromDOM(slug, iso){
    try {
      slug = String(slug || "").trim();
      iso  = String(iso || "").trim().toUpperCase();
      if (!slug || !iso) return null;

      const slugNodes = document.querySelectorAll(".js-bloc-metier");
      if (!slugNodes || !slugNodes.length) return null;

      for (const n of slugNodes) {
        const item = n.closest(".w-dyn-item") || n.closest(".w-dyn-items") || n.parentElement;
        if (!item) continue;

        const s = String(n.textContent || "").trim();
        if (s !== slug) continue;

        const isoEl = item.querySelector(".js-bloc-iso");
        const isoTxt = String(isoEl ? isoEl.textContent : "").trim().toUpperCase();
        if (isoTxt !== iso) continue;

        // Build blocFields with the exact keys expected by patches
        const fieldsMap = {
          // LEFT blocks (rich text)
          formation_bloc: ".js-bf-formation",
          acces_bloc: ".js-bf-acces",
          salaire_bloc: ".js-bf-salaire",
          marche_bloc: ".js-bf-marche",
          ...
          // RIGHT KPI chips (plain text)
          remote_level: ".js-chip-remote_level",
          automation_risk: ".js-chip-automation_risk",
          currency: ".js-chip-currency",

          // Salary numbers (plain text / numbers)
          salary_junior_min: ".js-sal-junior-min",
          salary_junior_max: ".js-sal-junior-max",
          salary_mid_min: ".js-sal-mid-min",
          salary_mid_max: ".js-sal-mid-max",
          salary_senior_min: ".js-sal-senior-min",
          salary_senior_max: ".js-sal-senior-max",
          salary_variable_share: ".js-sal-variable-share",

          // Statut (plain text)
          status_generation: ".js-statut-generation"
        };

        const blocFields = {};
        Object.keys(fieldsMap).forEach((k) => {
          const sel = fieldsMap[k];
          const el = item.querySelector(sel);
          const v  = readRichOrText(el);
          if (v && String(v).trim()) blocFields[k] = v;
        });

        return {
          fields: blocFields,
          raw: { slug: slug, iso: iso, from: "dom" }
        };
      }
      } catch(e) {
      // silent
  ...
  function findBloc(payload, slug, iso) {
    // Priority:
    // 1) CMS globals (Metier_pays_blocs) if present
    // 2) payload fields from Worker
    const cms = getCmsPaysBlocs();
    if (cms && cms.length) {
      for (const it of cms) {
    ...
    if (!candidates) {
      const dom = findBlocFromDOM(slug, iso);
      return dom ? { fieldData: dom.fields || {}, slug: slug, iso: iso, from: "dom" } : null;
    }
...
function showPaysCardsByHeader() {
  ...
  function getPaysCardByHeader(headerText){
    const headers = $all(".card-header .section-title");
    for (const h of headers){
      const t = (h.textContent || "").trim();
      if (!t) continue;
      if (t.includes(headerText)) {
        const card = h.closest(".card") || h.closest("section") || h.parentElement;
        if (!card) return null;
        const body = card.querySelector(":scope > .space-y-3, :scope > .space-y-4, :scope > .p-6, :scope > .p-5, :scope > div:not(.card-header)") || card.children[1] || null;
        return { card, headerEl: h, body };
      }
    }
    return null;
  }

  function hidePaysCardByHeader(headerText){
    const o = getPaysCardByHeader(headerText);
    if (o && o.card) o.card.style.display = "none";
  }

  function setPaysCardRichByHeader(headerText, htmlOrText){
    const o = getPaysCardByHeader(headerText);
    if (!o || !o.body) return;
    if (isEmptyRich(htmlOrText)) { hidePaysCardByHeader(headerText); return; }
    o.card.style.display = "";
    const s = String(htmlOrText);
    if (/<[a-z][\s\S]*>/i.test(s)) o.body.innerHTML = s;
    else o.body.innerHTML = `<p>${formatInlineBold(s).replace(/\n+/g, "<br/>")}</p>`;
  }

  function applyKpis(labelValuePairs){
    // Find KPI boxes by their small label text, hide those without value.
    const o = getPaysCardByHeader("Indicateurs clés");
    if (!o || !o.card) return false;
    const boxes = Array.from(o.card.querySelectorAll(".kpi-box"));
    let shown = 0;

    function setOne(labelContains, value){
      const v = String(value || "").trim();
      // locate box where label matches
      const box = boxes.find(b => {
        const label = (b.querySelector("p.text-xs")?.textContent || b.querySelector("p")?.textContent || "").trim();
        return label.includes(labelContains);
      });
      if (!box) return;
      if (!v) { box.style.display = "none"; return; }
      const valEl = box.querySelector("p.text-sm, p.font-bold, .kpi-value") || box.querySelectorAll("p")[1];
      if (valEl) valEl.textContent = v;
      box.style.display = "";
      shown++;
    }

    labelValuePairs.forEach(([lab, val]) => setOne(lab, val));
    // hide boxes not explicitly set but empty
    boxes.forEach(b => {
      if (b.style.display === "none") return;
      const valEl = b.querySelector("p.text-sm, p.font-bold, .kpi-value") || b.querySelectorAll("p")[1];
      const cur = (valEl?.textContent || "").trim();
      if (!cur) b.style.display = "none";
    });
    shown = boxes.filter(b => b.style.display !== "none").length;
    if (!shown) o.card.style.display = "none";
    return !!shown;
  }

  function formatSalaryRange(min, max, currency){
    if (!min && !max) return "";
    const sym = String(currency || "").toUpperCase().includes("USD") ? "$" :
                String(currency || "").toUpperCase().includes("GBP") ? "£" :
                "€";
    const fmt = n => {
      const k = Math.round(Number(n)/1000);
      return isFinite(k) ? String(k) + "K" : "";
    };
    const a = min ? fmt(min) : "";
    const b = max ? fmt(max) : "";
    const mid = (a && b) ? (a + "-" + b) : (a || b);
    return mid ? (mid + sym) : "";
  }

  function applySalaryGrid(sal, currency, iso){
    const o = getPaysCardByHeader("Grille salariale");
    if (!o || !o.card) return false;

    const junior = formatSalaryRange(sal?.junior?.min, sal?.junior?.max, currency);
    const mid    = formatSalaryRange(sal?.mid?.min, sal?.mid?.max, currency);
    const senior = formatSalaryRange(sal?.senior?.min, sal?.senior?.max, currency);

    const rows = Array.from(o.card.querySelectorAll(".space-y-4 > div"));
    function setRow(labelContains, rangeStr){
      const row = rows.find(r => ((r.querySelector("span")?.textContent||"").includes(labelContains)));
      if (!row) return false;
      const spans = row.querySelectorAll("span");
      const rangeEl = spans && spans.length ? spans[spans.length-1] : null;
      if (!rangeStr) { row.style.display="none"; return false; }
      if (rangeEl) rangeEl.textContent = rangeStr;
      row.style.display="";
      return true;
    }

    const ok1 = setRow("Junior", junior);
    const ok2 = setRow("Confirm", mid) || setRow("Confirmé", mid) || setRow("Confirm", mid);
    const ok3 = setRow("Senior", senior);

    // Update header with ISO if present
    try{
      if (iso && o.headerEl) {
        const t = o.headerEl.textContent || "";
        o.headerEl.textContent = t.replace(/\([^\)]*\)/, "(" + iso + ")"); // (France) -> (FR)
      }
    }catch(_){}

    const any = [ok1, ok2, ok3].some(Boolean);
    if (!any) o.card.style.display="none";
    return any;
  }
...
function applyPaysBloc(payload, slug, iso) {
    ...
    // ---------------------------------------------------------
    // 1) If bloc comes from CMS reader schema:
    //    { iso, metier, sections:[{key,label,type,value}], chips:{...}, salary:{...} }
    // ---------------------------------------------------------
    if (Array.isArray(b.sections)) {
      // helper: get section by key
      const byKey = {};
      b.sections.forEach(s => { if (s && s.key) byKey[String(s.key)] = s; });

      // These 4 map to left-column rich cards by ID
      if (byKey.acces_bloc && !isEmptyRich(byKey.acces_bloc.value)) { setRich("acces-title", byKey.acces_bloc.value); showCardForId("acces-title"); }
      if (byKey.marche_bloc && !isEmptyRich(byKey.marche_bloc.value)) { setRich("marche-title", byKey.marche_bloc.value); showCardForId("marche-title"); }
      if (byKey.salaire_bloc && !isEmptyRich(byKey.salaire_bloc.value)) { setRich("salaire-title", byKey.salaire_bloc.value); showCardForId("salaire-title"); }
      if (byKey.formation_bloc && !isEmptyRich(byKey.formation_bloc.value)) { setRich("formation-title", byKey.formation_bloc.value); showCardForId("formation-title"); }

      // Map CMS sections -> right-column “pays cards” (by header text)
      const map = [
        ["Skills_must_have", "Compétences incontournables"],
        ["Soft_skills", "Soft Skills essentielles"],
        ["Tools_stack", "Stack Technique Populaire"],
        ["Certifications", "Certifications utiles"],
        ["Schools_or_paths", "Écoles & Parcours recommandés"],
        ["Portfolio_projects", "Projets Portfolio essentiels"]
      ];
      map.forEach(([k, header]) => {
        const s = byKey[k];
        if (s && !isEmptyRich(s.value)) setPaysCardRichByHeader(header, s.value);
        else hidePaysCardByHeader(header);
      });

      // KPIs (chips)
      const chips = b.chips || {};
      const kpiValues = [
        ["Télétravail", chips.Remote_level],
        ["Risque d’automatisation", chips.Automation_risk],
        ["Statut", chips.Status_generation],
        ["Monnaie", chips.Currency]
      ];
      const anyKpi = applyKpis(kpiValues);
      if (!anyKpi) hidePaysCardByHeader("Indicateurs clés");

      // Salary grid
      const sal = b.salary || {};
      const anySalary = applySalaryGrid(sal, chips.Currency || null, iso);
      if (!anySalary) hidePaysCardByHeader("Grille salariale");

      return true;
    }

    // ---------------------------------------------------------
    // 2) Worker payload schema (legacy) — keep existing behavior
    // ---------------------------------------------------------
    if (!isEmptyRich(b.acces_bloc)) { setRich("acces-title", b.acces_bloc); showCardForId("acces-title"); }
    if (!isEmptyRich(b.marche_bloc)) { setRich("marche-title", b.marche_bloc); showCardForId("marche-title"); }
    if (!isEmptyRich(b.salaire_bloc)) { setRich("salaire-title", b.salaire_bloc); showCardForId("salaire-title"); }
    if (!isEmptyRich(b.formation_bloc)) { setRich("formation-title", b.formation_bloc); showCardForId("formation-title"); }
  ...
  async function boot() {
    ...
    * {
    ...
    // Validate slug existence (to differentiate: not-found vs not-available-in-country)
    const langHint = (typeof getLangFromURL==="function" ? getLangFromURL() : null) || "fr";
    const metiersList = await waitForMetiersData(2000);
    const exists = metiersList.some(x => normalizeSlug(x.slug || x?.fieldData?.slug) === normalizeSlug(slug));
    if (!exists) {
      renderNotFoundState(langHint);
      return;
    }
    ...
    let payload = await fetchMetierPayload({ iso, slug });

    // Fallback: if API returns no metier for this slug/iso but the metier exists in metiersData,
    // render a partial page instead of the "not available" state.
    if (!payload || !payload.metier) {
      try {
        ...
        const metiers = readJSON("metiersData");
        const m = (metiers || []).find(x => String(x?.slug || x?.fieldData?.slug || x?.fieldData?.Slug || "").trim() === slug);

        const countries = readJSON("countriesData").concat(readJSON("countriesData2") || []).concat(readJSON("countriesData3") || []);
        const c = (countries || []).find(x => String(x?.iso || x?.fieldData?.iso || x?.fieldData?.ISO || x?.fieldData?.code_iso || "").trim().toUpperCase() === String(iso).toUpperCase());

        const metierName = String(
          m?.fieldData?.Nom || m?.fieldData?.nom || m?.fieldData?.name || m?.name || m?.title || ""
        ).trim();

        const countryLang = String(
          c?.langue_finale || c?.fieldData?.langue_finale || c?.fieldData?.langue || c?.fieldData?.lang || c?.lang || ""
        ).trim().toLowerCase();

        const banners = {
          wide: c?.banners?.wide || c?.fieldData?.banniere_large || c?.fieldData?.banner_wide || c?.fieldData?.bannerWide || "",
          square: c?.banners?.square || c?.fieldData?.banniere_carre || c?.fieldData?.banner_square || c?.fieldData?.bannerSquare || ""
        };

        const faqs = (window.__ULYDIA_FAQS__ || []).filter(x =>
          slugify(x?.metier) === slug && (!x?.iso || String(x.iso).toUpperCase() === String(iso).toUpperCase())
        );
        ...
        if (metierName) {
          payload = {
            iso,
            slug,
            lang: countryLang || String(window.__ULYDIA_DEFAULT_LANG__ || "fr"),
            metier: { slug, name: metierName },
            pays: { iso, langue_finale: countryLang || "", banners },
            sponsor: null,
            blocs: null,
            faqs
          };
          console.warn("[metier-page] fallback payload used (metier exists but API returned none)", { iso, slug });
        }
      } catch (e) {
        console.warn("[metier-page] fallback payload failed", e);
      }
    }
...
setRich("description-title", f.description || "");
    ...
    // Pays bloc (STRICT match) — expose ONLY (render happens in PATCHES)
    hidePaysCardsByHeader(); // keep hidden until a patch explicitly shows a block

    const blocRaw = findBloc(payload || {}, slug, iso);
    const bloc = blocRaw ? (blocRaw.fieldData || blocRaw.fields || blocRaw) : null;

    // Stable globals for PATCHES
    window.__ULYDIA_BLOC_RAW__ = blocRaw || null;
    window.__ULYDIA_BLOC__ = bloc || null;

    // Stable context for PATCHES
    window.__ULYDIA_METIER_PAGE_CTX__ = {
      slug, iso, lang,
      payload: payload || null,
      metier: m || null,
      bloc: blocRaw || null,
      blocFields: bloc || null
    };

    // Signal ready (PATCHES can listen)
    window.__ULYDIA_METIER_PAGE_READY__ = true;
    try { window.__ULYDIA_METIER_READY_RESOLVE__ && window.__ULYDIA_METIER_READY_RESOLVE__(window.__ULYDIA_METIER_PAGE_CTX__); } catch(e) {}
    try { window.__ULYDIA_METIER_BUS__ && window.__ULYDIA_METIER_BUS__.emit("ULYDIA:METIER_READY", window.__ULYDIA_METIER_PAGE_CTX__); } catch(e) {}
    try { window.__ULYDIA_METIER_BUS__ && window.__ULYDIA_METIER_BUS__.emit("ULYDIA:METIER_BLOC_READY", { slug, iso, lang, bloc: blocRaw || null, blocFields: bloc || null }); } catch(e) {}


    // FAQ
    // Priority:
    // 1) CMS reader globals (window.__ULYDIA_FAQS__) if present
    // 2) payload.faq/faqs
    // 3) Worker endpoints fallback (then client-side filter)
    function getCmsFaqs(){
      const a = window.__ULYDIA_FAQS__ || window.__ULYDIA_FAQS__ || null;
      return Array.isArray(a) ? a : [];
    }

    async function waitForCmsFaqs(maxMs){
      const t0 = Date.now();
      // If the CMS wrappers exist, give them a moment to populate globals
      const hasWrap = !!(document.querySelector(".ul-cms-faq-source") || document.querySelector(".ul-cms-source"));
      if (!hasWrap) return getCmsFaqs();
      while (Date.now() - t0 < maxMs) {
        const list = getCmsFaqs();
    ...
    // Start with CMS if available (or soon-to-be available)
    let faqList = await waitForCmsFaqs(1200);
    if (!faqList || faqList.length === 0) {
      faqList = payload?.faq || payload?.faqs || payload?.FAQ || null;
    }
    ...
    // ✅ Safety: always filter client-side by (slug, iso, lang) to avoid leaking irrelevant FAQs
    function faqMatches(item){
      ...
      // Require a job/metier match.
      const s2 = String(
        item.job_slug || item.metier_slug || item.slug || item.metier || item.job || item.metier_ref || ""
      ).trim();
      if (!s2) return false;
      ...
      // - full URL
      // - "slug|FR"
      // - "Contrôleur aérien" (Name) -> slugify
      ...
      if (!(s2Norm === slug || slugify(s2Norm) === slug)) return false;

      // If iso/lang are present, enforce them
    ...
    // Done
    // Reveal page AFTER patches had a chance to mutate the DOM (avoids flicker)

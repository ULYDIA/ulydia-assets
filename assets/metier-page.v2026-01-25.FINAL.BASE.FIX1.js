...
*/
window.__METIER_PAGE_BUILD__ = "metier-page v2026-01-23-1830 (strict payload+catalog b1->wide b2->square + overlay)";
...
const ASSETS_BASE = "https://ulydia-assets.pages.dev/assets";
  ...
  function overlayError(title, err) {
    try {
      ...
      try{ try{ const r=ensureRoot(); r.style.visibility='visible'; r.style.opacity='1'; }catch(e){}
    // signal ready for patches
    try{ window.dispatchEvent(new CustomEvent('ulydia:metier:rendered',{detail:{slug,iso,lang}})); }catch(e){}
    hideLoaderOverlay(); }catch(_){}
...
function showMessageOverlay(message, sub){
  ...
  function try{ const r=ensureRoot(); r.style.visibility='visible'; r.style.opacity='1'; }catch(e){}
    // signal ready for patches
    try{ window.dispatchEvent(new CustomEvent('ulydia:metier:rendered',{detail:{slug,iso,lang}})); }catch(e){}
    hideLoaderOverlay(){
  ...
  function ensureRoot() {
    ...
    if (!root.__ul_init_hidden){
      root.__ul_init_hidden = true;
      try{
   ...
   <main class="max-w-[1200px] mx-auto px-6 py-10">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8"><!-- Main Content Column (2/3) -->
     <div class="lg:col-span-2 space-y-8"><!-- Vue d'ensemble / Description -->
      ...
      <div class="card" id="faq-card" style="display:none">
       ...
       <div class="space-y-3" data-ul-faqs-wrap><!-- filled dynamically --></div>
...
if (desired && CMS_LANGS.length && !CMS_LANGS.includes(desired)) {
  try {
    try{ const r=ensureRoot(); r.style.visibility='visible'; r.style.opacity='1'; }catch(e){}
    // signal ready for patches
    try{ window.dispatchEvent(new CustomEvent('ulydia:metier:rendered',{detail:{slug,iso,lang}})); }catch(e){}
    hideLoaderOverlay();
...
setRich("description-title", f.description || "");
    ...
    function faqMatches(item){
      ...
      // Accept variations (full URL or "slug|FR")
      const s2Norm = s2.replace(/^https?:\/\/[^/]+\//, "").split(/[?#]/)[0].split("|")[0].trim();
      const s2Slug = slugify(s2Norm);
      if (s2Norm !== slug && s2Slug !== slug) return false;

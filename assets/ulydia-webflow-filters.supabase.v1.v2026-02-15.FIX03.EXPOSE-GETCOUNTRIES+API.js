...
(function(){
  ...
  // ---- Public API for MONO packs that still call getCountries/getSectors/etc. ----
  // Keep it stable + backward compatible.
  window.ULYDIA_FILTERS_API = {
    getCountries: async () => { if(!COUNTRIES.length) await loadFilters(); return COUNTRIES.slice(); },
    getSectors: async () => { if(!SECTORS.length) await loadFilters(); return SECTORS.slice(); },
    getAvailableJobs: async (iso) => {
      const x = (iso || currentISO || "").toUpperCase();
      if(!x) return [];
      if(currentISO !== x || !JOBS_AVAILABLE.length){
        await onCountryChange(x);
      }
      return JOBS_AVAILABLE.slice();
    },
    setCountry: async (iso) => { await onCountryChange((iso||"").toUpperCase()); },
    state: () => ({ currentISO, currentSector, countries: COUNTRIES.length, sectors: SECTORS.length, jobs: JOBS_AVAILABLE.length })
  };

  // Legacy globals used by older MONO versions
  window.getCountries = window.ULYDIA_FILTERS_API.getCountries;
  window.getSectors = window.ULYDIA_FILTERS_API.getSectors;
  window.getAvailableJobs = window.ULYDIA_FILTERS_API.getAvailableJobs;

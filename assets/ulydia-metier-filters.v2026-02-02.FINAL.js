 ...
 * File: ulydia-metier-filters.v2026-02-02.FINAL.js
 * Purpose:
 *  - Add a top-of-page selection UI on /metier (Option A: 100% JS rendered page).
 *  - NO Webflow API calls. Reads from existing "exports CMS cachés" (window.__ULYDIA_*__)
 *  - Filters: Country -> Sector -> Job, then navigates to /metier?metier=...&country=...
 *
 * How to include:
 *  <script src="https://ulydia-assets.pages.dev/assets/ulydia-metier-filters.v2026-02-02.FINAL.js" defer></script>
 *
 * Expected data (any of these names; the script autodetects):
 *  - Countries: window.__ULYDIA_COUNTRIES__ | window.__ULYDIA_PAYS__ | window.__ULYDIA_COUNTRY_LIST__
 *      item fields accepted: { iso, code, alpha2, name, name_fr, name_en, default_lang, lang }
 *  - Sectors: window.__ULYDIA_SECTORS__ | window.__ULYDIA_SECTEURS__ | window.__ULYDIA_SECTEURS_ACTIVITE__
 *      item fields accepted: { id, _id, slug, name, name_fr, name_en, name_de, name_es, name_it }
 *  - Jobs/Metiers: window.__ULYDIA_METIERS__ | window.__ULYDIA_JOBS__ | window.__ULYDIA_METIER_LIST__
 *      item fields accepted:
 *        { slug, name, name_fr..., country_iso|country|iso, sector_slug|sector|sector_id|secteur_activite|sector_activity }
 *
 * Note:
 *  - "Liaison" Sector <-> Job is handled via either sector slug OR sector id (Reference field).
 *  - If your Job item uses a Webflow reference object, keep either:
 *      sector_slug: "finance"  OR  sector_id: "xxxxxxxx"
 *    (the script will try multiple keys).
 */
(() => {
  // =========================================================
  // Guard
  // =========================================================
  if (window.__ULYDIA_METIER_FILTERS__) return;
  window.__ULYDIA_METIER_FILTERS__ = true;
  ...
  function guessSectorKey(m) {
    ...
    // sometimes reference objects are embedded
  ...
  function guessLangFromCountry(countryIso, countries, qsLang) {
    ...
    // if your main metier page already sets a global lang, use it
  ...
  // =========================================================
  // Data detection (Option A exports)
  // =========================================================
  ...
  if (!metiers.length) {
    console.warn("[ULYDIA:filters] No metiers/jobs array found on window.__ULYDIA_*__. Filter bar disabled.");
    return;
  }
  ...
  // =========================================================
  // Styles (minimal + token-friendly)
  // =========================================================
  function injectStyles() {
    if (document.getElementById("ulydia-metier-filters-style")) return;

    // Try to reuse CSS variables if your UI defines them
    ...
    st.textContent = `
      ...
      .ulydia-filter-label {
        ...
        font-size: 12px;
        margin: 0 0 6px;
        color: var(--ulydia-muted, rgba(0,0,0,.65));
      }
  ...
  function render(mount) {
    const qs = getQS();
    const lang = guessLangFromCountry(qs.country, countries, qs.lang);
    const t = UI_TXT[lang] || UI_TXT.fr;

    const sectorIdx = buildIndexes(sectors);

    // If sector is not in QS but metier is selected, infer sector from metier
    let inferredSector = qs.sector;
    if (!inferredSector && qs.metier) {
      const m0 = metiers.find(m => guessMetierSlug(m) === qs.metier);
    ...
    const metiersByCountry = qs.country
      ? metiers.filter(m => guessCountryIsoFromMetier(m) === qs.country)
      : metiers.slice();

    // Determine available sectors for that country (via metiers linkage)
    ...
    metiersByCountry.forEach(m => {
      ...
      // also accept direct sector_slug on metier if sector collection isn't perfectly mapped
      const directSlug = norm(pickField(m, ["sector_slug", "secteur_slug", "sector"]));
      if (directSlug) availSectorSlugs.add(directSlug);
    });

    const sectorsFiltered = (qs.country && sectors.length)
      ? sectors.filter(s => availSectorSlugs.has(norm(pickField(s, ["slug"]))))
      : sectors.slice();

    // Filter metiers by sector
    const metiersBySector = inferredSector
      ? metiersByCountry.filter(m => {
          const key = guessSectorKey(m);
          if (!key) return false;
          if (key.type === "slug") return norm(key.value) === norm(inferredSector);
          // if metier stores id but QS stores slug, resolve
          const sec = resolveSectorForMetier(m, sectorIdx);
          const slug = sec ? norm(pickField(sec, ["slug"])) : "";
    ...
    // Options builders
    const countryOptions = (countries.length ? countries : dedupeCountriesFromMetiers(metiers))
      .map(c => {
        const iso = guessIso(c);
        const label = pickLocalized(c, "name", lang) || pickLocalized(c, "title", lang) || iso;
    ...
    const metierOptions = metiersBySector
      .map(m => {
        ...
        const label = pickLocalized(m, "name", lang) || pickLocalized(m, "title", lang) || slug;
        return { value: slug, label };
      })
      .filter(x => x.value)
      .sort((a,b) => a.label.localeCompare(b.label, undefined, { sensitivity: "base" }));
    ...
    const sectorDisabled  = !qs.country;
    const metierDisabled  = !qs.country;

    mount.innerHTML = `
      <div class="ulydia-filterbar">
        <div class="ulydia-filter">
          <label class="ulydia-filter-label">${escHtml(t.country)}</label>
          <select id="ulydia-filter-country" class="ulydia-filter-select" ${countryDisabled ? "disabled":""}>
        ...
        <div class="ulydia-filter">
          ...
          <select id="ulydia-filter-sector" class="ulydia-filter-select" ${sectorDisabled ? "disabled":""}>
            <option value="">${escHtml(t.choose)}</option>
            ${sectorOptions.map(o => `
        ...
        <div class="ulydia-filter">
          ...
          <select id="ulydia-filter-metier" class="ulydia-filter-select" ${metierDisabled ? "disabled":""}>
            <option value="">${escHtml(t.choose)}</option>
            ${metierOptions.map(o => `
    ...
    wireEvents(mount);
    log("rendered", { lang, qs, inferredSector, countryOptions: countryOptions.length, sectorOptions: sectorOptions.length, metierOptions: metierOptions.length });
  }
  ...
  function wireEvents(mount) {
    ...
    elCountry.addEventListener("change", () => {
      // Changing country invalidates sector & metier
      setQS({ country: upper(elCountry.value || ""), sector: "", metier: "" });
    });

    elSector.addEventListener("change", () => {
      // Changing sector invalidates metier
      setQS({ sector: norm(elSector.value || ""), metier: "" });
    });

    elMetier.addEventListener("change", () => {
      // Set metier only (keep others). If country missing, do nothing.
    ...
    elGo.addEventListener("click", () => {
      ...
      // we keep sector in query (optional) for better back-navigation

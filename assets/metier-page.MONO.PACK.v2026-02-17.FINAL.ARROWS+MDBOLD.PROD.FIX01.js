...
try{ window.__ULYDIA_normLang__ = normLang; }catch(e){}
  ...
  // =========================================================
  // ULYDIA — Metier Page — MONO BUNDLE — v2.9 DESIGNFULL — FINAL.ARROWS+MDBOLD.PROD
...
function getMPB(){
  ...
  // FIX48: Supabase Worker returns `mpb` as a single object (not an array). Handle that.
  if ((!list || !list.length) && pd && pd.mpb && typeof pd.mpb === 'object' && !Array.isArray(pd.mpb)) {
    try {
      var obj = pd.mpb;
      var iso = (pd.iso || pd.country || pd.country_iso || (pd.country_data && (pd.country_data.iso || pd.country_data.code || pd.country_data.country_code)) || '');
      var slug = (pd.slug || pd.metier_slug || pd.job_slug || (pd.job && (pd.job.slug || pd.job.fiche_slug || pd.job.metier_slug)) || '');
      if (iso && !obj.iso) obj.iso = iso;
      if (slug && !obj.metier_slug && !obj.slug) obj.metier_slug = slug;
      list = [obj];
    } catch(e){ /* ignore */ }
  }
...
.u-sal-v{font-weight:900;color:rgba(17,24,39,92);}
      ...
      .u-arrowline{display:flex;gap:12px;align-items:flex-start;padding:12px 12px;border:1px solid var(--border);border-radius:14px;background:#fff;}
      .u-arrowline-ico{width:38px;height:38px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex:0 0 auto;background:rgba(99,102,241,.10);border:1px solid rgba(99,102,241,.18);font-size:18px;font-weight:800;}
      .u-arrowline-txt{font-size:14px;line-height:1.55;color:var(--text);font-weight:650;}.u-arrowline-txt p{margin:0;}.u-arrowline-txt>*:first-child{margin-top:0;}.u-arrowline-txt>*:last-child{margin-bottom:0;}
      .u-arrowinline{font-weight:900;margin-right:8px;display:inline-block;vertical-align:middle;position:relative;top:2px;line-height:1;}
...
`;
  ...
  function card(title, headerTheme, html){
    ...
    if (html) b.innerHTML = postProcessRichHTML(html);
    c.appendChild(b);
    return c;
  }
...
function cleanRichHTML(html){
  ...
  // --- Markdown **bold** + Arrow alignment (FINAL.ARROWS+MDBOLD) ---
  function mdBoldToHtml(s){
    if(!s || typeof s !== "string") return s;
    return s.replace(/\*\*([^*]+?)\*\*/g, "<strong>$1</strong>");
  }

  function mergeArrowOnlyParagraphs(root){
    try{
      if(!root) return;
      ...
      for (var i=0;i<kids.length;i++){
        ...
        if (t === "→" || t === "➜" || t === "➡" || t === "➤"){
          ...
          if (next){
            var wrap = document.createElement("div");
            wrap.className = "u-arrowline";
            var a = document.createElement("span");
            a.className = "u-arrowline-ico";
            a.textContent = t;
            var b = document.createElement("div");
            b.className = "u-arrowline-txt";
            b.innerHTML = next.innerHTML || "";
            wrap.appendChild(a);
            wrap.appendChild(b);
            el.parentNode.insertBefore(wrap, el);
            try{ el.parentNode.removeChild(el);}catch(e){}
            try{ next.parentNode.removeChild(next);}catch(e){}
  ...
  function inlineArrowInElement(el){
    try{
      if(!el || !el.innerHTML) return;
      var html = el.innerHTML;
      // Handle patterns like: "→<br>Text..." or "→<br/><br/>Text..."
      html = html.replace(/^\s*(→|➜|➡|➤)\s*(?:<br\s*\/?>\s*)+/i, '<span class="u-arrowinline">$1</span> ');
      el.innerHTML = html;
    }catch(e){}
  }

  function postProcessRichHTML(html){
    html = cleanRichHTML(html);
    if (!html) return "";
    html = mdBoldToHtml(html);
    try{
      var d = document.createElement("div");
      d.innerHTML = html;

      // 1) merge arrow-only paragraphs with the next paragraph
      mergeArrowOnlyParagraphs(d);

      // 2) inline arrow if arrow + <br> inside same element
      var els = d.querySelectorAll("p, li, div");
      for (var i=0;i<els.length;i++){
        inlineArrowInElement(els[i]);
      }

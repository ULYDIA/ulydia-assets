...
// FINAL v1.3 (UI)
// Fixes:
// - Country list can come from window.__ULYDIA_COUNTRIES__ OR DOM (js-country-*)
// - Auto-select country from ?country=XX on load
// - If #metier-results is missing/empty, UI stays usable (metier select disabled + message)
// =========================================================
(() => {
  if (window.__ULYDIA_FS_FILTERS_UI_V13__) return;
  window.__ULYDIA_FS_FILTERS_UI_V13__ = true;
...
#ulydia-metier-filters .uf-field{display:flex;flex-direction:column;gap:6px;}
...
#ulydia-metier-filters select{
  width:100%;height:46px;padding:0 42px 0 14px;border-radius:14px;
  border:1px solid rgba(17,24,39,.12);background:#f9fafb;color:var(--uf-text);
  font-size:14px;font-weight:650;outline:none;appearance:none;-webkit-appearance:none;-moz-appearance:none;
}
#ulydia-metier-filters select:focus{background:#fff;border-color:var(--uf-accent);box-shadow:0 0 0 4px var(--uf-accent-soft);}
#ulydia-metier-filters .uf-chevron{position:absolute;right:12px;top:50%;transform:translateY(-50%);width:18px;height:18px;pointer-events:none;opacity:.55;}
#ulydia-metier-filters .uf-row2{margin-top:10px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;}
#ulydia-metier-filters .uf-count{font-size:13px;color:var(--uf-muted);font-weight:650;}
#ulydia-metier-filters .uf-reset{
  border:1px solid rgba(17,24,39,.12);background:#fff;color:var(--uf-text);
  height:36px;padding:0 12px;border-radius:12px;font-weight:700;font-size:13px;cursor:pointer;
}
#ulydia-metier-filters .uf-reset:hover{background:#f9fafb;}
#ulydia-metier-filters .uf-hint{font-size:12px;color:var(--uf-muted);font-weight:600;}
@media (max-width:820px){#ulydia-metier-filters .uf-grid{grid-template-columns:1fr;}#ulydia-metier-filters{margin:16px auto 10px;}}
`;
  ...
  // ---------------- Countries ----------------
  function countriesFromWindow() {
    const arr = window.__ULYDIA_COUNTRIES__;
    if (!Array.isArray(arr) || !arr.length) return [];
    // Expect: { iso, lang, name? }
    return arr
      .map(c => ({
        iso: norm(c.iso || "").toUpperCase(),
        name: norm(c.name || c.label || c.iso || ""),
        lang: norm(c.lang || c.language || "").toLowerCase()
      }))
      .filter(c => c.iso);
  }

  function countriesFromDOM() {
    const out = [];
  ...
  function getCountries() {
    const merged = [...countriesFromWindow(), ...countriesFromDOM()];
    const map = new Map();
    merged.forEach(c => {
      if (!c.iso) return;
      if (!map.has(c.iso)) map.set(c.iso, c);
      // enrich if existing is missing name/lang
      const ex = map.get(c.iso);
      if (ex) {
        if (!ex.name && c.name) ex.name = c.name;
        if (!ex.lang && c.lang) ex.lang = c.lang;
      }
    });
    const out = Array.from(map.values());
    out.sort((a,b) => (a.name || a.iso).localeCompare(b.name || b.iso));
    return out;
  }
  ...
  async function rebuildMetierSelect() {
    ...
    const root = $(CFG.results);
    if (!root) {
      fillSelect(selMetier, "Métier", [], m => m.slug, m => m.name);
      setDisabled(selMetier, true);
      updateCount();
      setHint("Astuce: ajoute une liste CMS cachée (#metier-results) si tu veux filtrer depuis cette page.");
      return;
    }
    setHint("");
  ...
  async function boot() {
    ...
    // Auto-select country from URL
    const q = parseQuery();
    if (q.country) {
      // ensure option exists; if not, add it so the user can at least see it
      ...
      // reset sector when country changes
      selSector.value = "";
      clickTrigger("sector", "");
    }

    // Reset button: reset sector only (keep country)

...
var q = getQuery();
    ...
    async function go(){
    // If navigation overlay is already shown by the search bar, avoid a second loader
    var __navFlag = false;
    try{ __navFlag = sessionStorage.getItem("ulydia_nav_loading") === "1"; }catch(_){}
    if(!__navFlag){
...
// Keep loader homogeneous: ensure it is visible at least MIN_LOADER_MS to avoid flicker
    ...
    if (!ctx.fiche) {
      ...
      // FIX: content is language-based, not country-based.
      // Some countries may not have MPB/sponsor data, but the fiche should still be shown.
      // Fallback: retry fetching fiche using a default ISO (FR) while keeping ctx.iso for sponsorship.
      try{
        if(ctx && ctx.slug && ctx.iso && String(ctx.iso).toUpperCase() !== "FR"){
          var __u = (WORKER||"") + "/v1/metier-page?slug=" + encodeURIComponent(ctx.slug) + "&iso=FR";
          // preserve lang if present
          if(ctx.lang) __u += "&lang=" + encodeURIComponent(ctx.lang);
          var __d = await fetchJSON(__u);
          if(__d && __d.data && __d.data.fiche){
            ctx.fiche = __d.data.fiche;
          }
        }
      }catch(_){}
    ...
    try{ window.hideMetierLoader && window.hideMetierLoader(); }catch(e){}
    renderPage(ctx);
    try{ window.dispatchEvent(new CustomEvent('ulydia:metier:rendered', { detail: { slug: ctx.slug, iso: ctx.iso } })); }catch(e){}

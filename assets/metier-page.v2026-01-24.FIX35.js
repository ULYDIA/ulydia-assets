...
*/
(() => {
  if (window.__ULYDIA_METIER_PAGE_FIX35__) return;
  window.__ULYDIA_METIER_PAGE_FIX35__ = true;

  const DEBUG = true;
  const log = (...a) => { if (DEBUG) console.log("[metier-page FIX35]", ...a); };
...
`;
  ...
  function findBlocFor(metierName, iso){
    const all = readCMSBlocs();
    const hit = all.find(x => (x.iso || "").toUpperCase() === (iso || "").toUpperCase() && (x.metier || "") === (metierName || ""));
    return { all, hit };
  }
      ...
      <div class="ul-metier">
        <div class="ul-container">
          <div class="ul-top">
            ...
            <div class="ul-sub">iso=${escapeHtml(iso)} • script=FIX35 • chargement…</div>
          </div>
        </div>
      </div>
    `;
      ...
      <div class="ul-metier">
        <div class="ul-container">
          <div class="ul-top">
            ...
            <div class="ul-sub">iso=${escapeHtml(iso)} • script=FIX35</div>
            ${bannerHtml}
          </div>
  ...
  function main(){
    ...
    waitForCMS(async () => {
      const { all, hit } = findBlocFor(metierTitle, iso);
  ...
  try { main(); } catch(e) { console.error("[metier-page FIX35] fatal", e); }
})();
